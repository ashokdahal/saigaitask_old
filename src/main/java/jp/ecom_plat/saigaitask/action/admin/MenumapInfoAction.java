...
import jp.ecom_plat.saigaitask.form.db.MenumapInfoForm;
...
@RequestMapping("/admin/menumapInfo")
public class MenumapInfoAction extends AbstractAdminAction<MenumapInfo> {
	protected MenumapInfoForm menumapInfoForm;
	...
	public String index() {
		return "/admin/menumapInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = menumapInfoService.getCount(conditions);

			// 一覧内容取得
			List<MenumapInfo> list = new ArrayList<MenumapInfo>();
			if(count > 0){
				list = menumapInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MenumapInfoForm menumapInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MenumapInfo.class, menumapInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				menumapInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MenumapInfo menumapInfo = menumapInfoService.findById(Long.decode(menumapInfoForm.id));
					...
					if(menumapInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MenumapInfo.class, menumapInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MenumapInfo.class, menumapInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							menumapInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(menumapInfoForm.id));
				int count = menumapInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = menumapInfoService.findById(Long.decode(menumapInfoForm.id));
					//削除実行
					try{
						menumapInfoService.delete(entity);
