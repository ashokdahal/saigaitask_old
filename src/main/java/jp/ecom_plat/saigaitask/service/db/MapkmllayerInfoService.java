...
import static jp.ecom_plat.saigaitask.entity.Names.mapkmllayerInfo;
...
import jp.ecom_plat.saigaitask.entity.names.MapkmllayerInfoNames;
import jp.ecom_plat.saigaitask.service.AbstractService;

@org.springframework.stereotype.Repository
public class MapkmllayerInfoService extends AbstractService<MapkmllayerInfo> {

    public MapkmllayerInfo findById(Long id) {
	 ...
	 * @return 地図参照レイヤ情報リスト
	 */
	public List<MapkmllayerInfo> findByMenuid(long menuid) {

		// Select
		AutoSelect<MapkmllayerInfo> select = select();

		// Where
		SimpleWhere where = new SimpleWhere();
		where.eq(MapkmllayerInfoNames.menuinfoid(), menuid);
		where.eq(MapkmllayerInfoNames.valid(), true);
		select.where(where);

		// OrderBy
		select.orderBy(
				Operations.asc(MapkmllayerInfoNames.disporder())
		);
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(mapkmllayerInfo().menuInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<MapkmllayerInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = mapkmllayerInfo().disporder().toString();
		}
		...
		if(!mapkmllayerInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(mapkmllayerInfo().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		...
		List<MapkmllayerInfo> list = findByCondition(conditions, mapkmllayerInfo().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(MapkmllayerInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
