...
import jp.ecom_plat.saigaitask.form.db.NoticetypeMasterForm;
...
@RequestMapping("/admin/noticetypeMaster")
public class NoticetypeMasterAction extends AbstractAdminAction<NoticetypeMaster> {
	protected NoticetypeMasterForm noticetypeMasterForm;
	...
	public String index() {
		return "/admin/noticetypeMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = noticetypeMasterService.getCount(conditions);

			// 一覧内容取得
			List<NoticetypeMaster> list = new ArrayList<NoticetypeMaster>();
			if(count > 0){
				list = noticetypeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = noticetypeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute NoticetypeMasterForm noticetypeMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			NoticetypeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(NoticetypeMaster.class, noticetypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				noticetypeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					NoticetypeMaster noticetypeMaster = noticetypeMasterService.findById(Integer.parseInt(noticetypeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(noticetypeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, NoticetypeMaster.class, noticetypeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(NoticetypeMaster.class, noticetypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							noticetypeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(noticetypeMasterForm.id));
				int count = noticetypeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = noticetypeMasterService.findById(Integer.parseInt(noticetypeMasterForm.id));
					//削除実行
					try{
						noticetypeMasterService.delete(entity);
