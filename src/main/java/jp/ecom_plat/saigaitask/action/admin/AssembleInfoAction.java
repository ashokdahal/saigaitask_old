...
import jp.ecom_plat.saigaitask.form.db.AssembleInfoForm;
...
@RequestMapping("/admin/assembleInfo")
public class AssembleInfoAction extends AbstractAdminAction<AssembleInfo> {
	protected AssembleInfoForm assembleInfoForm;
	...
	public String index() {
		return "/admin/assembleInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.assembleInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = assembleInfoService.getCount(conditions);

			// 一覧内容取得
			List<AssembleInfo> list = new ArrayList<AssembleInfo>();
			if(count > 0){
				list = assembleInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute AssembleInfoForm assembleInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(AssembleInfo.class, assembleInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				assembleInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					AssembleInfo assembleInfo = assembleInfoService.findById(Long.decode(assembleInfoForm.id));
					...
					if(assembleInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, AssembleInfo.class, assembleInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(AssembleInfo.class, assembleInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							assembleInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(assembleInfoForm.id));
				int count = assembleInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = assembleInfoService.findById(Long.decode(assembleInfoForm.id));
					//削除実行
					try{
						assembleInfoService.delete(entity);
