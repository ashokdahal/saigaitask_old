...
import jp.ecom_plat.saigaitask.form.db.ObservmenuInfoForm;
...
@RequestMapping("/admin/observmenuInfo")
public class ObservmenuInfoAction extends AbstractAdminAction<ObservmenuInfo> {
	protected ObservmenuInfoForm observmenuInfoForm;
	...
	public String index() {
		return "/admin/observmenuInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != Constants.ADMIN_LOCALGOVINFOID){
				conditions.put(Names.observmenuInfo().menuInfo().menutasktypeInfo().localgovinfoid().toString(),localgovinfoid);
			...
			// 一覧件数取得
			int count = observmenuInfoService.getCount(conditions);

			// 一覧内容取得
			List<ObservmenuInfo> list = new ArrayList<ObservmenuInfo>();
			if(count > 0){
				list = observmenuInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ObservmenuInfoForm observmenuInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ObservmenuInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ObservmenuInfo.class, observmenuInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				observmenuInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ObservmenuInfo observmenuInfo = observmenuInfoService.findById(Long.decode(observmenuInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(observmenuInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ObservmenuInfo.class, observmenuInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ObservmenuInfo.class, observmenuInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							observmenuInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(observmenuInfoForm.id));
				int count = observmenuInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = observmenuInfoService.findById(Long.decode(observmenuInfoForm.id));
					//削除実行
					try{
						observmenuInfoService.delete(entity);
			...
			if(StringUtils.isEmpty(selectTagColumn)
				...
				if("localgov_info".equals(tableName)){
					...
					if(localgovinfoid != ADMIN_LOCALGOVINFOID){
						...
						// menu_infoにlocalgovinfoidはないが、Joinしているmenutasktype_infoにはある。
...
//						}
				...
				// メニュー種別:監視観測で絞込
				if("menu_info".equals(tableName)) {
					conditions.put(Names.menuInfo().menutypeid().toString(), Menutype.OBSERVE);
				}
