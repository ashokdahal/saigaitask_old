...
import static jp.ecom_plat.saigaitask.entity.Names.telemeterpointInfo;
...
public class TelemeterpointInfoService extends AbstractService<TelemeterpointInfo> {

    public TelemeterpointInfo findById(Long id) {
        return select().id(id).getSingleResult();
    }

    public TelemeterpointInfo findByPointcode(Long pointcode) {
    	return select().where(
    			eq(telemeterpointInfo().pointcode(), pointcode)).limit(1).getSingleResult();
    }
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(telemeterpointInfo().telemeterofficeInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<TelemeterpointInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = telemeterpointInfo().id().toString();
		}
		...
		if(!telemeterpointInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(telemeterpointInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(TelemeterpointInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
