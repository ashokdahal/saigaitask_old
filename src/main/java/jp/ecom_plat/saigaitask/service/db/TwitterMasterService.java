...
import static jp.ecom_plat.saigaitask.entity.Names.twitterMaster;
...
public class TwitterMasterService extends AbstractService<TwitterMaster> {

    public TwitterMaster findById(Integer id) {
	 ...
	 * @return 検索結果
	 */
	public List<TwitterMaster> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = twitterMaster().id().toString();
		}
		...
		if(!twitterMaster().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(twitterMaster().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(TwitterMaster entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
