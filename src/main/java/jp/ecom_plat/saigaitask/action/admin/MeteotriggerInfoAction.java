...
import jp.ecom_plat.saigaitask.form.db.MeteotriggerInfoForm;
...
@RequestMapping("/admin/meteotriggerInfo")
public class MeteotriggerInfoAction extends AbstractAdminAction<MeteotriggerInfo> {
	protected MeteotriggerInfoForm meteotriggerInfoForm;
	...
	public String index() {
		return "/admin/meteotriggerInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = meteotriggerInfoService.getCount(conditions);

			// 一覧内容取得
			List<MeteotriggerInfo> list = new ArrayList<MeteotriggerInfo>();
			if(count > 0){
				list = meteotriggerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteotriggerInfoForm meteotriggerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteotriggerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteotriggerInfo.class, meteotriggerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteotriggerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteotriggerInfo meteotriggerInfo = meteotriggerInfoService.findById(Long.decode(meteotriggerInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteotriggerInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteotriggerInfo.class, meteotriggerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteotriggerInfo.class, meteotriggerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteotriggerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteotriggerInfoForm.id));
				int count = meteotriggerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteotriggerInfoService.findById(Long.decode(meteotriggerInfoForm.id));
					//削除実行
					try{
						meteotriggerInfoService.delete(entity);
