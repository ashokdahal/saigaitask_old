...
import jp.ecom_plat.saigaitask.form.db.FacebookpostdefaultInfoForm;
...
/**
 * Facebook投稿先デフォルト情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/facebookpostdefaultInfo")
public class FacebookpostdefaultInfoAction extends AbstractAdminAction<FacebookpostdefaultInfo> {
	protected FacebookpostdefaultInfoForm facebookpostdefaultInfoForm;
	...
	public String index() {
		return "/admin/facebookpostdefaultInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = facebookpostdefaultInfoService.getCount(conditions);

			// 一覧内容取得
			List<FacebookpostdefaultInfo> list = new ArrayList<FacebookpostdefaultInfo>();
			if(count > 0){
				list = facebookpostdefaultInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute FacebookpostdefaultInfoForm facebookpostdefaultInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			FacebookpostdefaultInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(FacebookpostdefaultInfo.class, facebookpostdefaultInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				facebookpostdefaultInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					FacebookpostdefaultInfo facebookpostdefaultInfo = facebookpostdefaultInfoService.findById(Long.decode(facebookpostdefaultInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(facebookpostdefaultInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, FacebookpostdefaultInfo.class, facebookpostdefaultInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(FacebookpostdefaultInfo.class, facebookpostdefaultInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							facebookpostdefaultInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(facebookpostdefaultInfoForm.id));
				int count = facebookpostdefaultInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = facebookpostdefaultInfoService.findById(Long.decode(facebookpostdefaultInfoForm.id));
					//削除実行
					try{
						facebookpostdefaultInfoService.delete(entity);
