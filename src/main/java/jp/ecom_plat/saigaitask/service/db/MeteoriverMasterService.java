...
import static jp.ecom_plat.saigaitask.entity.Names.meteoriverMaster;
...
public class MeteoriverMasterService extends AbstractService<MeteoriverMaster> {

    public MeteoriverMaster findById(Integer id) {
	 ...
	 * @return 検索結果
	 */
	public List<MeteoriverMaster> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = meteoriverMaster().disporder().toString();
		}
		...
		if(!meteoriverMaster().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(meteoriverMaster().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		conditions.put(meteoriverMaster().disporder().toString()+"_IS_NOT_NULL", Boolean.TRUE);
		List<MeteoriverMaster> list = findByCondition(conditions, meteoriverMaster().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(MeteoriverMaster entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
