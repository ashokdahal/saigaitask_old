...
import jp.ecom_plat.saigaitask.form.db.SummarylistInfoForm;
...
@RequestMapping("/admin/summarylistInfo")
public class SummarylistInfoAction extends AbstractAdminAction<SummarylistInfo> {
	protected SummarylistInfoForm summarylistInfoForm;
	...
	public String index() {
		return "/admin/summarylistInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.summarylistInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = summarylistInfoService.getCount(conditions);

			// 一覧内容取得
			List<SummarylistInfo> list = new ArrayList<SummarylistInfo>();
			if(count > 0){
				list = summarylistInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute SummarylistInfoForm summarylistInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			SummarylistInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(SummarylistInfo.class, summarylistInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				summarylistInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					SummarylistInfo summarylistInfo = summarylistInfoService.findById(Long.decode(summarylistInfoForm.id));
					...
					if(summarylistInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, SummarylistInfo.class, summarylistInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(SummarylistInfo.class, summarylistInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							summarylistInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(summarylistInfoForm.id));
				int count = summarylistInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = summarylistInfoService.findById(Long.decode(summarylistInfoForm.id));
					//削除実行
					try{
						summarylistInfoService.delete(entity);
