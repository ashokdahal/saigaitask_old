...
import jp.ecom_plat.saigaitask.form.db.NoticeaddressInfoForm;
...
@RequestMapping("/admin/noticeaddressInfo")
public class NoticeaddressInfoAction extends AbstractAdminAction<NoticeaddressInfo> {
	protected NoticeaddressInfoForm noticeaddressInfoForm;
	...
	public String index() {
		return "/admin/noticeaddressInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.noticeaddressInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = noticeaddressInfoService.getCount(conditions);

			// 一覧内容取得
			List<NoticeaddressInfo> list = new ArrayList<NoticeaddressInfo>();
			if(count > 0){
				list = noticeaddressInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute NoticeaddressInfoForm noticeaddressInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			NoticeaddressInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(NoticeaddressInfo.class, noticeaddressInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				noticeaddressInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					NoticeaddressInfo noticeaddressInfo = noticeaddressInfoService.findById(Long.decode(noticeaddressInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(noticeaddressInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, NoticeaddressInfo.class, noticeaddressInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(NoticeaddressInfo.class, noticeaddressInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							noticeaddressInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(noticeaddressInfoForm.id));
				int count = noticeaddressInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = noticeaddressInfoService.findById(Long.decode(noticeaddressInfoForm.id));
					//削除実行
					try{
						noticeaddressInfoService.delete(entity);
