...
import jp.ecom_plat.saigaitask.form.db.TablecalculateInfoForm;
...
@RequestMapping("/admin/tablecalculateInfo")
public class TablecalculateInfoAction extends AbstractAdminAction<TablecalculateInfo> {
	protected TablecalculateInfoForm tablecalculateInfoForm;
	...
	public String index() {
		return "/admin/tablecalculateInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = tablecalculateInfoService.getCount(conditions);

			// 一覧内容取得
			List<TablecalculateInfo> list = new ArrayList<TablecalculateInfo>();
			if(count > 0){
				list = tablecalculateInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TablecalculateInfoForm tablecalculateInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TablecalculateInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TablecalculateInfo.class, tablecalculateInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				tablecalculateInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TablecalculateInfo tablecalculateInfo = tablecalculateInfoService.findById(Long.decode(tablecalculateInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(tablecalculateInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TablecalculateInfo.class, tablecalculateInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TablecalculateInfo.class, tablecalculateInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							tablecalculateInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(tablecalculateInfoForm.id));
				int count = tablecalculateInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = tablecalculateInfoService.findById(Long.decode(tablecalculateInfoForm.id));
					//削除実行
					try{
						tablecalculateInfoService.delete(entity);
