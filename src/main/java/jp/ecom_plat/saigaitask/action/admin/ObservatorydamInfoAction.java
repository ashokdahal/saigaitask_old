...
import jp.ecom_plat.saigaitask.form.db.ObservatorydamInfoForm;
...
@RequestMapping("/admin/observatorydamInfo")
public class ObservatorydamInfoAction extends AbstractAdminAction<ObservatorydamInfo> {
	protected ObservatorydamInfoForm observatorydamInfoForm;
	...
	public String index() {
		return "/admin/observatorydamInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.observatorydamInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = observatorydamInfoService.getCount(conditions);

			// 一覧内容取得
			List<ObservatorydamInfo> list = new ArrayList<ObservatorydamInfo>();
			if(count > 0){
				list = observatorydamInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ObservatorydamInfoForm observatorydamInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ObservatorydamInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ObservatorydamInfo.class, observatorydamInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				observatorydamInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ObservatorydamInfo observatorydamInfo = observatorydamInfoService.findById(Long.decode(observatorydamInfoForm.id));
					...
					if(observatorydamInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ObservatorydamInfo.class, observatorydamInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ObservatorydamInfo.class, observatorydamInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							observatorydamInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(observatorydamInfoForm.id));
				int count = observatorydamInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = observatorydamInfoService.findById(Long.decode(observatorydamInfoForm.id));
					//削除実行
					try{
						observatorydamInfoService.delete(entity);
