...
import jp.ecom_plat.saigaitask.form.db.ObservatoryrainlayerInfoForm;
...
/**
 * 雨量観測地点レイヤ情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/observatoryrainlayerInfo")
public class ObservatoryrainlayerInfoAction extends AbstractAdminAction<ObservatoryrainlayerInfo> {
	protected ObservatoryrainlayerInfoForm observatoryrainlayerInfoForm;
	...
	public String index() {
		return "/admin/observatoryrainlayerInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.observatoryrainlayerInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = observatoryrainlayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<ObservatoryrainlayerInfo> list = new ArrayList<ObservatoryrainlayerInfo>();
			if(count > 0){
				list = observatoryrainlayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ObservatoryrainlayerInfoForm observatoryrainlayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ObservatoryrainlayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ObservatoryrainlayerInfo.class, observatoryrainlayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				observatoryrainlayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ObservatoryrainlayerInfo observatoryrainlayerInfo = observatoryrainlayerInfoService.findById(Long.decode(observatoryrainlayerInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(observatoryrainlayerInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ObservatoryrainlayerInfo.class, observatoryrainlayerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ObservatoryrainlayerInfo.class, observatoryrainlayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							observatoryrainlayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(observatoryrainlayerInfoForm.id));
				int count = observatoryrainlayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = observatoryrainlayerInfoService.findById(Long.decode(observatoryrainlayerInfoForm.id));
					//削除実行
					try{
						observatoryrainlayerInfoService.delete(entity);
