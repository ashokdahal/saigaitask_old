...
import jp.ecom_plat.saigaitask.form.db.MenuprocessInfoForm;
...
@RequestMapping("/admin/menuprocessInfo")
public class MenuprocessInfoAction extends AbstractAdminAction<MenuprocessInfo> {
	protected MenuprocessInfoForm menuprocessInfoForm;
	...
	public String index() {
		return "/admin/menuprocessInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = menuprocessInfoService.getCount(conditions);

			// 一覧内容取得
			List<MenuprocessInfo> list = new ArrayList<MenuprocessInfo>();
			if(count > 0){
				list = menuprocessInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = menuprocessInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MenuprocessInfoForm menuprocessInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MenuprocessInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MenuprocessInfo.class, menuprocessInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				menuprocessInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MenuprocessInfo menuprocessInfo = menuprocessInfoService.findById(Long.decode(menuprocessInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(menuprocessInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MenuprocessInfo.class, menuprocessInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MenuprocessInfo.class, menuprocessInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							menuprocessInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(menuprocessInfoForm.id));
				int count = menuprocessInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = menuprocessInfoService.findById(Long.decode(menuprocessInfoForm.id));
					//削除実行
					try{
						menuprocessInfoService.deleteLogically(entity);
