...
import jp.ecom_plat.saigaitask.form.db.JsonimportlayerInfoForm;
...
@RequestMapping("/admin/jsonimportlayerInfo")
public class JsonimportlayerInfoAction extends AbstractAdminAction<JsonimportlayerInfo> {
	...
	public String index() {
		return "/admin/jsonimportlayerInfo/index";
	}
	...
	public String jqgridindex(
		...
		try{
			...
			// 一覧件数取得
			int count = jsonimportlayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<JsonimportlayerInfo> list = new ArrayList<JsonimportlayerInfo>();
			if(count > 0){
				list = jsonimportlayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JsonimportlayerInfoForm jsonimportlayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			JsonimportlayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JsonimportlayerInfo.class, jsonimportlayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				jsonimportlayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JsonimportlayerInfo jsonimportlayerInfo = jsonimportlayerInfoService.findById(Long.decode(jsonimportlayerInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(jsonimportlayerInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JsonimportlayerInfo.class, jsonimportlayerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JsonimportlayerInfo.class, jsonimportlayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							jsonimportlayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(jsonimportlayerInfoForm.id));
				int count = jsonimportlayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = jsonimportlayerInfoService.findById(Long.decode(jsonimportlayerInfoForm.id));
					//削除実行
					try{
						jsonimportlayerInfoService.delete(entity);
