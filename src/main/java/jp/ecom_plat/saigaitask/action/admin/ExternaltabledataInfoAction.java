...
import jp.ecom_plat.saigaitask.form.db.ExternaltabledataInfoForm;
...
@RequestMapping("/admin/externaltabledataInfo")
public class ExternaltabledataInfoAction extends AbstractAdminAction<ExternaltabledataInfo> {
	protected ExternaltabledataInfoForm externaltabledataInfoForm;
	...
	public String index() {
		return "/admin/externaltabledataInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧内容取得
			List<ExternaltabledataInfo> list = new ArrayList<ExternaltabledataInfo>();
			if(count > 0){
				list = externaltabledataInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ExternaltabledataInfoForm externaltabledataInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ExternaltabledataInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ExternaltabledataInfo.class, externaltabledataInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				externaltabledataInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ExternaltabledataInfo externaltabledataInfo = externaltabledataInfoService.findById(Long.decode(externaltabledataInfoForm.id));
					...
					if(externaltabledataInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ExternaltabledataInfo.class, externaltabledataInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ExternaltabledataInfo.class, externaltabledataInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							externaltabledataInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(externaltabledataInfoForm.id));
				int count = externaltabledataInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = externaltabledataInfoService.findById(Long.decode(externaltabledataInfoForm.id));
					//削除実行
					try{
						externaltabledataInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	...
	@ResponseBody
	public String insert(@ModelAttribute ExternaltabledataInfoForm externaltabledataInfoForm) {
		ExternaltabledataInfo externaltabledataInfo = Beans.createAndCopy(ExternaltabledataInfo.class, externaltabledataInfoForm).execute();
		Long menuinfoid = externaltabledataInfo.menuinfoid;
		if(menuinfoid!=null) {
