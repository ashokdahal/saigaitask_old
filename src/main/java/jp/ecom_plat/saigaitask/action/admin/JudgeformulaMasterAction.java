...
import jp.ecom_plat.saigaitask.form.db.JudgeformulaMasterForm;
...
@RequestMapping("/admin/judgeformulaMaster")
public class JudgeformulaMasterAction extends AbstractAdminAction<JudgeformulaMaster> {
	protected JudgeformulaMasterForm judgeformulaMasterForm;
	...
	public String index() {
		return "/admin/judgeformulaMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = judgeformulaMasterService.getCount(conditions);

			// 一覧内容取得
			List<JudgeformulaMaster> list = new ArrayList<JudgeformulaMaster>();
			if(count > 0){
				list = judgeformulaMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = judgeformulaMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JudgeformulaMasterForm judgeformulaMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			JudgeformulaMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JudgeformulaMaster.class, judgeformulaMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				judgeformulaMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JudgeformulaMaster judgeformulaMaster = judgeformulaMasterService.findById(Integer.parseInt(judgeformulaMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(judgeformulaMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JudgeformulaMaster.class, judgeformulaMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JudgeformulaMaster.class, judgeformulaMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							judgeformulaMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(judgeformulaMasterForm.id));
				int count = judgeformulaMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = judgeformulaMasterService.findById(Integer.parseInt(judgeformulaMasterForm.id));
					//削除実行
					try{
						judgeformulaMasterService.delete(entity);
