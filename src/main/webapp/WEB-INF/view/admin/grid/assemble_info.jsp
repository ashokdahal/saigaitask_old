...
function assemble_info(subgrid_id, row_id){
	var tabName = 'assemble_info';
	var assemble_info_gridId = 'main';
	if(subgrid_id != null){
		assemble_info_gridId = subgrid_id;
	}
	var assemble_info_tabId = tabName;
	if(row_id != null){
		assemble_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var assemble_info_pagerId = assemble_info_tabId+'Nav';
	$('#'+assemble_info_gridId).append('<table id="'+assemble_info_tabId+'" class="scroll"></table><div id="'+assemble_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+assemble_info_tabId).jqGrid({
		caption: '<%=lang.__("Staff assembly info")%>',
		mtype: 'POST',
		...
		colModel:[
			...
			editoptions:{value:'',
				...
				readonly:false
					...
					buildSelect: function (data) {
						...
						var rowid = $('#'+assemble_info_tabId).jqGrid('getGridParam', 'selrow');
						var selectedId = null;
						if(rowid != null){
							//編集の場合
							var rowdata = $('#'+assemble_info_tabId).jqGrid('getRowData', rowid);
							selectedId = rowdata.localgovinfoid;
						}else{
						}
						return createSelectTag(rowid, selectTag, selectedId, 'assemble_info', 'localgov_info');
					}
			},
			...
			editoptions:{value:'',
				...
				readonly:false
					...
					buildSelect: function (data) {
						...
						return createSelectTag(rowid, selectTag, selectedId, 'assemble_info', 'stationclass_info');
					}
			},
			editrules:{required:true, number:false, integer:false, email:false, edithidden:false},
			sortable:true,
			...
			editoptions:{value:'',
				...
				delimiter:'|d|',
				readonly:false
					,dataUrl:convertTabNameToMethodMame(tabName)+'/createSelectTag/noticetemplateid/notice_template/id/title',
					//フォームのプルダウン作成
					buildSelect: function (data) {
						var selectTag = $.parseJSON(data).selectTag;
						var rowid = $('#'+assemble_info_tabId).jqGrid('getGridParam', 'selrow');
						...
						if(rowid != null){
							//編集の場合
							var rowdata = $('#'+assemble_info_tabId).jqGrid('getRowData', rowid);
							selectedId = rowdata.noticetemplateid;
						}else{
						}
						return createSelectTag(rowid, selectTag, selectedId, 'assemble_info', 'notice_template');
					}
			},
			editrules:{required:true, number:false, integer:false, email:false, edithidden:false},
			sortable:true,
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Template ID(*)")%>'},
			hidden:false},
			...
			editoptions:{value:'',
				...
				readonly:false
					...
					buildSelect: function (data) {
						...
						return createSelectTag(rowid, selectTag, selectedId, 'assemble_info', 'noticegroup_info');
					}
			},
			editrules:{required:true, number:false, integer:false, email:false, edithidden:false},
			sortable:true,
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Gathering title(*)")%>'},
			hidden:false},
			{name:'note',
			width:'150',
			align:'left',
		...
		beforeProcessing:function(data){
			...
			$('#'+assemble_info_tabId).jqGrid('setColProp', 'localgovinfoid', {editoptions:{value:str}});
			...
			$('#'+assemble_info_tabId).jqGrid('setColProp', 'stationclassinfoid', {editoptions:{value:str}});
			...
			for(i=0; i < data.rows.length; i++){
				...
				var ref = data.rows[i].noticeTemplate;
				if(ref != null){
					var refkey = ref.id;
					var refval = ref.title;
					str += refkey + ':' + refkey + '：' + refval;
				}

			}
			$('#'+assemble_info_tabId).jqGrid('setColProp', 'noticetemplateid', {editoptions:{value:str}});
			...
			$('#'+assemble_info_tabId).jqGrid('setColProp', 'noticegroupinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#assemble_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+assemble_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+assemble_info_tabId).jqGrid('navGrid',
			'#'+assemble_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+assemble_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+assemble_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Staff assembly info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					var rowdata = $('#'+assemble_info_tabId).jqGrid('getRowData', rowid);
					s += '&noticetemplateid='+rowdata.noticetemplateid;
					var rowdata = $('#'+assemble_info_tabId).jqGrid('getRowData', rowid);
					s += '&noticegroupinfoid='+rowdata.noticegroupinfoid;
					$('#'+assemble_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+assemble_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+assemble_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+assemble_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+assemble_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+assemble_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+assemble_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Staff assembly info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+assemble_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Staff assembly info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+assemble_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+assemble_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Staff assembly info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+assemble_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ assemble_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_assemble_info #id').val('');
					}else{
...
//	$('#'+assemble_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+assemble_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
