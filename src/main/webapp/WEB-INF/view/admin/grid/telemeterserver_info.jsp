...
function telemeterserver_info(subgrid_id, row_id){
	var tabName = 'telemeterserver_info';
	var telemeterserver_info_gridId = 'main';
	if(subgrid_id != null){
		telemeterserver_info_gridId = subgrid_id;
	}
	var telemeterserver_info_tabId = tabName;
	if(row_id != null){
		telemeterserver_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var telemeterserver_info_pagerId = telemeterserver_info_tabId+'Nav';
	$('#'+telemeterserver_info_gridId).append('<table id="'+telemeterserver_info_tabId+'" class="scroll"></table><div id="'+telemeterserver_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+telemeterserver_info_tabId).jqGrid({
		caption: '<%=lang.__("Telemeter server info")%>',
		mtype: 'POST',
		...
		colModel:[
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+telemeterserver_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+telemeterserver_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.localgovinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'telemeterserver_info', 'localgov_info', false);
						}
				},
			...
			{name:'interval',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Interval (in seconds)(*)")%>'},
				hidden:false},
			{name:'delay',
				width:'150',
				align:'right',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Delay(*)")%>'},
				hidden:false},
			{name:'note',
				width:'150',
				align:'left',
		...
		pager: telemeterserver_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+telemeterserver_info_tabId).jqGrid('setColProp', 'localgovinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#telemeterserver_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+telemeterserver_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+telemeterserver_info_tabId).jqGrid('navGrid',
			'#'+telemeterserver_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+telemeterserver_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+telemeterserver_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Telemeter server info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+telemeterserver_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+telemeterserver_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
	  				var pagenum = $('#'+telemeterserver_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+telemeterserver_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+telemeterserver_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+telemeterserver_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+telemeterserver_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Telemeter server info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+telemeterserver_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Telemeter server info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+telemeterserver_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+telemeterserver_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Telemeter server info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+telemeterserver_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ telemeterserver_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_telemeterserver_info #id').val('');
					}else{
...
//	$('#'+telemeterserver_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+telemeterserver_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');

	//サブグリッド設定
	$('#'+telemeterserver_info_tabId).jqGrid('setGridParam',
			{subGridRowExpanded: function(subgrid_id, row_id) {
				telemeteroffice_info(subgrid_id, row_id);
			}}
	);	//サブグリッドend
