...
import jp.ecom_plat.saigaitask.form.db.AlarmdefaultgroupInfoForm;
...
/**
 * アラームデフォルトグループ情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/alarmdefaultgroupInfo")
public class AlarmdefaultgroupInfoAction extends AbstractAdminAction<AlarmdefaultgroupInfo> {
	protected AlarmdefaultgroupInfoForm alarmdefaultgroupInfoForm;
	...
	public String index() {
		return "/admin/alarmdefaultgroupInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.alarmdefaultgroupInfo().groupInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = alarmdefaultgroupInfoService.getCount(conditions);

			// 一覧内容取得
			List<AlarmdefaultgroupInfo> list = new ArrayList<AlarmdefaultgroupInfo>();
			if(count > 0){
				list = alarmdefaultgroupInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute AlarmdefaultgroupInfoForm alarmdefaultgroupInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			AlarmdefaultgroupInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(AlarmdefaultgroupInfo.class, alarmdefaultgroupInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				alarmdefaultgroupInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					AlarmdefaultgroupInfo alarmdefaultgroupInfo = alarmdefaultgroupInfoService.findById(Long.decode(alarmdefaultgroupInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(alarmdefaultgroupInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, AlarmdefaultgroupInfo.class, alarmdefaultgroupInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(AlarmdefaultgroupInfo.class, alarmdefaultgroupInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							alarmdefaultgroupInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(alarmdefaultgroupInfoForm.id));
				int count = alarmdefaultgroupInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = alarmdefaultgroupInfoService.findById(Long.decode(alarmdefaultgroupInfoForm.id));
					//削除実行
					try{
					alarmdefaultgroupInfoService.delete(entity);
