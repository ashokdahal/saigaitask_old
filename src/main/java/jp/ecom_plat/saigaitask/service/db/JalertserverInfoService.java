...
import static jp.ecom_plat.saigaitask.entity.Names.jalertserverInfo;
...
public class JalertserverInfoService extends AbstractService<JalertserverInfo> {

    public JalertserverInfo findById(Long id) {
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(jalertserverInfo().localgovInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<JalertserverInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = jalertserverInfo().id().toString();
		}
		...
		if(!jalertserverInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(jalertserverInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(JalertserverInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
