...
import jp.ecom_plat.saigaitask.form.db.CkanmetadatadefaultInfoForm;
...
@RequestMapping("/admin/ckanmetadatadefaultInfo")
public class CkanmetadatadefaultInfoAction extends AbstractAdminAction<CkanmetadatadefaultInfo> {
	protected CkanmetadatadefaultInfoForm ckanmetadatadefaultInfoForm;
	...
	public String index() {
		return "/admin/ckanmetadatadefaultInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.ckanmetadatadefaultInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = ckanmetadatadefaultInfoService.getCount(conditions);

			// 一覧内容取得
			List<CkanmetadatadefaultInfo> list = new ArrayList<CkanmetadatadefaultInfo>();
			if(count > 0){
				list = ckanmetadatadefaultInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute CkanmetadatadefaultInfoForm ckanmetadatadefaultInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			CkanmetadatadefaultInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(CkanmetadatadefaultInfo.class, ckanmetadatadefaultInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				ckanmetadatadefaultInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					CkanmetadatadefaultInfo ckanmetadatadefaultInfo = ckanmetadatadefaultInfoService.findById(Long.decode(ckanmetadatadefaultInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(ckanmetadatadefaultInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, CkanmetadatadefaultInfo.class, ckanmetadatadefaultInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(CkanmetadatadefaultInfo.class, ckanmetadatadefaultInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							ckanmetadatadefaultInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(ckanmetadatadefaultInfoForm.id));
				int count = ckanmetadatadefaultInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = ckanmetadatadefaultInfoService.findById(Long.decode(ckanmetadatadefaultInfoForm.id));
					//削除実行
					try{
						ckanmetadatadefaultInfoService.delete(entity);
