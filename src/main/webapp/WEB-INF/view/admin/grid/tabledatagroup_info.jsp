...
function tabledatagroup_info(){
	tabledatagroup_info("","")
}

function tabledatagroup_info(subgrid_id, row_id){
	var tabName = 'tabledatagroup_info';
	var tabledatagroup_info_gridId = 'main';
	if(subgrid_id != null){
		tabledatagroup_info_gridId = subgrid_id;
	}
	var tabledatagroup_info_tabId = tabName;
	if(row_id != null){
		tabledatagroup_info_tabId += row_id;
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'ERROR', 'ERROR');

	var tabledatagroup_info_pagerId = tabledatagroup_info_tabId+'Nav';
	$('#'+tabledatagroup_info_gridId).html('<table id="'+tabledatagroup_info_tabId+'" class="scroll"></table><div id="'+tabledatagroup_info_pagerId+'" class="scroll"></div>');

	//グリッド
	$('#'+tabledatagroup_info_tabId).jqGrid({
		caption: '<%=lang.__("Table list group info")%>',
		mtype: 'POST',
		...
		pager: tabledatagroup_info_pagerId,
		beforeProcessing:function(data){
			...
			$('#'+tabledatagroup_info_tabId).jqGrid('setColProp', 'tablemasterinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//幅指定
			$('#'+tabledatagroup_info_tabId).jqGrid('setGridWidth', 1024, true);
			//1行おきに背景色をかえる
			var rowIDs = $('#'+tabledatagroup_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+tabledatagroup_info_tabId).jqGrid('navGrid',
			'#'+tabledatagroup_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+tabledatagroup_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+tabledatagroup_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				...
				var s = "";
					var rowid = $('#'+tabledatagroup_info_tabId).jqGrid('getGridParam', 'selrow');
					var rowdata = $('#'+tabledatagroup_info_tabId).jqGrid('getRowData', rowid);
					s += '&tablemasterinfoid='+rowdata.tablemasterinfoid;
					$('#'+tabledatagroup_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
				},
				afterShowForm:function(formid){
				},
				onclickSubmit:function(opt, data){
					//シリアライズ化しpostDataに一時保存した編集前データは何故か送信されないためこのイベントトリガーで送信する。
					return {'serializedPreEditData':$('#'+tabledatagroup_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+tabledatagroup_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+tabledatagroup_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});
				...
				afterComplete:function(response, postdata, formid){
					...
					$('#'+tabledatagroup_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+tabledatagroup_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+tabledatagroup_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
						...
						$('#'+tabledatagroup_info_tabId).jqGrid(
								...
								{recreateForm: true,
									...
									afterSubmit: function(){	//フォームサブミット後、グリッドデータをサーバから再取得する設定。
										...
										//$('#'+tabledatagroup_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
										$('#'+tabledatagroup_info_tabId).jqGrid('setGridParam',{datatype:'json'});
										return [true,''];
									},
									ajaxEditOptions :{
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+tabledatagroup_info_tabId).jqGrid('GridToForm', rowid, $('FrmGrid_'+ tabledatagroup_info_tabId).id);
					}else{
	...
	//サブグリッド設定
	$('#'+tabledatagroup_info_tabId).jqGrid('setGridParam',
