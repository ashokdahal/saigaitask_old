...
import jp.ecom_plat.saigaitask.form.db.PubliccommonsSendToInfoForm;
...
/**
 * 公共情報コモンズ発信先データのアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/publiccommonsSendToInfo")
public class PubliccommonsSendToInfoAction extends AbstractAdminAction<PubliccommonsSendToInfo> {
	protected PubliccommonsSendToInfoForm publiccommonsSendToInfoForm;
	...
	public String index() {
		return "/admin/publiccommonsSendToInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.publiccommonsSendToInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = publiccommonsSendToInfoService.getCount(conditions);

			// 一覧内容取得
			List<PubliccommonsSendToInfo> list = new ArrayList<PubliccommonsSendToInfo>();
			if(count > 0){
				list = publiccommonsSendToInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute PubliccommonsSendToInfoForm publiccommonsSendToInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			PubliccommonsSendToInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(PubliccommonsSendToInfo.class, publiccommonsSendToInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();

				//登録実行
				publiccommonsSendToInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					PubliccommonsSendToInfo publiccommonsSendToInfo = publiccommonsSendToInfoService.findById(Long.decode(publiccommonsSendToInfoForm.id));
					...
					if(publiccommonsSendToInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, PubliccommonsSendToInfo.class, publiccommonsSendToInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(PubliccommonsSendToInfo.class, publiccommonsSendToInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							...
							//更新実行
							publiccommonsSendToInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(publiccommonsSendToInfoForm.id));
				int count = publiccommonsSendToInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = publiccommonsSendToInfoService.findById(Long.decode(publiccommonsSendToInfoForm.id));
					//削除実行
					try{
						publiccommonsSendToInfoService.delete(entity);
