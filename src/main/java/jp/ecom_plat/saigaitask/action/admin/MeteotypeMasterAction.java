...
import jp.ecom_plat.saigaitask.form.db.MeteotypeMasterForm;
...
/**
 * 気象情報等取得種別マスタのアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/meteotypeMaster")
public class MeteotypeMasterAction extends AbstractAdminAction<MeteotypeMaster> {
	protected MeteotypeMasterForm meteotypeMasterForm;
	...
	public String index() {
		return "/admin/meteotypeMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = meteotypeMasterService.getCount(conditions);

			// 一覧内容取得
			List<MeteotypeMaster> list = new ArrayList<MeteotypeMaster>();
			if(count > 0){
				list = meteotypeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = meteotypeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteotypeMasterForm meteotypeMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteotypeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteotypeMaster.class, meteotypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteotypeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteotypeMaster meteotypeMaster = meteotypeMasterService.findById(Integer.parseInt(meteotypeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteotypeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteotypeMaster.class, meteotypeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteotypeMaster.class, meteotypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteotypeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteotypeMasterForm.id));
				int count = meteotypeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteotypeMasterService.findById(Integer.parseInt(meteotypeMasterForm.id));
					//削除実行
					try{
						meteotypeMasterService.delete(entity);
