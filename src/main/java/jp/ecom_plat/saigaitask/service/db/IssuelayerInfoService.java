...
import static jp.ecom_plat.saigaitask.entity.Names.issuelayerInfo;
...
public class IssuelayerInfoService extends AbstractService<IssuelayerInfo> {
	 ...
	 * @return
	 */
    public IssuelayerInfo findById(Long id) {
     ...
     * @return
     */
    public IssuelayerInfo findByLocalgovInfoId(Long gid) {
    	return select().where(
    			eq(issuelayerInfo().localgovinfoid(), gid)
    			,isNotNull(issuelayerInfo().tablemasterinfoid())
    		).limit(1).orderBy(asc(issuelayerInfo().id())).getSingleResult();
    }
	/**
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(issuelayerInfo().tablemasterInfo(), ne(issuelayerInfo().tablemasterInfo().deleted(), true))
			.innerJoin(issuelayerInfo().localgovInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<IssuelayerInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = issuelayerInfo().id().toString();
		}
		...
		if(!issuelayerInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(issuelayerInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(IssuelayerInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
