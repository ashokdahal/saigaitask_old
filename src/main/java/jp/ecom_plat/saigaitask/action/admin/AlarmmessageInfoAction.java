...
import jp.ecom_plat.saigaitask.form.db.AlarmmessageInfoForm;
...
/**
 * アラームメッセージ設定のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/alarmmessageInfo")
public class AlarmmessageInfoAction extends AbstractAdminAction<AlarmmessageInfo> {
	protected AlarmmessageInfoForm alarmmessageInfoForm;
	...
	public String index() {
		return "/admin/alarmmessageInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.alarmmessageInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = alarmmessageInfoService.getCount(conditions);

			// 一覧内容取得
			List<AlarmmessageInfo> list = new ArrayList<AlarmmessageInfo>();
			if(count > 0){
				list = alarmmessageInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = alarmmessageInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute AlarmmessageInfoForm alarmmessageInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			AlarmmessageInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(AlarmmessageInfo.class, alarmmessageInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				alarmmessageInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					AlarmmessageInfo alarmmessageInfo = alarmmessageInfoService.findById(Long.decode(alarmmessageInfoForm.id));
					...
					if(alarmmessageInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, AlarmmessageInfo.class, alarmmessageInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(AlarmmessageInfo.class, alarmmessageInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							alarmmessageInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(alarmmessageInfoForm.id));
				int count = alarmmessageInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = alarmmessageInfoService.findById(Long.decode(alarmmessageInfoForm.id));
					//削除実行
					try{
					alarmmessageInfoService.delete(entity);
