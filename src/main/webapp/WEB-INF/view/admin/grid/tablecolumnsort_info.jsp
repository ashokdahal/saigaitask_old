...
function tablecolumnsort_info(subgrid_id, row_id){
	var tabName = 'tablecolumnsort_info';
	var tablecolumnsort_info_gridId = 'main';
	if(subgrid_id != null){
		tablecolumnsort_info_gridId = subgrid_id;
	}
	var tablecolumnsort_info_tabId = tabName;
	if(row_id != null){
		tablecolumnsort_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var tablecolumnsort_info_pagerId = tablecolumnsort_info_tabId+'Nav';
	$('#'+tablecolumnsort_info_gridId).append('<table id="'+tablecolumnsort_info_tabId+'" class="scroll"></table><div id="'+tablecolumnsort_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+tablecolumnsort_info_tabId).jqGrid({
		caption: '<%=lang.__("Attribute sort info")%>',
		mtype: 'POST',
		...
		colModel:[
			...
			editoptions:{value:'',
				...
				readonly:false
					...
					buildSelect: function (data) {
						...
						var rowid = $('#'+tablecolumnsort_info_tabId).jqGrid('getGridParam', 'selrow');
						var selectedId = null;
						if(rowid != null){
							//編集の場合
							var rowdata = $('#'+tablecolumnsort_info_tabId).jqGrid('getRowData', rowid);
							selectedId = rowdata.tablemasterinfoid;
						}else{
						}
						return createSelectTag(rowid, selectTag, selectedId, 'tablecolumnsort_info', 'tablemaster_info');
					}
			},
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Table item name(*)")%>'},
			hidden:false},
			{name:'sorttext',
			width:'150',
			align:'left',
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Ascending order CSV(*)")%>'},
			hidden:false},
		],
		ajaxGridOptions :{
		...
		pager: tablecolumnsort_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+tablecolumnsort_info_tabId).jqGrid('setColProp', 'tablemasterinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+tablecolumnsort_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+tablecolumnsort_info_tabId).jqGrid('navGrid',
			'#'+tablecolumnsort_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+tablecolumnsort_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+tablecolumnsort_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Attribute sort info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+tablecolumnsort_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+tablecolumnsort_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+tablecolumnsort_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+tablecolumnsort_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+tablecolumnsort_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+tablecolumnsort_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+tablecolumnsort_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Attribute sort info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+tablecolumnsort_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Attribute sort info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+tablecolumnsort_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+tablecolumnsort_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Attribute sort info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+tablecolumnsort_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ tablecolumnsort_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_tablecolumnsort_info #id').val('');
					}else{
...
//	$('#'+tablecolumnsort_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+tablecolumnsort_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
