...
import static jp.ecom_plat.saigaitask.entity.Names.disasteritemInfo;
...
public class DisasteritemInfoService extends AbstractService<DisasteritemInfo> {

    public DisasteritemInfo findById(Long id) {
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(disasteritemInfo().localgovInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<DisasteritemInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = disasteritemInfo().id().toString();
		}
		...
		if(!disasteritemInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(disasteritemInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(DisasteritemInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
     ...
     * @return リスト
     */
    public List<DisasteritemInfo> findByLocalgovInfoId(Long govid) {
    	return select().where(
    			eq(disasteritemInfo().localgovinfoid(), govid)
    			).orderBy(asc(disasteritemInfo().line()), asc(disasteritemInfo().id())).getResultList();
    }
}