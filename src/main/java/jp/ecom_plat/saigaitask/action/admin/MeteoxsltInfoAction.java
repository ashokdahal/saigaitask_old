...
import jp.ecom_plat.saigaitask.form.db.MeteoxsltInfoForm;
...
@RequestMapping("/admin/meteoxsltInfo")
public class MeteoxsltInfoAction extends AbstractAdminAction<MeteoxsltInfo> {
	protected MeteoxsltInfoForm meteoxsltInfoForm;
	...
	public String index() {
		return "/admin/meteoxsltInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.meteoxsltInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = meteoxsltInfoService.getCount(conditions);

			// 一覧内容取得
			List<MeteoxsltInfo> list = new ArrayList<MeteoxsltInfo>();
			if(count > 0){
				list = meteoxsltInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteoxsltInfoForm meteoxsltInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteoxsltInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteoxsltInfo.class, meteoxsltInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteoxsltInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteoxsltInfo meteoxsltInfo = meteoxsltInfoService.findById(Long.decode(meteoxsltInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteoxsltInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteoxsltInfo.class, meteoxsltInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteoxsltInfo.class, meteoxsltInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteoxsltInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteoxsltInfoForm.id));
				int count = meteoxsltInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteoxsltInfoService.findById(Long.decode(meteoxsltInfoForm.id));
					//削除実行
					try{
					meteoxsltInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	...
	@ResponseBody
	public String upload(@ModelAttribute MeteoxsltInfoForm meteoxsltInfoForm) throws Exception {
		boolean success = fileUpload(meteoxsltInfoForm.uploadFile, meteoxsltInfoForm.localgovinfoid, meteoxsltInfoForm.filepath);

		//ファイルパスをDBに設定する
//		if (success) {
//			MeteoxsltInfo meteoxsltInfo = meteoxsltInfoService.findById(Long.decode(meteoxsltInfoForm.id));
//			meteoxsltInfo.filepath = meteoxsltInfoForm.uploadFile.getName();
//			meteoxsltInfoService.update(meteoxsltInfo);
//		}
