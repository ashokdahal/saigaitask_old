...
import jp.ecom_plat.saigaitask.form.db.CkanauthInfoForm;
...
@RequestMapping("/admin/ckanauthInfo")
public class CkanauthInfoAction extends AbstractAdminAction<CkanauthInfo> {
	protected CkanauthInfoForm ckanauthInfoForm;
	...
	public String index() {
		return "/admin/ckanauthInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.ckanauthInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = ckanauthInfoService.getCount(conditions);

			// 一覧内容取得
			List<CkanauthInfo> list = new ArrayList<CkanauthInfo>();
			if(count > 0){
				list = ckanauthInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute CkanauthInfoForm ckanauthInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(CkanauthInfo.class, ckanauthInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				ckanauthInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					CkanauthInfo ckanauthInfo = ckanauthInfoService.findById(Long.decode(ckanauthInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(ckanauthInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, CkanauthInfo.class, ckanauthInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(CkanauthInfo.class, ckanauthInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							ckanauthInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(ckanauthInfoForm.id));
				int count = ckanauthInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = ckanauthInfoService.findById(Long.decode(ckanauthInfoForm.id));
					//削除実行
					try{
						ckanauthInfoService.delete(entity);
