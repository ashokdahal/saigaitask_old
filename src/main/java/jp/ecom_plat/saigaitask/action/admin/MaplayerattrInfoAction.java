...
import jp.ecom_plat.saigaitask.form.db.MaplayerattrInfoForm;
...
@RequestMapping("/admin/maplayerattrInfo")
public class MaplayerattrInfoAction extends AbstractAdminAction<MaplayerattrInfo> {
	protected MaplayerattrInfoForm maplayerattrInfoForm;
	...
	public String index() {
		return "/admin/maplayerattrInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = maplayerattrInfoService.getCount(conditions);

			// 一覧内容取得
			List<MaplayerattrInfo> list = new ArrayList<MaplayerattrInfo>();
			if(count > 0){
				list = maplayerattrInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = maplayerattrInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MaplayerattrInfoForm maplayerattrInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MaplayerattrInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MaplayerattrInfo.class, maplayerattrInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				maplayerattrInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MaplayerattrInfo maplayerattrInfo = maplayerattrInfoService.findById(Long.decode(maplayerattrInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(maplayerattrInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MaplayerattrInfo.class, maplayerattrInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MaplayerattrInfo.class, maplayerattrInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							maplayerattrInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(maplayerattrInfoForm.id));
				int count = maplayerattrInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = maplayerattrInfoService.findById(Long.decode(maplayerattrInfoForm.id));
					//削除実行
					try{
						maplayerattrInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	...
	public String jqgridecommap(
		...
		try{
			...
			//地図レイヤ情報のテーブルIDに紐付くテーブルマスタ情報のレイヤIDを取得
//			String parentId = (String)request.getParameter("parentId");
			conditions.put(Names.maplayerInfo().id().toString(), parentId);
			List<MaplayerInfo> maplayerInfoList = maplayerInfoService.findByCondition(conditions, null, null, null, null);
			String layerId = maplayerInfoList.get(0).tablemasterInfo.layerid;
			String layerName = maplayerInfoList.get(0).tablemasterInfo.name;
			...
			String j = ecommapDataGetService.getAttributeList(loginDataDto.getEcomUser(), layerId);
			//レイヤ名追加
			...
			JSONArray jsonArray = json.getJSONArray("items");
	  	    ...
	  	    if (limit != NON_LIMIT)
	  	    	jsonArray = JSONArray.fromObject(jsonArray.subList((page - 1) * limit, Math.min(page * limit, count)));
