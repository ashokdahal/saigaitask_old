...
import static jp.ecom_plat.saigaitask.entity.Names.judgeformulaMaster;
...
public class JudgeformulaMasterService extends AbstractService<JudgeformulaMaster> {

    public JudgeformulaMaster findById(Integer id) {
	 ...
	 * @return 検索結果
	 */
	public List<JudgeformulaMaster> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = judgeformulaMaster().disporder().toString();
		}
		...
		if(!judgeformulaMaster().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(judgeformulaMaster().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		conditions.put(judgeformulaMaster().disporder().toString()+"_IS_NOT_NULL", Boolean.TRUE);
		List<JudgeformulaMaster> list = findByCondition(conditions, judgeformulaMaster().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(JudgeformulaMaster entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
