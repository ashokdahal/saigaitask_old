...
function ecomgwpost_info(subgrid_id, row_id){
	var tabName = 'ecomgwpost_info';
	var ecomgwpost_info_gridId = 'main';
	if(subgrid_id != null){
		ecomgwpost_info_gridId = subgrid_id;
	}
	var ecomgwpost_info_tabId = tabName;
	if(row_id != null){
		ecomgwpost_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var ecomgwpost_info_pagerId = ecomgwpost_info_tabId+'Nav';
	$('#'+ecomgwpost_info_gridId).append('<table id="'+ecomgwpost_info_tabId+'" class="scroll"></table><div id="'+ecomgwpost_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+ecomgwpost_info_tabId).jqGrid({
		caption: '<%=lang.__("e-Com GW posting info")%>',
		mtype: 'POST',
		...
		colNames: [
			...
			'<%=lang.__("Set name")%>',
			'<%=lang.__("Post destination group ID")%>',
			'<%=lang.__("Parts common ID")%>',
			'<%=lang.__("Parts individual ID")%>',
			'<%=lang.__("Post destination URL")%>',
			'<%=lang.__("Notes")%>',
		...
		colModel:[
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+ecomgwpost_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+ecomgwpost_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.localgovinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'ecomgwpost_info', 'localgov_info', false);
						}
				},
			...
			{name:'groupid',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Post destination group ID(*)")%>'},
				hidden:false},
			{name:'partsid',
				width:'150',
				align:'left',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Parts common ID(*)")%>'},
				hidden:false},
			{name:'blockid',
				width:'150',
				align:'left',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Parts individual ID(*)")%>'},
				hidden:false},
			{name:'posturl',
				width:'150',
				align:'left',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Post destination URL(*)")%>'},
				hidden:false},
			{name:'note',
				width:'150',
				align:'left',
		...
		beforeProcessing:function(data){
			...
			$('#'+ecomgwpost_info_tabId).jqGrid('setColProp', 'localgovinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+ecomgwpost_info_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+ecomgwpost_info_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+ecomgwpost_info_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+ecomgwpost_info_tabId).jqGrid('navGrid',
			'#'+ecomgwpost_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+ecomgwpost_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+ecomgwpost_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - e-Com GW posting info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+ecomgwpost_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+ecomgwpost_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
	  				var pagenum = $('#'+ecomgwpost_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+ecomgwpost_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+ecomgwpost_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+ecomgwpost_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+ecomgwpost_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - e-Com GW posting info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+ecomgwpost_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - E-Com GW posting info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+ecomgwpost_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+ecomgwpost_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - E-Com GW posting info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+ecomgwpost_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ ecomgwpost_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_ecomgwpost_info #id').val('');
					}else{
...
//	$('#'+ecomgwpost_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+ecomgwpost_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');

	//サブグリッド設定
	$('#'+ecomgwpost_info_tabId).jqGrid('setGridParam',
			{subGridRowExpanded: function(subgrid_id, row_id) {
				ecomgwpostdefault_info(subgrid_id, row_id);
			}}
	);	//サブグリッドend
