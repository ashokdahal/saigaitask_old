...
//public class MenuactionInfoAction extends AbstractAdminAction<MenuactionInfo> {
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/menuactionInfo")
public class MenuactionInfoAction{
//	protected MenuactionInfoForm menuactionInfoForm;
//
//	/** サービスクラス */
//	@Resource
//	protected MenuactionInfoExService menuactionInfoExService;
//
//	/** サービスクラス */
//	@Resource
...
//
	...
	public String index() {
		return "/admin/menuactionInfo/index";
	}
//
//	/**
...
//
//		// 一覧件数取得
//		int count = menuactionInfoExService.getCount(conditions);
//
//		// 一覧内容取得
//		List<MenuactionInfo> list = new ArrayList<MenuactionInfo>();
//		if(count > 0){
//			list = menuactionInfoExService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
//		}
//
//		//ユーザーデータ取得
...
//
//		//操作種別に応じたアクションを実行
//		MenuactionInfo entity;
//		//登録
//		if("add".equals(oper)) {
//			entity = Beans.createAndCopy(MenuactionInfo.class, menuactionInfoForm).dateConverter("yyyy-MM-dd").execute();
//			//登録実行
//			menuactionInfoExService.insert(entity);
//			//採番したキー値を返却
//			JSONObject json = new JSONObject();
...
//			}else{
//				//現時点の編集対象データを取得
//				MenuactionInfo menuactionInfo = menuactionInfoExService.findById(Integer.parseInt(menuactionInfoForm.id));
//
//				//結果返却用JSON
//				JSONObject json = new JSONObject();
...
//					if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MenuactionInfo.class, menuactionInfo)){
//						//変更されていない場合、更新
//						entity = Beans.createAndCopy(MenuactionInfo.class, menuactionInfoForm).dateConverter("yyyy-MM-dd").execute();
//						PropertyName<?>[] updateExcludesAry = null;
//						List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
//						//更新実行
//						menuactionInfoExService.update(entity, updateExcludesAry);
...
//			//検索条件マップ
//			BeanMap conditions = new BeanMap();
//			conditions.put("id", Integer.parseInt(menuactionInfoForm.id));
//			int count = menuactionInfoExService.getCount(conditions);
//			if(count==1){
//				//削除対象が存在する場合、削除実行。
//				entity = menuactionInfoExService.findById(Integer.parseInt(menuactionInfoForm.id));
//				//削除実行
//				menuactionInfoExService.delete(entity);
//			}else{
