...
import jp.ecom_plat.saigaitask.form.db.MapkmllayerInfoForm;
...
@RequestMapping("/admin/mapkmllayerInfo")
public class MapkmllayerInfoAction extends AbstractAdminAction<MapreferencelayerInfo> {
	protected MapkmllayerInfoForm mapkmllayerInfoForm;
	...
	public String index() {
		return "/admin/mapkmllayerInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = mapkmllayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<MapkmllayerInfo> list = new ArrayList<MapkmllayerInfo>();
			if(count > 0){
				list = mapkmllayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = mapkmllayerInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MapkmllayerInfoForm mapkmllayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MapkmllayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MapkmllayerInfo.class, mapkmllayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				mapkmllayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MapkmllayerInfo mapreferencelayerInfo = mapkmllayerInfoService.findById(Long.decode(mapkmllayerInfoForm.id));
					...
					if(mapreferencelayerInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MapreferencelayerInfo.class, mapreferencelayerInfo)){
							...
							//更新実行
							mapkmllayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(mapkmllayerInfoForm.id));
				int count = mapkmllayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = mapkmllayerInfoService.findById(Long.decode(mapkmllayerInfoForm.id));
					//削除実行
					try{
						mapkmllayerInfoService.delete(entity);
