...
import jp.ecom_plat.saigaitask.form.db.ToolboxDataForm;
...
@RequestMapping("/admin/toolboxData")
public class ToolboxDataAction  extends AbstractAdminAction<ToolboxData> {
	/** アクションフォーム */
	protected ToolboxDataForm toolboxDataForm;

	/** サービスクラス */
	@Resource
	protected ToolboxDataService toolboxDataService;
	...
	public String index(Map<String,Object>model, @ModelAttribute ToolboxDataForm toolboxDataForm) {
		this.toolboxDataForm = toolboxDataForm;
		return "/admin/toolboxData/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.toolboxData().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = toolboxDataService.getCount(conditions);

			// 一覧内容取得
			List<ToolboxData> list = new ArrayList<ToolboxData>();
			if(count > 0){
				list = toolboxDataService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = toolboxDataService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ToolboxDataForm toolboxDataForm) throws ServiceException {
		try{
			//操作種別取得
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ToolboxData.class, toolboxDataForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				toolboxDataService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ToolboxData ToolboxData = toolboxDataService.findById(Long.decode(toolboxDataForm.id));
					...
					if(ToolboxData != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, DisasteritemInfo.class, ToolboxData)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ToolboxData.class, toolboxDataForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							toolboxDataService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(toolboxDataForm.id));
				int count = toolboxDataService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = toolboxDataService.findById(Long.decode(toolboxDataForm.id));
					//削除実行
					try{
						toolboxDataService.delete(entity);
