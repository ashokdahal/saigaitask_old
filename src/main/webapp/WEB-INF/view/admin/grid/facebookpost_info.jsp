...
function facebookpost_info(subgrid_id, row_id){
	var tabName = 'facebookpost_info';
	var facebookpost_info_gridId = 'main';
	if(subgrid_id != null){
		facebookpost_info_gridId = subgrid_id;
	}
	var facebookpost_info_tabId = tabName;
	if(row_id != null){
		facebookpost_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var facebookpost_info_pagerId = facebookpost_info_tabId+'Nav';
	$('#'+facebookpost_info_gridId).append('<table id="'+facebookpost_info_tabId+'" class="scroll"></table><div id="'+facebookpost_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+facebookpost_info_tabId).jqGrid({
		caption: '<%=lang.__("Facebook posting info")%>',
		mtype: 'POST',
		...
		colModel:[
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+facebookpost_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+facebookpost_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.localgovinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'facebookpost_info', 'localgov_info', false);
						}
				},
			...
			{name:'name',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Set name(*)")%>'},
				hidden:false},
			{name:'pageid',
				width:'150',
				align:'left',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Page ID(*)")%>'},
				hidden:false},
			{name:'pagetype',
				width:'150',
				align:'left',
				...
				edittype:'select',
				editable:true,
				editoptions:{value:'<%=lang.__("1: Home timeline(specify Facebook account ID as page ID);2:Facebook page(specify Facebook page ID as page ID);3:Group(specify group page ID as page ID)")%>',
					defaultValue:'',
					readonly:false
				},
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Page type(*)")%>'},
				hidden:false},
			{name:'note',
				width:'150',
				align:'left',
		...
		pager: facebookpost_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+facebookpost_info_tabId).jqGrid('setColProp', 'localgovinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#facebookpost_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+facebookpost_info_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+facebookpost_info_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+facebookpost_info_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+facebookpost_info_tabId).jqGrid('navGrid',
			'#'+facebookpost_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+facebookpost_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+facebookpost_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Facebook posting info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+facebookpost_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+facebookpost_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
	  				var pagenum = $('#'+facebookpost_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+facebookpost_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+facebookpost_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+facebookpost_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+facebookpost_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Facebook posting info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+facebookpost_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Facebook posting info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+facebookpost_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+facebookpost_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Facebook posting info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+facebookpost_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ facebookpost_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_facebookpost_info #id').val('');
					}else{
...
//	$('#'+facebookpost_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+facebookpost_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');

	//サブグリッド設定
	$('#'+facebookpost_info_tabId).jqGrid('setGridParam',
			{subGridRowExpanded: function(subgrid_id, row_id) {
				facebookpostdefault_info(subgrid_id, row_id);
			}}
	);	//サブグリッドend
