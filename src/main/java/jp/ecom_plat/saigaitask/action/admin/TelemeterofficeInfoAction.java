...
import jp.ecom_plat.saigaitask.form.db.TelemeterofficeInfoForm;
...
/**
 * テレメータ管理事務所情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/telemeterofficeInfo")
public class TelemeterofficeInfoAction extends AbstractAdminAction<TelemeterofficeInfo> {
	protected TelemeterofficeInfoForm telemeterofficeInfoForm;
	...
	public String index() {
		return "/admin/telemeterofficeInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = telemeterofficeInfoService.getCount(conditions);

			// 一覧内容取得
			List<TelemeterofficeInfo> list = new ArrayList<TelemeterofficeInfo>();
			if(count > 0){
				list = telemeterofficeInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TelemeterofficeInfoForm telemeterofficeInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TelemeterofficeInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TelemeterofficeInfo.class, telemeterofficeInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				telemeterofficeInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TelemeterofficeInfo telemeterofficeInfo = telemeterofficeInfoService.findById(Long.decode(telemeterofficeInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(telemeterofficeInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TelemeterofficeInfo.class, telemeterofficeInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TelemeterofficeInfo.class, telemeterofficeInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							telemeterofficeInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(telemeterofficeInfoForm.id));
				int count = telemeterofficeInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = telemeterofficeInfoService.findById(Long.decode(telemeterofficeInfoForm.id));
					//削除実行
					try{
						telemeterofficeInfoService.delete(entity);
