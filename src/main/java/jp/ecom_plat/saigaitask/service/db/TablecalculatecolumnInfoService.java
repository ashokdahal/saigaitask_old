...
import static jp.ecom_plat.saigaitask.entity.Names.tablecalculatecolumnInfo;
...
public class TablecalculatecolumnInfoService extends AbstractService<TablecalculatecolumnInfo> {

    public TablecalculatecolumnInfo findById(Long id) {
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(tablecalculatecolumnInfo().tablemasterInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<TablecalculatecolumnInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = tablecalculatecolumnInfo().id().toString();
		}
		...
		if(!tablecalculatecolumnInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(tablecalculatecolumnInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(TablecalculatecolumnInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
	...
	@Override
	public DeleteCascadeResult deleteCascade(TablecalculatecolumnInfo entity, DeleteCascadeResult result) {

		result.cascade(TablecalculateInfo.class, Names.tablecalculateInfo().tablecalculatecolumninfoid(), entity.id);
