...
import jp.ecom_plat.saigaitask.form.db.NoticegroupuserInfoForm;
...
/**
 * 通知グループユーザ情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/noticegroupuserInfo")
public class NoticegroupuserInfoAction extends AbstractAdminAction<NoticegroupuserInfo> {
	protected NoticegroupuserInfoForm noticegroupuserInfoForm;
	...
	public String index() {
		return "/admin/noticegroupuserInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = noticegroupuserInfoService.getCount(conditions);

			// 一覧内容取得
			List<NoticegroupuserInfo> list = new ArrayList<NoticegroupuserInfo>();
			if(count > 0){
				list = noticegroupuserInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute NoticegroupuserInfoForm noticegroupuserInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			NoticegroupuserInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(NoticegroupuserInfo.class, noticegroupuserInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				noticegroupuserInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					NoticegroupuserInfo noticegroupuserInfo = noticegroupuserInfoService.findById(Long.decode(noticegroupuserInfoForm.id));
					...
					if(noticegroupuserInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, NoticegroupuserInfo.class, noticegroupuserInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(NoticegroupuserInfo.class, noticegroupuserInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							noticegroupuserInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(noticegroupuserInfoForm.id));
				int count = noticegroupuserInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = noticegroupuserInfoService.findById(Long.decode(noticegroupuserInfoForm.id));
					//削除実行
					try{
						noticegroupuserInfoService.delete(entity);
