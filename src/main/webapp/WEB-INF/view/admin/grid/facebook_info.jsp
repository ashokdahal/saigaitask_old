...
function facebook_info(subgrid_id, row_id){
	var tabName = 'facebook_info';
	var facebook_info_gridId = 'main';
	if(subgrid_id != null){
		facebook_info_gridId = subgrid_id;
	}
	var facebook_info_tabId = tabName;
	if(row_id != null){
		facebook_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var facebook_info_pagerId = facebook_info_tabId+'Nav';
	$('#'+facebook_info_gridId).append('<table id="'+facebook_info_tabId+'" class="scroll"></table><div id="'+facebook_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+facebook_info_tabId).jqGrid({
		caption: '<%=lang.__("Facebook settings")%>',
		mtype: 'POST',
			...
			editoptions:{value:'',
				...
				readonly:false
					...
					buildSelect: function (data) {
						...
						var rowid = $('#'+facebook_info_tabId).jqGrid('getGridParam', 'selrow');
						var selectedId = null;
						if(rowid != null){
							//編集の場合
							var rowdata = $('#'+facebook_info_tabId).jqGrid('getRowData', rowid);
							selectedId = rowdata.localgovinfoid;
						}else{
						}
						return createSelectTag(rowid, selectTag, selectedId, 'facebook_info', 'localgov_info');
					}
			},
		...
		beforeProcessing:function(data){
			...
			$('#'+facebook_info_tabId).jqGrid('setColProp', 'localgovinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#facebook_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+facebook_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+facebook_info_tabId).jqGrid('navGrid',
			'#'+facebook_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+facebook_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+facebook_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Facebook settings")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+facebook_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+facebook_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+facebook_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+facebook_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+facebook_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+facebook_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+facebook_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Facebook settings")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+facebook_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Facebook settings")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+facebook_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+facebook_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Facebook settings")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+facebook_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ facebook_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_facebook_info #id').val('');
					}else{
...
//	$('#'+facebook_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+facebook_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
