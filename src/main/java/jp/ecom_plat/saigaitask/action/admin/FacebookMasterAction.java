...
import jp.ecom_plat.saigaitask.form.db.FacebookMasterForm;
...
/**
 * Facebookマスタのアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/facebookMaster")
public class FacebookMasterAction extends AbstractAdminAction<FacebookMaster> {
	protected FacebookMasterForm facebookMasterForm;
	...
	public String index() {
		return "/admin/facebookMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = facebookMasterService.getCount(conditions);

			// 一覧内容取得
			List<FacebookMaster> list = new ArrayList<FacebookMaster>();
			if(count > 0){
				list = facebookMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute FacebookMasterForm facebookMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			FacebookMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(FacebookMaster.class, facebookMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				facebookMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					FacebookMaster facebookMaster = facebookMasterService.findById(Integer.parseInt(facebookMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(facebookMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, FacebookMaster.class, facebookMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(FacebookMaster.class, facebookMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							facebookMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(facebookMasterForm.id));
				int count = facebookMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = facebookMasterService.findById(Integer.parseInt(facebookMasterForm.id));
					//削除実行
					try{
						facebookMasterService.delete(entity);
