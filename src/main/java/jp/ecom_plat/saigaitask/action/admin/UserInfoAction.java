...
import jp.ecom_plat.saigaitask.form.db.UserInfoForm;
 ...
 * ユーザ情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController("/admin/userInfo")
@RequestMapping("/admin/userInfo")
public class UserInfoAction extends AbstractAdminAction<UserInfo> {
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = userInfoService.getCount(conditions);

			// 一覧内容取得
			List<UserInfo> list = new ArrayList<UserInfo>();
			if(count > 0){
				list = userInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = userInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute UserInfoForm userInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(UserInfo.class, userInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					UserInfo userInfo = userInfoService.findById(Long.decode(userInfoForm.id));
					...
					if(userInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, UserInfo.class, userInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(UserInfo.class, userInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							userInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(userInfoForm.id));
				int count = userInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = userInfoService.findById(Long.decode(userInfoForm.id));
					//削除実行
					try{
