...
import jp.ecom_plat.saigaitask.form.db.SafetystateInfoForm;
...
@RequestMapping("/admin/safetystateInfo")
public class SafetystateInfoAction extends AbstractAdminAction<SafetystateInfo> {
	protected SafetystateInfoForm safetystateInfoForm;
	...
	public String index() {
		return "/admin/safetystateInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.safetystateInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = safetystateInfoService.getCount(conditions);

			// 一覧内容取得
			List<SafetystateInfo> list = new ArrayList<SafetystateInfo>();
			if(count > 0){
				list = safetystateInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = safetystateInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute SafetystateInfoForm safetystateInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			SafetystateInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(SafetystateInfo.class, safetystateInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				safetystateInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					SafetystateInfo safetystateInfo = safetystateInfoService.findById(Long.decode(safetystateInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(safetystateInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, SafetystateInfo.class, safetystateInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(SafetystateInfo.class, safetystateInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							safetystateInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(safetystateInfoForm.id));
				int count = safetystateInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = safetystateInfoService.findById(Long.decode(safetystateInfoForm.id));
					//削除実行
					try{
						safetystateInfoService.delete(entity);
