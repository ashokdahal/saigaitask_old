...
import jp.ecom_plat.saigaitask.form.db.ExternalmapdataInfoForm;
...
@RequestMapping("/admin/externalmapdataInfo")
public class ExternalmapdataInfoAction extends AbstractAdminAction<ExternalmapdataInfo> {
	protected ExternalmapdataInfoForm externalmapdataInfoForm;
	...
	public String index() {
		return "/admin/externalmapdataInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ExternalmapdataInfoForm externalmapdataInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ExternalmapdataInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ExternalmapdataInfo.class, externalmapdataInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				externalmapdataInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ExternalmapdataInfo externalmapdataInfo = externalmapdataInfoService.findById(Long.decode(externalmapdataInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(externalmapdataInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ExternalmapdataInfo.class, externalmapdataInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ExternalmapdataInfo.class, externalmapdataInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							externalmapdataInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(externalmapdataInfoForm.id));
				int count = externalmapdataInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = externalmapdataInfoService.findById(Long.decode(externalmapdataInfoForm.id));
					//削除実行
					try{
						externalmapdataInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	...
	@ResponseBody
	public String insert(@ModelAttribute ExternalmapdataInfoForm externalmapdataInfoForm) {
		ExternalmapdataInfo externalmapdataInfo = Beans.createAndCopy(ExternalmapdataInfo.class, externalmapdataInfoForm).execute();
		Long menuinfoid = externalmapdataInfo.menuinfoid;
		if(menuinfoid!=null) {
			//表示順取得
			int maxDisporder = 0;
			BeanMap maxDisporderConditions = new BeanMap();
			//親IDが存在する場合、親IDに紐付くデータの内、最大の表示順を取得する。
			maxDisporderConditions.put(ExternalmapdataInfoNames.menuinfoid().toString(), menuinfoid);
			maxDisporder = externalmapdataInfoService.getLargestDisporder(maxDisporderConditions);
			externalmapdataInfo.disporder = maxDisporder;
