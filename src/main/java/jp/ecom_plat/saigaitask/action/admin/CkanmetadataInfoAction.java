...
import jp.ecom_plat.saigaitask.form.db.CkanmetadataInfoForm;
...
@RequestMapping("/admin/ckanmetadataInfo")
public class CkanmetadataInfoAction extends AbstractAdminAction<CkanmetadataInfo> {
	protected CkanmetadataInfoForm ckanmetadataInfoForm;
	...
	public String index() {
		return "/admin/ckanmetadataInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.ckanmetadataInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = ckanmetadataInfoService.getCount(conditions);

			// 一覧内容取得
			List<CkanmetadataInfo> list = new ArrayList<CkanmetadataInfo>();
			if(count > 0){
				list = ckanmetadataInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute CkanmetadataInfoForm ckanmetadataInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			CkanmetadataInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(CkanmetadataInfo.class, ckanmetadataInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					CkanmetadataInfo ckanmetadataInfo = ckanmetadataInfoService.findById(Long.decode(ckanmetadataInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(ckanmetadataInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, CkanmetadataInfo.class, ckanmetadataInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(CkanmetadataInfo.class, ckanmetadataInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							ckanmetadataInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				conditions.put("id", Integer.parseInt(ckanmetadataInfoForm.id));
				int count = ckanmetadataInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = ckanmetadataInfoService.findById(Long.decode(ckanmetadataInfoForm.id));
					String mdFileID = entity.name;
					//削除実行
					try{
						ckanmetadataInfoService.delete(entity);					

						//クリアリングハウスの削除を行う
						boolean isTraining = entity.layerid != null && !entity.layerid.equals(tablemasterInfoService.findById(entity.tablemasterinfoid).layerid);
						boolean ret = CKANUtil.deleteMetadata(mdFileID, ckanService.getApiKey(isTraining), isTraining);
						if (!ret) {
							JSONObject json = new JSONObject();
