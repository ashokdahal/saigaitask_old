...
import jp.ecom_plat.saigaitask.form.db.DemoInfoForm;
...
@RequestMapping("/admin/demoInfo")
public class DemoInfoAction extends AbstractAdminAction<DemoInfo> {
	protected DemoInfoForm demoInfoForm;
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.demoInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = demoInfoService.getCount(conditions);

			// 一覧内容取得
			List<DemoInfo> list = new ArrayList<DemoInfo>();
			if(count > 0){
				list = demoInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = demoInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute DemoInfoForm demoInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(DemoInfo.class, demoInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				demoInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					DemoInfo demoInfo = demoInfoService.findById(Long.decode(demoInfoForm.id));
					...
					if(demoInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, DemoInfo.class, demoInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(DemoInfo.class, demoInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							demoInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(demoInfoForm.id));
				int count = demoInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = demoInfoService.findById(Long.decode(demoInfoForm.id));
					//削除実行
					try{
						demoInfoService.delete(entity);
