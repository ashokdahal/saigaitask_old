...
import jp.ecom_plat.saigaitask.form.db.MenutaskInfoForm;
...
@RequestMapping("/admin/menutaskInfo")
public class MenutaskInfoAction extends AbstractAdminAction<MenutaskInfo> {
	protected MenutaskInfoForm menutaskInfoForm;
	...
	public String index() {
		return "/admin/menutaskInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = menutaskInfoService.getCount(conditions);

			// 一覧内容取得
			List<MenutaskInfo> list = new ArrayList<MenutaskInfo>();
			if(count > 0){
				list = menutaskInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = menutaskInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MenutaskInfoForm menutaskInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MenutaskInfo.class, menutaskInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				menutaskInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MenutaskInfo menutaskInfo = menutaskInfoService.findById(Long.decode(menutaskInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(menutaskInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MenutaskInfo.class, menutaskInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MenutaskInfo.class, menutaskInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							menutaskInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(menutaskInfoForm.id));
				int count = menutaskInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = menutaskInfoService.findById(Long.decode(menutaskInfoForm.id));
					//削除実行
					try{
						menutaskInfoService.deleteLogically(entity);
