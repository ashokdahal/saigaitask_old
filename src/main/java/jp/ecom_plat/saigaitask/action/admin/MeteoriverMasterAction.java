...
import jp.ecom_plat.saigaitask.form.db.MeteoriverMasterForm;
...
@RequestMapping("/admin/meteoriverMaster")
public class MeteoriverMasterAction extends AbstractAdminAction<MeteoriverMaster> {
	protected MeteoriverMasterForm meteoriverMasterForm;

	/** サービスクラス */
	@Resource
	protected MeteoriverMasterService meteoriverMasterService;
	...
	public String index() {
		return "/admin/meteoriverMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = meteoriverMasterService.getCount(conditions);

			// 一覧内容取得
			List<MeteoriverMaster> list = new ArrayList<MeteoriverMaster>();
			if(count > 0){
				list = meteoriverMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = meteoriverMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteoriverMasterForm meteoriverMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteoriverMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteoriverMaster.class, meteoriverMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteoriverMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteoriverMaster meteoriverMaster = meteoriverMasterService.findById(Integer.parseInt(meteoriverMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteoriverMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteoriverMaster.class, meteoriverMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteoriverMaster.class, meteoriverMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteoriverMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteoriverMasterForm.id));
				int count = meteoriverMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteoriverMasterService.findById(Integer.parseInt(meteoriverMasterForm.id));
					//削除実行
					try{
						meteoriverMasterService.delete(entity);
