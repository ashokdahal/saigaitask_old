...
import static jp.ecom_plat.saigaitask.entity.Names.ckanauthInfo;
...
public class CkanauthInfoService extends AbstractService<CkanauthInfo> {

    public CkanauthInfo findById(Long id) {
        return select().leftOuterJoin(ckanauthInfo().localgovInfo()).id(id).getSingleResult();
    }
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.leftOuterJoin(ckanauthInfo().localgovInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<CkanauthInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = ckanauthInfo().id().toString();
		}
		...
		if(!ckanauthInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(ckanauthInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return 更新数
	*/
	public int update(CkanauthInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
	...
	@Override
	public DeleteCascadeResult deleteCascade(CkanauthInfo entity, DeleteCascadeResult result) {
