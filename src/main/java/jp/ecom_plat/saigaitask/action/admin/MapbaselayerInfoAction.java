...
import jp.ecom_plat.saigaitask.form.db.MapbaselayerInfoForm;
...
@RequestMapping("/admin/mapbaselayerInfo")
public class MapbaselayerInfoAction extends AbstractAdminAction<MapbaselayerInfo> {
	protected MapbaselayerInfoForm mapbaselayerInfoForm;
	...
	public String index() {
		return "/admin/mapbaselayerInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = mapbaselayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<MapbaselayerInfo> list = new ArrayList<MapbaselayerInfo>();
			if(count > 0){
				list = mapbaselayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = mapbaselayerInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MapbaselayerInfoForm mapbaselayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MapbaselayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MapbaselayerInfo.class, mapbaselayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				mapbaselayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MapbaselayerInfo mapbaselayerInfo = mapbaselayerInfoService.findById(Long.decode(mapbaselayerInfoForm.id));
					...
					if(mapbaselayerInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MapbaselayerInfo.class, mapbaselayerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MapbaselayerInfo.class, mapbaselayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							mapbaselayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(mapbaselayerInfoForm.id));
				int count = mapbaselayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = mapbaselayerInfoService.findById(Long.decode(mapbaselayerInfoForm.id));
					//削除実行
					try{
						mapbaselayerInfoService.delete(entity);
...
//			String parentId = (String)request.getParameter("parentId");
	        ...
	        if(mapmasterInfo != null){
	        	// 地図レイヤ情報取得
	            Map<String, String> mapLayerInfos;
	            mapLayerInfos = ecommapDataGetService.getMapUsedBaseLayersAll(mapmasterInfo.communityid, mapmasterInfo.mapgroupid, mapmasterInfo.mapid);
		  	    ...
		  	    List<String> mapLayerInfoIdList = new ArrayList<String>(mapLayerInfos.keySet());
		  	    if (limit != NON_LIMIT)
		  	    	mapLayerInfoIdList = mapLayerInfoIdList.subList((page - 1) * limit, Math.min(page * limit, count));
	            for(String mapLayerInfoId : mapLayerInfoIdList){
	            	...
	            	result.setLayerInfo(mapLayerInfoId + ":" + mapLayerInfos.get(mapLayerInfoId));
	            	results.add(result);
	            }
	        }
