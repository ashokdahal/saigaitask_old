...
import jp.ecom_plat.saigaitask.entity.db.MeteovolcanoMaster;
import jp.ecom_plat.saigaitask.form.db.MeteovolcanoMasterForm;
import jp.ecom_plat.saigaitask.service.TableService;
import jp.ecom_plat.saigaitask.service.db.MeteovolcanoMasterService;
...
@RequestMapping("/admin/meteovolcanoMaster")
public class MeteovolcanoMasterAction extends AbstractAdminAction<MeteovolcanoMaster> {
	protected MeteovolcanoMasterForm meteovolcanoMasterForm;

	/** サービスクラス */
	@Resource
	protected MeteovolcanoMasterService meteovolcanoMasterService;
	...
	public String index() {
		return "/admin/meteovolcanoMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = meteovolcanoMasterService.getCount(conditions);

			// 一覧内容取得
			List<MeteovolcanoMaster> list = new ArrayList<MeteovolcanoMaster>();
			if(count > 0){
				list = meteovolcanoMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = meteovolcanoMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteovolcanoMasterForm meteovolcanoMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteovolcanoMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteovolcanoMaster.class, meteovolcanoMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteovolcanoMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteovolcanoMaster meteovolcanoMaster = meteovolcanoMasterService.findById(Integer.parseInt(meteovolcanoMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteovolcanoMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteovolcanoMaster.class, meteovolcanoMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteovolcanoMaster.class, meteovolcanoMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteovolcanoMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteovolcanoMasterForm.id));
				int count = meteovolcanoMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteovolcanoMasterService.findById(Integer.parseInt(meteovolcanoMasterForm.id));
					//削除実行
					try{
						meteovolcanoMasterService.delete(entity);
