...
function menutasktype_info(subgrid_id, row_id){

	var templateFlags = {
		'<%= Constants.MENUTASKTYPE_TEMPLATE_NONE %>':'<%=lang.__("Not template")%>',
		'<%= Constants.MENUTASKTYPE_TEMPLATE_PARENT %>':'<%=lang.__("Template(Parent)")%>',
		'<%= Constants.MENUTASKTYPE_TEMPLATE_CHILD %>':'<%=lang.__("Template(Child)")%>'
	};
	var templateFlagSelectItems = '';
	for (var item in templateFlags) {
		templateFlagSelectItems += item + ':' + item + '：' + templateFlags[item] +';';
	}
	templateFlagSelectItems = templateFlagSelectItems.slice( 0, -1 );	

	var tabName = 'menutasktype_info';
	var menutasktype_info_gridId = 'main';
	if(subgrid_id != null){
		menutasktype_info_gridId = subgrid_id;
	}
	var menutasktype_info_tabId = tabName;
	if(row_id != null){
		menutasktype_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var menutasktype_info_pagerId = menutasktype_info_tabId+'Nav';
	$('#'+menutasktype_info_gridId).append('<table id="'+menutasktype_info_tabId+'" class="scroll"></table><div id="'+menutasktype_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+menutasktype_info_tabId).jqGrid({
		caption: '<%=lang.__("Task type info")%>',
		mtype: 'POST',
		...
		colModel:[
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+menutasktype_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+menutasktype_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.localgovinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'menutasktype_info', 'localgov_info', false);
						}
				},
			...
			{name:'template',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Template flag")%>'},
				hidden:false},
			{name:'disporder',
				width:'100',
				align:'right',
		...
		pager: menutasktype_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+menutasktype_info_tabId).jqGrid('setColProp', 'localgovinfoid', {editoptions:{value:str}});
		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#menutasktype_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+menutasktype_info_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+menutasktype_info_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+menutasktype_info_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+menutasktype_info_tabId).jqGrid('navGrid',
			'#'+menutasktype_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+menutasktype_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+menutasktype_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Task type info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+menutasktype_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+menutasktype_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+menutasktype_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+menutasktype_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+menutasktype_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+menutasktype_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+menutasktype_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Task type info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+menutasktype_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Task type info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+menutasktype_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+menutasktype_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Task type info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
									...
									beforeSubmit:function(postdata, formid){
										//一括コピー用
										showProcessing('FrmGrid_'+ menutasktype_info_tabId);
										postdata.isAllCopy = $('#isAllCopy').prop('checked');
										postdata.allCopyId = $('#'+menutasktype_info_tabId).jqGrid('getRowData', rowid).id;
										return [true,"",""];
									},
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+menutasktype_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ menutasktype_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_menutasktype_info #id').val('');
					}else{
...
//	$('#'+menutasktype_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+menutasktype_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');

	//サブグリッド設定
	$('#'+menutasktype_info_tabId).jqGrid('setGridParam',
			{subGridRowExpanded: function(subgrid_id, row_id) {
				menu_info(subgrid_id, row_id);
			}}
	);	//サブグリッドend
