...
function observatoryrainlayer_info(subgrid_id, row_id){
	var tabName = 'observatoryrainlayer_info';
	var observatoryrainlayer_info_gridId = 'main';
	if(subgrid_id != null){
		observatoryrainlayer_info_gridId = subgrid_id;
	}
	var observatoryrainlayer_info_tabId = tabName;
	if(row_id != null){
		observatoryrainlayer_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var observatoryrainlayer_info_pagerId = observatoryrainlayer_info_tabId+'Nav';
	$('#'+observatoryrainlayer_info_gridId).append('<table id="'+observatoryrainlayer_info_tabId+'" class="scroll"></table><div id="'+observatoryrainlayer_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+observatoryrainlayer_info_tabId).jqGrid({
		caption: '<%=lang.__("Rainfall observation point layer info")%>',
		mtype: 'POST',
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+observatoryrainlayer_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+observatoryrainlayer_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.localgovinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'observatoryrainlayer_info', 'localgov_info', false);
						}
				},
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							return createSelectTag(rowid, selectTag, selectedId, 'observatoryrainlayer_info', 'tablemaster_info', false);
						}
				},
				editrules:{required:true, number:false, integer:false, email:false, edithidden:false},
				sortable:true,
		...
		pager: observatoryrainlayer_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+observatoryrainlayer_info_tabId).jqGrid('setColProp', 'localgovinfoid', {editoptions:{value:str}});
			...
			$('#'+observatoryrainlayer_info_tabId).jqGrid('setColProp', 'tablemasterinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#observatoryrainlayer_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+observatoryrainlayer_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+observatoryrainlayer_info_tabId).jqGrid('navGrid',
			'#'+observatoryrainlayer_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+observatoryrainlayer_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+observatoryrainlayer_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Rainfall observation point layer info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+observatoryrainlayer_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+observatoryrainlayer_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
	  				var pagenum = $('#'+observatoryrainlayer_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+observatoryrainlayer_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+observatoryrainlayer_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+observatoryrainlayer_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+observatoryrainlayer_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Rainfall observation point layer info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+observatoryrainlayer_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
				beforeInitData:function(formid){
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Rainfall observation point layer info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+observatoryrainlayer_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
		...
		//コピーボタン
			...
			'#'+observatoryrainlayer_info_pagerId,
			{
				id:'copy_observatoryrainlayer_info',
				caption:'',
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+observatoryrainlayer_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Rainfall observation point layer info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+observatoryrainlayer_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ observatoryrainlayer_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_observatoryrainlayer_info #id').val('');
						//「属性ID」のプルダウン作成
						initAttrSelect();
					}else{
...
//	$('#'+observatoryrainlayer_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+observatoryrainlayer_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
	...
	function initAttrSelect() {
		...
		$('#tablemasterinfoid').on('change', function() {
			makeAttrSelect(observatoryrainlayer_info_tabId, tabName, attrcols, objid);
		});
		makeAttrSelect(observatoryrainlayer_info_tabId, tabName, attrcols, objid);
	}
