...
<c:set var="dam" value="${observatorydamInfo}"/>
<div class="container" style="width:900px; /*height:600px;*/ margin:0 auto;">
	<div class="header" style="width:900px; height:45px; background-color:#000000; color:#BEF1FF; display:block; line-height:45px;">
		<span style="font-weight:bold; margin:4px;"><%=lang.__("Telemeter")%>&nbsp;<%=lang.__("Dam data")%>&nbsp;&nbsp;${f:h(dam.name)}（${f:h(dam.readname)}）</span>
	</div><!-- end .header -->
...
<c:choose>
	<c:when test="${timeaxis==Observ.MIN}">
					<a href="${f:url('/observ/dam')}/${id}/${Observ.HOUR}?time=${time}"><%=lang.__("On the hour every hour")%></a>&nbsp;/&nbsp;<span style="font-weight:bold">${Observ.MIN}<%=lang.__(" minutes each")%></span>
	</c:when>
	<c:otherwise>
					<span style="font-weight:bold;"><%=lang.__("On the hour every hour")%></span>&nbsp;/&nbsp;<a href="${f:url('/observ/dam')}/${id}/${Observ.MIN}?time=${time}">${Observ.MIN}<%=lang.__(" minutes each")%></a>
	</c:otherwise>
</c:choose>
				</td>
			</tr>
		...
		<table width="900" border="0">
			<tr>
				<td colspan="2">
					<!-- 観測地点情報 -->
					<table width="100%" border="1" style="font-size:80%; text-align:center;">
						<caption></caption>
						<tr>
						...
						<tr>
							<td rowspan="2">${f:h(dam.name) }</td>
							<!-- <td rowspan="2">{国河川|自治体|etc.}</td> -->
							<td rowspan="2">${f:h(dam.officename) }</td>
							<td rowspan="2">${f:h(dam.address) }</td>
					...
					<table border="1" style="font-size:80%" class="list">
						<thead>
							...
							<tr>
								...
								<th colspan="2" width="100" class="head"><%=lang.__("Reservoir water level (m)<!--2-->")%></th>
								<th width="100" class="head"><%=lang.__("Water storage rate   (%)")%></th>
								<th colspan="2" width="100" class="head"><%=lang.__("Total inflow amount (m3/s)")%></th>
								<th colspan="2" width="100" class="head"><%=lang.__("Total discharge amount (m3/s)")%></th>
							</tr>
						</thead>
						<tbody>
...
<c:forEach var="e" varStatus="s" items="${tableItems}">
	...
	<c:set var="md" value="${dateParts[1]}/${dateParts[2]}" />
							<tr>
								...
								<td style="border-style:none none none solid; border-top:${pre_date!=e.datelabel?'solid 1px black':'none'};" width="70px">${(empty e.contentscode3)?'N/A':(e.contentscode3!=0)?e.contentsname3:e.itemdata3}</td>
								<td style="border-style:none solid none none; border-top:${pre_date!=e.datelabel?'solid 1px black':'none'}; text-align:center;">${(e.itemdataarrow3) }</td>
								<td style="border-style:none none none solid; border-top:${pre_date!=e.datelabel?'solid 1px black':'none'};" width="70px">${(empty e.contentscode4)?'N/A':(e.contentscode4!=0)?e.contentsname4:e.itemdata1}</td>
								<td style="border-style:none solid none none; border-top:${pre_date!=e.datelabel?'solid 1px black':'none'}; text-align:center;">${(e.itemdataarrow4) }</td>
							</tr>
	<c:set var="pre_date" value="${f:h(e.datelabel)}"/>
</c:forEach>
						</tbody>
					</table>
				</td>
				<td width="500" style="text-align:center;">
					<div id="damChart_div" style="width:450px; height:250px;"></div>&nbsp;
					<div id="damChart2_div" style="width:450px; height:250px;"></div>&nbsp;
				</td>
			</tr>
		</table>
	</div><!-- end .content -->
		...
		<table width="900" border="0">
			<tr>
				<td style="text-align:right;">
					<div>
						<ul class="buttons">
							<li><a class="btn blue" href="#" onClick="${dam.iframe?'self.parent.tb_remove();':'window.close();'} return false;"><%=lang.__("Close")%></a></li>
...
<c:if test="${fn:length(graphItems) > 0 }">
	...
	function drawChart() {
		...
		data.addColumn({type:'number',   role:'data',   label:'<%=lang.__("Reservoir water level (mm)")%>'});
		data.addColumn({type:'number',   role:'data',   label:'<%=lang.__("Water storage rate   (%) ")%>'});

		//レコードの追加([datetime, num, num])<%-- 累加雨量が0の場合は、グラフを描画しないようにするため値をnullにする. --%>
		data.addRows([
<c:forEach var="e" varStatus="s" items="${graphItems}">
					[{v:new Date('${e.datevalue } ${e.timevalue }'), f:'${e.datelabel } ${e.timelabel }'}, {v:${(e.contentscode1==0)?e.itemdata1:'null' }, f:'${(e.contentscode1==0)?e.itemdata1:e.contentsname1 }'}, {v:${(e.contentscode2==0)?e.itemdata2:'null' }, f:'${(e.contentscode2==0)?e.itemdata2:e.contentsname2 }'}]${s.last?'':',' }
</c:forEach>
		]);

//		//フォーマッターの作成
//		var formatter_dec1 = new google.visualization.NumberFormat({fractionDigits: 1});			//小数点一位を表示
...
//		var currentDatetime = new Date('${graphItems[fn:length(graphItems)-1].datevalue } ${graphItems[fn:length(graphItems)-1].timevalue }');
		...
		var options = {
				...
				series: {
					...
					1: { //貯水率
						...
						PointSize: 0,			//グラフの点,   Available: 'line',
						lineWidth: 2,			//グラフの線幅, Available: 'line',
						//areaOpacity: 0.5,		//透過性（0.0-1.0）, default: 0.3, Available: 'steppedArea'
						visibleInLegend: true
					}
				},
				//縦軸
				...
				vAxes: {
					0: { //左軸用(line)
						title: '<%=lang.__("Reservoir water level (m)")%>',
						//データ範囲
					...
					1: { //右軸用(line)
						title: '<%=lang.__("Water storage rate (%)")%>',
						//データ範囲
						viewWindowMode: 'pretty',		//{'pretty'|'maximized'|'explicit'}
						//目盛
						format: '0',
		...
		// グラフ描画
		var damChart = new google.visualization.ComboChart(document.getElementById('damChart_div'));
		damChart.draw(data, options);
	 }
</c:if>
<c:if test="${fn:length(graphItems) > 0 }">
	...
	function drawChart2() {
		...
		//カラムの宣言
		data.addColumn({type:'datetime', role:'domain', label:'<%=lang.__("Times of day")%>'});
		data.addColumn({type:'number',   role:'data',   label:'<%=lang.__("Total inflow amount (m3/s) ")%>'});
		data.addColumn({type:'number',   role:'data',   label:'<%=lang.__("Total discharge amount (m3/s) ")%>'});

		//レコードの追加([datetime, num, num])<%-- 累加雨量が0の場合は、グラフを描画しないようにするため値をnullにする. --%>
		data.addRows([
<c:forEach var="e" varStatus="s" items="${graphItems}">
					[{v:new Date('${e.datevalue } ${e.timevalue }'), f:'${e.datelabel } ${e.timelabel }'}, {v:${(e.contentscode3==0)?e.itemdata3:'null' }, f:'${(e.contentscode3==0)?e.itemdata3:e.contentsname3 }'}, {v:${(e.contentscode4==0)?e.itemdata4:'null' }, f:'${(e.contentscode4==0)?e.itemdata4:e.contentsname4 }'}]${s.last?'':',' }
</c:forEach>
		]);
...
//		var currentDatetime = new Date('${graphItems[fn:length(graphItems)-1].datevalue } ${graphItems[fn:length(graphItems)-1].timevalue }');
		...
		var options = {
				...
				hAxis: {
					...
					titleTextStyle: { /* color: 'black', */ /*fontName: <global-font-name>, */ /* fontSize: <global-font-size> */ },
					//横軸
					baseline: minDatetime,			//軸の描画位置  無効: steppedArea
					Color: 'none',					//軸の色              無効: steppedArea
					viewWindowMode: 'explicit',		//pretty or explicit
					viewWindow: { min: minDatetime, max: maxDatetime },		//表示データ範囲
					direction: 1,					//軸の向き  default: 1
					//目盛
					...
					slantedText: false,				//目盛値を斜体にして全体を表示
					maxTextLines: 2,				//目盛値の表示行の数
					maxAlternation: 1,				//目盛値の段違い表記の数
					//mintextSpacing: /* <textStyle-font-size>, */
					//showTextEvery: 3,				//目盛値の表示間隔
					//format: 'M/dd k:mm',			//目盛フォーマット  eg, 23:59 + 1 hour =>> 24:59
					format: 'M/dd H:mm',			//目盛フォーマット  eg, 23:59 + 1 hour =>> 00:59
					//目盛線
					gridlines:      { color: '#b4b4b4', count: 5 },		//有効: 'bars', 無効: 'steppedArea', 1=24時間毎
					minorGridlines: { color: '#b4b4b4', count: 1 },		//有効: 'bars', 無効: 'steppedArea'
				}
		};

		// グラフ描画
		var damChart2 = new google.visualization.ComboChart(document.getElementById('damChart2_div'));
		damChart2.draw(data, options);
	 }
</c:if>
</script>
</html>
