...
import jp.ecom_plat.saigaitask.form.db.AuthorizationInfoForm;
...
@RequestMapping("/admin/authorizationInfo")
public class AuthorizationInfoAction extends AbstractAdminAction<AuthorizationInfo> {
	protected AuthorizationInfoForm authorizationInfoForm;

	/** サービスクラス */
	@Resource
	protected AuthorizationInfoService authorizationInfoService;
	@Resource
	...
	public String index() {
		...
		if (loginDataDto.isUseCkan()) {
			return "forward:/admin/ckanauthInfo";
		}

		return "/admin/authorizationInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.authorizationInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = authorizationInfoService.getCount(conditions);

			// 一覧内容取得
			List<AuthorizationInfo> list = new ArrayList<AuthorizationInfo>();
			if(count > 0){
				list = authorizationInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute AuthorizationInfoForm authorizationInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			AuthorizationInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(AuthorizationInfo.class, authorizationInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				authorizationInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					AuthorizationInfo authorizationInfo = authorizationInfoService.findById(Long.decode(authorizationInfoForm.id));
					...
					if(authorizationInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, AuthorizationInfo.class, authorizationInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(AuthorizationInfo.class, authorizationInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							authorizationInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(authorizationInfoForm.id));
				int count = authorizationInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = authorizationInfoService.findById(Long.decode(authorizationInfoForm.id));
					//削除実行
					try{
						authorizationInfoService.delete(entity);
