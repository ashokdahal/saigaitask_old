...
function decisionsupporttype_master(subgrid_id, row_id){
	var tabName = 'decisionsupporttype_master';
	var decisionsupporttype_master_gridId = 'main';
	if(subgrid_id != null){
		decisionsupporttype_master_gridId = subgrid_id;
	}
	var decisionsupporttype_master_tabId = tabName;
	if(row_id != null){
		decisionsupporttype_master_tabId += row_id;
	}else{
		row_id = '';
	}

	var decisionsupporttype_master_pagerId = decisionsupporttype_master_tabId+'Nav';
	$('#'+decisionsupporttype_master_gridId).append('<table id="'+decisionsupporttype_master_tabId+'" class="scroll"></table><div id="'+decisionsupporttype_master_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+decisionsupporttype_master_tabId).jqGrid({
		caption: '<%=lang.__("Decision support layer type master")%>',
		mtype: 'POST',
		...
		pager: decisionsupporttype_master_pagerId,
		autowidth: true,
		autoencode: true,
		...
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#decisionsupporttype_masterNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+decisionsupporttype_master_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+decisionsupporttype_master_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+decisionsupporttype_master_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+decisionsupporttype_master_tabId).jqGrid('navGrid',
			'#'+decisionsupporttype_master_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+decisionsupporttype_master_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+decisionsupporttype_master_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - decision support layer type master")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					var rowid = $('#'+decisionsupporttype_master_tabId).jqGrid('getGridParam', 'selrow');
					var s = "";
					$('#'+decisionsupporttype_master_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+decisionsupporttype_master_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				...
				afterComplete:function(response, postdata, formid){
					...
					$('#'+decisionsupporttype_master_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Decision support layer type master")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+decisionsupporttype_master_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - decision support layer type master")%>',
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
		...
		//コピーボタン
			...
			'#'+decisionsupporttype_master_pagerId,
			{
				id:'copy_decisionsupporttype_master',
				caption:'',
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+decisionsupporttype_master_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy -Decision support layer type master")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
									...
									afterSubmit: function(){	//フォームサブミット後、グリッドデータをサーバから再取得する設定。
										...
										//$('#'+decisionsupporttype_master_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
										$('#'+decisionsupporttype_master_tabId).jqGrid('setGridParam',{datatype:'json'});
										return [true,''];
									},
									ajaxEditOptions :{
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+decisionsupporttype_master_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ decisionsupporttype_master_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_decisionsupporttype_master #id').val('');
					}else{
...
//	$('#'+decisionsupporttype_master_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+decisionsupporttype_master_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
