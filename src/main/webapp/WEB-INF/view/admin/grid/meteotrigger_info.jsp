...
function meteotrigger_info(subgrid_id, row_id){
	var tabName = 'meteotrigger_info';
	var meteotrigger_info_gridId = 'main';
	if(subgrid_id != null){
		meteotrigger_info_gridId = subgrid_id;
	}
	var meteotrigger_info_tabId = tabName;
	if(row_id != null){
		meteotrigger_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'meteorequest_info', 'meteorequestinfoid');

	var meteotrigger_info_pagerId = meteotrigger_info_tabId+'Nav';
	$('#'+meteotrigger_info_gridId).append('<table id="'+meteotrigger_info_tabId+'" class="scroll"></table><div id="'+meteotrigger_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+meteotrigger_info_tabId).jqGrid({
		caption: '<%=lang.__("Weather trigger info")%>',
		mtype: 'POST',
		url:convertTabNameToMethodMame(tabName)+'/jqgridindex/'+parentgrid_refkey+'/'+row_id+'',
		datatype: 'json',
		colNames: [
			'ID',
			'<%=lang.__("Weather info acquisition ID")%>',
			'<%=lang.__("Trigger")%>',
		...
		colModel:[
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Weather info acquisition ID(*)")%>'},
			hidden:false},
			{name:'trigger',
			width:'150',
			align:'left',
			...
			editoptions:{value:'',
				...
				readonly:false
					...
					buildSelect: function (data) {
						...
						var rowid = $('#'+meteotrigger_info_tabId).jqGrid('getGridParam', 'selrow');
						var selectedId = null;
						if(rowid != null){
							//編集の場合
							var rowdata = $('#'+meteotrigger_info_tabId).jqGrid('getRowData', rowid);
							selectedId = rowdata.noticegroupinfoid;
						}else{
						}
						return createSelectTag(rowid, selectTag, selectedId, 'meteotrigger_info', 'noticegroup_info');
					}
			},
			editrules:{required:false, number:false, integer:false, email:false, edithidden:false},
			sortable:true,
			...
			editoptions:{value:'',
				...
				readonly:false
					...
					buildSelect: function (data) {
						...
						return createSelectTag(rowid, selectTag, selectedId, 'meteotrigger_info', 'stationclass_info');
					}
			},
			editrules:{required:false, number:false, integer:false, email:false, edithidden:false},
			sortable:true,
		...
		pager: meteotrigger_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+meteotrigger_info_tabId).jqGrid('setColProp', 'meteorequestinfoid', {editoptions:{value:str}});
			...
			$('#'+meteotrigger_info_tabId).jqGrid('setColProp', 'noticegroupinfoid', {editoptions:{value:str}});
			...
			$('#'+meteotrigger_info_tabId).jqGrid('setColProp', 'stationclassinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#meteotrigger_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+meteotrigger_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+meteotrigger_info_tabId).jqGrid('navGrid',
			'#'+meteotrigger_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+meteotrigger_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+meteotrigger_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Weather trigger info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+meteotrigger_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
					$('#tr_id .CaptionTD', formid).after('<td id="dispid">'+rowid+'</td>');
					$('#tr_id .DataTD', formid).hide();
					$('#tr_meteorequestinfoid .CaptionTD', formid).after('<td id="dispmeteorequestinfoid">'+row_id+'</td>');
					$('#tr_meteorequestinfoid .DataTD', formid).hide();
					//フォームモードに編集モードを設定
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+meteotrigger_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+meteotrigger_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+meteotrigger_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+meteotrigger_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+meteotrigger_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+meteotrigger_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
				...
				afterclickPgButtons:function(whichbutton, formid, rowid){
					...
					$('#tr_id .CaptionTD', formid).after('<td id="dispid">'+rowid+'</td>');
					$('#tr_id .DataTD', formid).hide();
					$('#tr_meteorequestinfoid #dispmeteorequestinfoid', formid).remove();
					$('#tr_meteorequestinfoid .CaptionTD', formid).after('<td id="dispmeteorequestinfoid">'+row_id+'</td>');
					$('#tr_meteorequestinfoid .DataTD', formid).hide();
					return [true,''];
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Weather trigger info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+meteotrigger_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_meteorequestinfoid .CaptionTD', formid).after('<td id="dispmeteorequestinfoid">'+row_id+'</td>');
					$('#tr_meteorequestinfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Weather trigger info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+meteotrigger_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+meteotrigger_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Weather trigger info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+meteotrigger_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ meteotrigger_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_meteotrigger_info #id').val('');
					}else{
...
//	$('#'+meteotrigger_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+meteotrigger_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
