...
import jp.ecom_plat.saigaitask.form.db.TelemeterpointInfoForm;
...
@RequestMapping("/admin/telemeterpointInfo")
public class TelemeterpointInfoAction extends AbstractAdminAction<TelemeterpointInfo> {
	protected TelemeterpointInfoForm telemeterpointInfoForm;
	...
	public String index() {
		return "/admin/telemeterpointInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = telemeterpointInfoService.getCount(conditions);

			// 一覧内容取得
			List<TelemeterpointInfo> list = new ArrayList<TelemeterpointInfo>();
			if(count > 0){
				list = telemeterpointInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TelemeterpointInfoForm telemeterpointInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TelemeterpointInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TelemeterpointInfo.class, telemeterpointInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				telemeterpointInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TelemeterpointInfo telemeterpointInfo = telemeterpointInfoService.findById(Long.decode(telemeterpointInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(telemeterpointInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TelemeterpointInfo.class, telemeterpointInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TelemeterpointInfo.class, telemeterpointInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							telemeterpointInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(telemeterpointInfoForm.id));
				int count = telemeterpointInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = telemeterpointInfoService.findById(Long.decode(telemeterpointInfoForm.id));
					//削除実行
					try{
						telemeterpointInfoService.delete(entity);
