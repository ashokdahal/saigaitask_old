...
import jp.ecom_plat.saigaitask.form.db.ClearinghousemetadatadefaultInfoForm;
...
/**
 * メタデータデフォルト設定情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/clearinghousemetadatadefaultInfo")
public class ClearinghousemetadatadefaultInfoAction extends AbstractAdminAction<ClearinghousemetadatadefaultInfo> {
	protected ClearinghousemetadatadefaultInfoForm clearinghousemetadatadefaultInfoForm;
	...
	public String index() {
		...
		if (loginDataDto.isUseCkan()) {
			return "forward:/admin/ckanmetadatadefaultInfo";
		}

		return "/admin/clearinghousemetadatadefaultInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.clearinghousemetadatadefaultInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = clearinghousemetadatadefaultInfoService.getCount(conditions);

			// 一覧内容取得
			List<ClearinghousemetadatadefaultInfo> list = new ArrayList<ClearinghousemetadatadefaultInfo>();
			if(count > 0){
				list = clearinghousemetadatadefaultInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ClearinghousemetadatadefaultInfoForm clearinghousemetadatadefaultInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ClearinghousemetadatadefaultInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ClearinghousemetadatadefaultInfo.class, clearinghousemetadatadefaultInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				clearinghousemetadatadefaultInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ClearinghousemetadatadefaultInfo clearinghousemetadatadefaultInfo = clearinghousemetadatadefaultInfoService.findById(Long.decode(clearinghousemetadatadefaultInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(clearinghousemetadatadefaultInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ClearinghousemetadatadefaultInfo.class, clearinghousemetadatadefaultInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ClearinghousemetadatadefaultInfo.class, clearinghousemetadatadefaultInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							clearinghousemetadatadefaultInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(clearinghousemetadatadefaultInfoForm.id));
				int count = clearinghousemetadatadefaultInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = clearinghousemetadatadefaultInfoService.findById(Long.decode(clearinghousemetadatadefaultInfoForm.id));
					//削除実行
					try{
						clearinghousemetadatadefaultInfoService.delete(entity);
