...
function jalertrequest_info(subgrid_id, row_id){
	var tabName = 'jalertrequest_info';
	var jalertrequest_info_gridId = 'main';
	if(subgrid_id != null){
		jalertrequest_info_gridId = subgrid_id;
	}
	var jalertrequest_info_tabId = tabName;
	if(row_id != null){
		jalertrequest_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var jalertrequest_info_pagerId = jalertrequest_info_tabId+'Nav';
	$('#'+jalertrequest_info_gridId).append('<table id="'+jalertrequest_info_tabId+'" class="scroll"></table><div id="'+jalertrequest_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+jalertrequest_info_tabId).jqGrid({
		caption: '<%=lang.__("JAlert info")%>',
		mtype: 'POST',
		...
		colNames: [
			...
			'<%=lang.__("JAlert info type")%>',
			'<%=lang.__("District ID (different by the weather conditions)")%>',
		...
		colModel:[
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+jalertrequest_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+jalertrequest_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.localgovinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'jalertrequest_info', 'localgov_info', false);
						}
				},
			...
			{name:'jalerttypeid',
				...
				editoptions:{value:'',
					...
					delimiter:'|d|',
					readonly:false
						,dataUrl:convertTabNameToMethodMame(tabName)+'/createSelectTag/jalerttypeid/jalerttype_master/id/name',
						//フォームのプルダウン作成
						buildSelect: function (data) {
							var selectTag = $.parseJSON(data).selectTag;
							var rowid = $('#'+jalertrequest_info_tabId).jqGrid('getGridParam', 'selrow');
							...
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+jalertrequest_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.jalerttypeid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'jalertrequest_info', 'jalerttype_master', false);
						}
				},
				editrules:{required:true, number:false, integer:false, email:false, edithidden:false},
				sortable:true,
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("JAlert info type(*)")%>'},
				hidden:false},
			{name:'meteoareaid',
				width:'150',
				align:'left',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("District ID (different by the weather conditions)(*)")%>'},
				hidden:false},
			{name:'meteoareaid2',
				width:'150',
				align:'left',
		...
		pager: jalertrequest_info_pagerId,
		autowidth: true,
		autoencode: true,
		beforeProcessing:function(data){
			...
			$('#'+jalertrequest_info_tabId).jqGrid('setColProp', 'localgovinfoid', {editoptions:{value:str}});
			...
			for(i=0; i < data.rows.length; i++){
				...
				var ref = data.rows[i].jalerttypeMaster;
				if(ref != null){
					var refkey = ref.id;
					var refval = ref.name;
					str += refkey + ':' + refkey + '：' + refval;
			...
			$('#'+jalertrequest_info_tabId).jqGrid('setColProp', 'jalerttypeid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+jalertrequest_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+jalertrequest_info_tabId).jqGrid('navGrid',
			'#'+jalertrequest_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+jalertrequest_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+jalertrequest_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - JAlert info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					var rowdata = $('#'+jalertrequest_info_tabId).jqGrid('getRowData', rowid);
					s += '&jalerttypeid='+rowdata.jalerttypeid;
					$('#'+jalertrequest_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+jalertrequest_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				...
				afterComplete:function(response, postdata, formid){
					...
					$('#'+jalertrequest_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - JAlert info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+jalertrequest_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - JAlert info")%>',
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+jalertrequest_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - JAlert info")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
									closeOnEscape:true,
									...
									afterSubmit: function(){	//フォームサブミット後、グリッドデータをサーバから再取得する設定。
										...
										//$('#'+jalertrequest_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
										$('#'+jalertrequest_info_tabId).jqGrid('setGridParam',{datatype:'json'});
										return [true,''];
									},
									ajaxEditOptions :{
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+jalertrequest_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ jalertrequest_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_jalertrequest_info #id').val('');
					}else{
...
//	$('#'+jalertrequest_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+jalertrequest_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');

	//サブグリッド設定
	$('#'+jalertrequest_info_tabId).jqGrid('setGridParam',
			{subGridRowExpanded: function(subgrid_id, row_id) {
				jalerttrigger_info(subgrid_id, row_id);
			}}
	);	//サブグリッドend
