...
import jp.ecom_plat.saigaitask.form.db.TwitterInfoForm;
...
@RequestMapping("/admin/twitterInfo")
public class TwitterInfoAction extends AbstractAdminAction<TwitterInfo> {
	protected TwitterInfoForm twitterInfoForm;
	...
	public String index() {
		return "/admin/twitterInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.twitterInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = twitterInfoService.getCount(conditions);

			// 一覧内容取得
			List<TwitterInfo> list = new ArrayList<TwitterInfo>();
			if(count > 0){
				list = twitterInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TwitterInfoForm twitterInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TwitterInfo.class, twitterInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				twitterInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TwitterInfo twitterInfo = twitterInfoService.findById(Long.decode(twitterInfoForm.id));
					...
					if(twitterInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TwitterInfo.class, twitterInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TwitterInfo.class, twitterInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							twitterInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(twitterInfoForm.id));
				int count = twitterInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = twitterInfoService.findById(Long.decode(twitterInfoForm.id));
					//削除実行
					try{
						twitterInfoService.delete(entity);
