...
import jp.ecom_plat.saigaitask.form.db.NoticedefaultgroupInfoForm;
...
/**
 * 通知デフォルトグループ情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/noticedefaultgroupInfo")
public class NoticedefaultgroupInfoAction extends AbstractAdminAction<NoticedefaultgroupInfo> {
	protected NoticedefaultgroupInfoForm noticedefaultgroupInfoForm;
	...
	public String index() {
		return "/admin/noticedefaultgroupInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = noticedefaultgroupInfoService.getCount(conditions);

			// 一覧内容取得
			List<NoticedefaultgroupInfo> list = new ArrayList<NoticedefaultgroupInfo>();
			if(count > 0){
				list = noticedefaultgroupInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute NoticedefaultgroupInfoForm noticedefaultgroupInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			NoticedefaultgroupInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(NoticedefaultgroupInfo.class, noticedefaultgroupInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				noticedefaultgroupInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					NoticedefaultgroupInfo noticedefaultgroupInfo = noticedefaultgroupInfoService.findById(Long.decode(noticedefaultgroupInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(noticedefaultgroupInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, NoticedefaultgroupInfo.class, noticedefaultgroupInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(NoticedefaultgroupInfo.class, noticedefaultgroupInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							noticedefaultgroupInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(noticedefaultgroupInfoForm.id));
				int count = noticedefaultgroupInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = noticedefaultgroupInfoService.findById(Long.decode(noticedefaultgroupInfoForm.id));
					//削除実行
					try{
					noticedefaultgroupInfoService.delete(entity);
