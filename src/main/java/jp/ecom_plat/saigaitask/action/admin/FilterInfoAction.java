...
import jp.ecom_plat.saigaitask.form.db.FilterInfoForm;
...
@RequestMapping("/admin/filterInfo")
public class FilterInfoAction extends AbstractAdminAction<FilterInfo> {
	protected FilterInfoForm filterInfoForm;
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = filterInfoService.getCount(conditions);

			// 一覧内容取得
			List<FilterInfo> list = new ArrayList<FilterInfo>();
			if(count > 0){
				list = filterInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute FilterInfoForm filterInfoForm) throws ServiceException {
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(FilterInfo.class, filterInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				filterInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					FilterInfo filterInfo = filterInfoService.findById(Long.decode(filterInfoForm.id));
					...
					if(filterInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, FilterInfo.class, filterInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(FilterInfo.class, filterInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							filterInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(filterInfoForm.id));
				int count = filterInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = filterInfoService.findById(Long.decode(filterInfoForm.id));
					//削除実行
					try{
						filterInfoService.delete(entity);
