...
import jp.ecom_plat.saigaitask.entity.db.JalerttypeMaster;
import jp.ecom_plat.saigaitask.form.db.JalerttypeMasterForm;
import jp.ecom_plat.saigaitask.service.db.JalerttypeMasterService;
...
/**
 * JAlert種別マスタのアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/jalerttypeMaster")
public class JalerttypeMasterAction extends AbstractAdminAction<JalerttypeMaster> {
	protected JalerttypeMasterForm jalerttypeMasterForm;

	/** サービスクラス */
	@Resource
	protected JalerttypeMasterService jalerttypeMasterService;
	...
	public String index() {
		return "/admin/jalerttypeMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = jalerttypeMasterService.getCount(conditions);

			// 一覧内容取得
			List<JalerttypeMaster> list = new ArrayList<JalerttypeMaster>();
			if(count > 0){
				list = jalerttypeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = jalerttypeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JalerttypeMasterForm jalerttypeMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			JalerttypeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JalerttypeMaster.class, jalerttypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				jalerttypeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JalerttypeMaster jalerttypeMaster = jalerttypeMasterService.findById(Integer.decode(jalerttypeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(jalerttypeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JalerttypeMaster.class, jalerttypeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JalerttypeMaster.class, jalerttypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							jalerttypeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(jalerttypeMasterForm.id));
				int count = jalerttypeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = jalerttypeMasterService.findById(Integer.decode(jalerttypeMasterForm.id));
					//削除実行
					jalerttypeMasterService.delete(entity);
				}else{
