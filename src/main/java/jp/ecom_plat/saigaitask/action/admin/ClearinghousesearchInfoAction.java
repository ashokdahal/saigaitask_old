...
import jp.ecom_plat.saigaitask.form.db.ClearinghousesearchInfoForm;
...
/**
 * クリアリングハウス検索情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/clearinghousesearchInfo")
public class ClearinghousesearchInfoAction extends AbstractAdminAction<ClearinghousesearchInfo> {
	protected ClearinghousesearchInfoForm clearinghousesearchInfoForm;

	/** サービスクラス */
	@Resource
	protected ClearinghousesearchInfoService clearinghousesearchInfoService;
	...
	public String index() {
		return "/admin/clearinghousesearchInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.clearinghousesearchInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = clearinghousesearchInfoService.getCount(conditions);

			// 一覧内容取得
			List<ClearinghousesearchInfo> list = new ArrayList<ClearinghousesearchInfo>();
			if(count > 0){
				list = clearinghousesearchInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ClearinghousesearchInfoForm clearinghousesearchInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ClearinghousesearchInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ClearinghousesearchInfo.class, clearinghousesearchInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				clearinghousesearchInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ClearinghousesearchInfo clearinghousesearchInfo = clearinghousesearchInfoService.findById(Long.decode(clearinghousesearchInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(clearinghousesearchInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ClearinghousesearchInfo.class, clearinghousesearchInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ClearinghousesearchInfo.class, clearinghousesearchInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							clearinghousesearchInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(clearinghousesearchInfoForm.id));
				int count = clearinghousesearchInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = clearinghousesearchInfoService.findById(Long.decode(clearinghousesearchInfoForm.id));
					//削除実行
					try{
						clearinghousesearchInfoService.delete(entity);
