...
import static jp.ecom_plat.saigaitask.entity.Names.meteoriverareaMaster;
...
public class MeteoriverareaMasterService extends AbstractService<MeteoriverareaMaster> {

    public MeteoriverareaMaster findById(Integer id) {
	 ...
	 * @return 検索結果
	 */
	public List<MeteoriverareaMaster> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = meteoriverareaMaster().id().toString();
		}
		...
		if(!meteoriverareaMaster().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(meteoriverareaMaster().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		conditions.put(meteoriverareaMaster().disporder().toString()+"_IS_NOT_NULL", Boolean.TRUE);
		List<MeteoriverareaMaster> list = findByCondition(conditions, meteoriverareaMaster().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(MeteoriverareaMaster entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
