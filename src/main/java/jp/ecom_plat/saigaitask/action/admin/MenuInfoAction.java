...
public class MenuInfoAction extends AbstractAdminAction<MenuInfo> {
	protected MenuInfoForm menuInfoForm;
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = menuInfoService.getCount(conditions);

			// 一覧内容取得
			List<MenuInfo> list = new ArrayList<MenuInfo>();
			if(count > 0){
				list = menuInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MenuInfoForm menuInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MenuInfo.class, menuInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MenuInfo menuInfo = menuInfoService.findById(Long.decode(menuInfoForm.getId()));
					...
					if(menuInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MenuInfo.class, menuInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MenuInfo.class, menuInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							menuInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(menuInfoForm.getId()));
				int count = menuInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = menuInfoService.findById(Long.decode(menuInfoForm.getId()));
					//削除実行
					try{
						menuInfoService.deleteLogically(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
			...
			if(StringUtils.isEmpty(selectTagColumn)
				...
				List<?> parentList;
				if("_attr".equals(tableName)) {
					//属性IDプルダウン
					parentList = mapService.getAttrInfoFromTablamasterId(request.getParameter("tablemasterId"));
				}
				else {
	...
	@ResponseBody
	public String upload(@ModelAttribute MenuInfoForm menuInfoForm) throws Exception {
		// 自治体IDを取得
		MenutasktypeInfo menutasktypeInfo = menutasktypeInfoService.findById(Long.parseLong(menuInfoForm.getMenutasktypeinfoid()));
		LocalgovInfo localgovInfo = localgovInfoService.findById(menutasktypeInfo.localgovinfoid);
		...
		String tempFileName = Constants.MENUINFOID_EXCELLIST_FILENAMEPREFIX + "menuinfoid_" + uuidStr + "_template.xlsx";
		// 正式ファイル名生成
		String fileName = Constants.MENUINFOID_EXCELLIST_FILENAMEPREFIX + "menuinfoid_" + menuInfoForm.getId() + "_template.xlsx";

		// ファイルアップロード
		File uploadedTmpFile = excelListTemplatefileUpload(menuInfoForm.getExcellisttemplatefile(), Long.toString(localgovInfo.id), tempFileName);

		// テンプレート内容チェック
		JSONObject json = excellistService.check(uploadedTmpFile);
		if(json.containsKey(JQG_MESSAGE)){
			// チェック結果に問題があった場合は一時ファイルを削除して終了
			uploadedTmpFile.delete();
			responseService.responseJsonText(json);
		}else{
			// チェック結果に問題がなければ一時ファイルを正式ファイル名に変更して保存
			File uploadedFile = new File(uploadedTmpFile.getParentFile().getAbsolutePath() + "/" + fileName);
			FileUtil.fileCopyByPath(uploadedTmpFile.getAbsolutePath(), uploadedFile.getAbsolutePath());
			uploadedTmpFile.delete();
			responseService.responseJsonText(json);
		}
