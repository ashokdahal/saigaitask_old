...
import jp.ecom_plat.saigaitask.form.db.MeteowarningcodeMasterForm;
...
@RequestMapping("/admin/meteowarningcodeMaster")
public class MeteowarningcodeMasterAction extends AbstractAdminAction<MeteowarningcodeMaster> {
	protected MeteowarningcodeMasterForm meteowarningcodeMasterForm;

	/** サービスクラス */
	@Resource
	protected MeteowarningcodeMasterService meteowarningcodeMasterService;
	...
	public String index() {
		return "/admin/meteowarningcodeMaster/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = meteowarningcodeMasterService.getCount(conditions);

			// 一覧内容取得
			List<MeteowarningcodeMaster> list = new ArrayList<MeteowarningcodeMaster>();
			if(count > 0){
				list = meteowarningcodeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = meteowarningcodeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteowarningcodeMasterForm meteowarningcodeMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteowarningcodeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteowarningcodeMaster.class, meteowarningcodeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteowarningcodeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteowarningcodeMaster meteowarningcodeMaster = meteowarningcodeMasterService.findById(Long.decode(meteowarningcodeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteowarningcodeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteowarningcodeMaster.class, meteowarningcodeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteowarningcodeMaster.class, meteowarningcodeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteowarningcodeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteowarningcodeMasterForm.id));
				int count = meteowarningcodeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteowarningcodeMasterService.findById(Long.decode(meteowarningcodeMasterForm.id));
					//削除実行
					meteowarningcodeMasterService.delete(entity);
				}else{
