...
function noticedefaultgroup_info(subgrid_id, row_id){
	var tabName = 'noticedefaultgroup_info';
	var noticedefaultgroup_info_gridId = 'main';
	if(subgrid_id != null){
		noticedefaultgroup_info_gridId = subgrid_id;
	}
	var noticedefaultgroup_info_tabId = tabName;
	if(row_id != null){
		noticedefaultgroup_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'noticedefault_info', 'noticedefaultinfoid');

	var noticedefaultgroup_info_pagerId = noticedefaultgroup_info_tabId+'Nav';
	$('#'+noticedefaultgroup_info_gridId).append('<table id="'+noticedefaultgroup_info_tabId+'" class="scroll"></table><div id="'+noticedefaultgroup_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+noticedefaultgroup_info_tabId).jqGrid({
		caption: '<%=lang.__("Notification default group info")%>',
		mtype: 'POST',
			...
			editoptions:{value:'',
				...
				readonly:false
					...
					buildSelect: function (data) {
						...
						var rowid = $('#'+noticedefaultgroup_info_tabId).jqGrid('getGridParam', 'selrow');
						var selectedId = null;
						if(rowid != null){
							//編集の場合
							var rowdata = $('#'+noticedefaultgroup_info_tabId).jqGrid('getRowData', rowid);
							selectedId = rowdata.noticegroupinfoid;
						}else{
						}
						return createSelectTag(rowid, selectTag, selectedId, 'noticedefaultgroup_info', 'noticegroup_info');
					}
			},
			editrules:{required:true, number:false, integer:false, email:false, edithidden:false},
			sortable:true,
		...
		pager: noticedefaultgroup_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+noticedefaultgroup_info_tabId).jqGrid('setColProp', 'noticedefaultinfoid', {editoptions:{value:str}});
			...
			$('#'+noticedefaultgroup_info_tabId).jqGrid('setColProp', 'noticegroupinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+noticedefaultgroup_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+noticedefaultgroup_info_tabId).jqGrid('navGrid',
			'#'+noticedefaultgroup_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+noticedefaultgroup_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+noticedefaultgroup_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Notification default group info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+noticedefaultgroup_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+noticedefaultgroup_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+noticedefaultgroup_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+noticedefaultgroup_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+noticedefaultgroup_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+noticedefaultgroup_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+noticedefaultgroup_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Notification default group info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+noticedefaultgroup_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_noticedefaultinfoid .CaptionTD', formid).after('<td id="dispnoticedefaultinfoid">'+row_id+'</td>');
					$('#tr_noticedefaultinfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Notification default group info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+noticedefaultgroup_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
		...
		//コピーボタン
			...
			'#'+noticedefaultgroup_info_pagerId,
			{
				id:'copy_noticedefaultgroup_info',
				caption:'',
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+noticedefaultgroup_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Notification default group info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+noticedefaultgroup_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ noticedefaultgroup_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_noticedefaultgroup_info #id').val('');
					}else{
...
//	$('#'+noticedefaultgroup_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+noticedefaultgroup_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
