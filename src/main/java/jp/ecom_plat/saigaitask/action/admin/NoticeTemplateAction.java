...
import jp.ecom_plat.saigaitask.form.db.NoticeTemplateForm;
...
@RequestMapping("/admin/noticeTemplate")
public class NoticeTemplateAction extends AbstractAdminAction<NoticeTemplate> {
	protected NoticeTemplateForm noticeTemplateForm;
	...
	public String index() {
		return "/admin/noticeTemplate/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.noticeTemplate().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = noticeTemplateService.getCount(conditions);

			// 一覧内容取得
			List<NoticeTemplate> list = new ArrayList<NoticeTemplate>();
			if(count > 0){
				list = noticeTemplateService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = noticeTemplateService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute NoticeTemplateForm noticeTemplateForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(NoticeTemplate.class, noticeTemplateForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					NoticeTemplate noticeTemplate = noticeTemplateService.findById(Long.decode(noticeTemplateForm.id));
					...
					if(noticeTemplate != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, NoticeTemplate.class, noticeTemplate)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(NoticeTemplate.class, noticeTemplateForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							noticeTemplateService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(noticeTemplateForm.id));
				int count = noticeTemplateService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = noticeTemplateService.findById(Long.decode(noticeTemplateForm.id));
					//削除実行
					try{
