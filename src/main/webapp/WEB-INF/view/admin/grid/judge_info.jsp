...
function judge_info(subgrid_id, row_id){
	var tabName = 'judge_info';
	var judge_info_gridId = 'main';
	if(subgrid_id != null){
		judge_info_gridId = subgrid_id;
	}
	var judge_info_tabId = tabName;
	if(row_id != null){
		judge_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'judgeman_info', 'judgemaninfoid');

	var judge_info_pagerId = judge_info_tabId+'Nav';
	$('#'+judge_info_gridId).append('<table id="'+judge_info_tabId+'" class="scroll"></table><div id="'+judge_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+judge_info_tabId).jqGrid({
		caption: '<%=lang.__("Data judging info")%>',
		mtype: 'POST',
		...
		colModel:[
			...
			{name:'telemeterdatacode',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Observation code(*)")%>'},
				hidden:false},
			{name:'itemcode',
				width:'150',
				align:'right',
			...
			{name:'judgeformulaid',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Formula ID(*)")%>'},
				hidden:false},
			{name:'note',
				width:'150',
				align:'left',
		...
		beforeProcessing:function(data){
			...
			$('#'+judge_info_tabId).jqGrid('setColProp', 'judgemaninfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#judge_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+judge_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+judge_info_tabId).jqGrid('navGrid',
			'#'+judge_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+judge_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+judge_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Data judging info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					var rowid = $('#'+judge_info_tabId).jqGrid('getGridParam', 'selrow');
					var s = "";
					var rowdata = $('#'+judge_info_tabId).jqGrid('getRowData', rowid);
					s += '&judgemaninfoid='+rowdata.judgemaninfoid;
					$('#'+judge_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+judge_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
	  				var pagenum = $('#'+judge_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+judge_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+judge_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+judge_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+judge_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Data judging info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+judge_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_judgemaninfoid .CaptionTD', formid).after('<td id="dispjudgemaninfoid">'+row_id+'</td>');
					$('#tr_judgemaninfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Data judging info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+judge_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+judge_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Data judging info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+judge_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ judge_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_judge_info #id').val('');
					}else{
...
//	$('#'+judge_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+judge_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
