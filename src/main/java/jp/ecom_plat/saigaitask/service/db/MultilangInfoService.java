...
import static jp.ecom_plat.saigaitask.entity.Names.multilangInfo;
...
public class MultilangInfoService extends AbstractService<MultilangInfo> {

	public MultilangInfo findById(Long id) {
		return select().id(id).getSingleResult();
	}

	public MultilangInfo findByCode(String code) {
		return select()
				.where(eq(multilangInfo().code(), code)
		).getSingleResult();
	}
	 ...
	 * @return 検索結果
	 */
	public List<MultilangInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = multilangInfo().id().toString();
		}
		...
		if(!multilangInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(multilangInfo().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		...
		List<MultilangInfo> list = findByCondition(conditions, multilangInfo().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(MultilangInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
	...
	/**
	 * 言語コードによりスキーマサーチパスを切り替える
	 * @param code 言語コード
	 */
	public void setSchema(String code) {
		String sql = "SET search_path = " + code + ",public";
		jdbcManager.updateBySql(sql).execute();
	}

	@Override
	public DeleteCascadeResult deleteCascade(MultilangInfo entity, DeleteCascadeResult result) {

		result.cascade(MultilangmesInfo.class, Names.multilangmesInfo().multilanginfoid(), entity.id);
		result.cascade(LocalgovInfo.class, Names.localgovInfo().multilanginfoid(), entity.id);
