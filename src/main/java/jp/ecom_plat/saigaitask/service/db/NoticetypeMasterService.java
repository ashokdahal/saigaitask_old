...
import static jp.ecom_plat.saigaitask.entity.Names.noticetypeMaster;
...
public class NoticetypeMasterService extends AbstractService<NoticetypeMaster> {

    public NoticetypeMaster findById(Integer id) {
	...
	public List<NoticetypeMaster> findAll(){
	 ...
	 * @return 検索結果
	 */
	public List<NoticetypeMaster> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = noticetypeMaster().disporder().toString();
		}
		...
		if(!noticetypeMaster().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(noticetypeMaster().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		conditions.put(noticetypeMaster().disporder().toString()+"_IS_NOT_NULL", Boolean.TRUE);
		List<NoticetypeMaster> list = findByCondition(conditions, noticetypeMaster().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(NoticetypeMaster entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
