...
import jp.ecom_plat.saigaitask.form.db.PostingphotolayerInfoForm;
...
@RequestMapping("/admin/postingphotolayerInfo")
public class PostingphotolayerInfoAction  extends AbstractAdminAction<PostingphotolayerInfo> {
	protected PostingphotolayerInfoForm postingphotolayerInfoForm;
	...
	public String index() {
		return "/admin/postingphotolayerInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.postingphotolayerInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = postingphotolayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<PostingphotolayerInfo> list = new ArrayList<PostingphotolayerInfo>();
			if(count > 0){
				list = postingphotolayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute PostingphotolayerInfoForm postingphotolayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			PostingphotolayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(PostingphotolayerInfo.class, postingphotolayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				postingphotolayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					PostingphotolayerInfo postingphotolayerInfo = postingphotolayerInfoService.findById(Long.decode(postingphotolayerInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(postingphotolayerInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, DisasteritemInfo.class, postingphotolayerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(PostingphotolayerInfo.class, postingphotolayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							postingphotolayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(postingphotolayerInfoForm.id));
				int count = postingphotolayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = postingphotolayerInfoService.findById(Long.decode(postingphotolayerInfoForm.id));
					//削除実行
					try{
						postingphotolayerInfoService.delete(entity);
