...
import jp.ecom_plat.saigaitask.form.db.MeteolayerInfoForm;
...
@RequestMapping("/admin/meteolayerInfo")
public class MeteolayerInfoAction extends AbstractAdminAction<MeteolayerInfo> {
	protected MeteolayerInfoForm meteolayerInfoForm;
	...
	public String index() {
		return "/admin/meteolayerInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = meteolayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<MeteolayerInfo> list = new ArrayList<MeteolayerInfo>();
			if(count > 0){
				list = meteolayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = meteolayerInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteolayerInfoForm meteolayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteolayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteolayerInfo.class, meteolayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteolayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteolayerInfo meteolayerInfo = meteolayerInfoService.findById(Long.decode(meteolayerInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteolayerInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteolayerInfo.class, meteolayerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteolayerInfo.class, meteolayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteolayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteolayerInfoForm.id));
				int count = meteolayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteolayerInfoService.findById(Long.decode(meteolayerInfoForm.id));
					//削除実行
					try{
						meteolayerInfoService.delete(entity);
