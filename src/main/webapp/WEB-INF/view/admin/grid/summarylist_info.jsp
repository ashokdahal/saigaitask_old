...
function summarylist_info(subgrid_id, row_id){
	var tabName = 'summarylist_info';
	var summarylist_info_gridId = 'main';
	if(subgrid_id != null){
		summarylist_info_gridId = subgrid_id;
	}
	var summarylist_info_tabId = tabName;
	if(row_id != null){
		summarylist_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var summarylist_info_pagerId = summarylist_info_tabId+'Nav';
	$('#'+summarylist_info_gridId).append('<table id="'+summarylist_info_tabId+'" class="scroll"></table><div id="'+summarylist_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+summarylist_info_tabId).jqGrid({
		caption: '<%=lang.__("Summary list info")%>',
		mtype: 'POST',
		...
		colNames: [
			...
			'<%=lang.__("Summary target table ID")%>',
			'<%=lang.__("Table ID")%>',
			'<%=lang.__("Local gov. code<!--2-->")%>',
		],
		colModel:[
			{name:'id',
				width:'60',
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+summarylist_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+summarylist_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.localgovinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'summarylist_info', 'localgov_info', false);
						}
				},
			...
			{name:'targettablemasterinfoid',
				...
				editoptions:{value:'',
					...
					delimiter:'|d|',
					readonly:false
						,dataUrl:convertTabNameToMethodMame(tabName)+'/createSelectTag/targettablemasterinfoid/tablemaster_info/id/name',
						//フォームのプルダウン作成
						buildSelect: function (data) {
							var selectTag = $.parseJSON(data).selectTag;
							var rowid = $('#'+summarylist_info_tabId).jqGrid('getGridParam', 'selrow');
							...
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+summarylist_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.targettablemasterinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'summarylist_info', 'tablemaster_info', false);
						}
				},
				editrules:{required:true, number:false, integer:false, email:false, edithidden:false},
				sortable:true,
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Summary target table ID(*)")%>'},
				hidden:false},
			...
			{name:'localgovcode',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Local gov. code(*)")%>'},
				hidden:false},
		],
		ajaxGridOptions :{
		...
		pager: summarylist_info_pagerId,
		autowidth: true,
		autoencode: true,
		beforeProcessing:function(data){
			...
			$('#'+summarylist_info_tabId).jqGrid('setColProp', 'localgovinfoid', {editoptions:{value:str}});
			...
			for(i=0; i < data.rows.length; i++){
				...
				var ref = data.rows[i].targettablemasterInfo;
				if(ref != null){
					var refkey = ref.id;
					var refval = ref.name;
					str += refkey + ':' + refkey + '：' + refval;
			...
			$('#'+summarylist_info_tabId).jqGrid('setColProp', 'targettablemasterinfoid', {editoptions:{value:str}});
			...
			$('#'+summarylist_info_tabId).jqGrid('setColProp', 'tablemasterinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#summarylist_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+summarylist_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+summarylist_info_tabId).jqGrid('navGrid',
			'#'+summarylist_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+summarylist_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+summarylist_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Summary list info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					var rowdata = $('#'+summarylist_info_tabId).jqGrid('getRowData', rowid);
					s += '&targettablemasterinfoid='+rowdata.targettablemasterinfoid;
					var rowdata = $('#'+summarylist_info_tabId).jqGrid('getRowData', rowid);
					s += '&tablemasterinfoid='+rowdata.tablemasterinfoid;
					$('#'+summarylist_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+summarylist_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+summarylist_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+summarylist_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});
					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+summarylist_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+summarylist_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+summarylist_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:' ' + '<%=lang.__("Add - Summary list info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+summarylist_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
				beforeInitData:function(formid){
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Summary list info")%>',
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+summarylist_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Copy - Summary list info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
									...
									afterSubmit: function(){	//フォームサブミット後、グリッドデータをサーバから再取得する設定。
										...
										//$('#'+summarylist_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
										$('#'+summarylist_info_tabId).jqGrid('setGridParam',{datatype:'json'});
										return [true,''];
									},
									ajaxEditOptions :{
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+summarylist_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ summarylist_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_summarylist_info #id').val('');
...
//	$('#'+summarylist_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+summarylist_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
	...
	function initAttrSelect() {
		...
		var attrcols = [
			{ name: 'localgovcode', required:true },
		];
		$('#tablemasterinfoid').on('change', function() {
			makeAttrSelect(summarylist_info_tabId, tabName, attrcols, objid);
		});
		makeAttrSelect(summarylist_info_tabId, tabName, attrcols, objid);
	}

	//サブグリッド設定
	$('#'+summarylist_info_tabId).jqGrid('setGridParam',
