...
import jp.ecom_plat.saigaitask.form.db.TelemeterserverInfoForm;
...
@RequestMapping("/admin/telemeterserverInfo")
public class TelemeterserverInfoAction extends AbstractAdminAction<TelemeterserverInfo> {
	protected TelemeterserverInfoForm telemeterserverInfoForm;
	...
	public String index() {
		return "/admin/telemeterserverInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.telemeterserverInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = telemeterserverInfoService.getCount(conditions);

			// 一覧内容取得
			List<TelemeterserverInfo> list = new ArrayList<TelemeterserverInfo>();
			if(count > 0){
				list = telemeterserverInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TelemeterserverInfoForm telemeterserverInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TelemeterserverInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TelemeterserverInfo.class, telemeterserverInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				telemeterserverInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TelemeterserverInfo telemeterserverInfo = telemeterserverInfoService.findById(Long.decode(telemeterserverInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(telemeterserverInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TelemeterserverInfo.class, telemeterserverInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TelemeterserverInfo.class, telemeterserverInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							telemeterserverInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(telemeterserverInfoForm.id));
				int count = telemeterserverInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = telemeterserverInfoService.findById(Long.decode(telemeterserverInfoForm.id));
					//削除実行
					try{
						telemeterserverInfoService.delete(entity);
