...
//public class AlarmmailInfoAction extends AbstractAdminAction<AlarmmailInfo> {
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/alarmmailInfo")
    public class AlarmmailInfoAction {
//    public class AlarmmailInfoAction {
//	//@Resource
//	protected AlarmmailInfoForm alarmmailInfoForm;
//
//	/** サービスクラス */
//	@Resource
//	protected AlarmmailInfoExService alarmmailInfoExService;
//
//	/** サービスクラス */
//	@Resource
...
//
	...
	public String index() {
		return "/admin/alarmmailInfo/index";
	}
//
//	/**
...
//
//		// 一覧件数取得
//		int count = alarmmailInfoExService.getCount(conditions);
//
//		// 一覧内容取得
//		List<AlarmmailInfo> list = new ArrayList<AlarmmailInfo>();
//		if(count > 0){
//			list = alarmmailInfoExService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
//		}
//
//		//ユーザーデータ取得
...
//
//		//操作種別に応じたアクションを実行
//		AlarmmailInfo entity;
//		//登録
//		if("add".equals(oper)) {
//			entity = Beans.createAndCopy(AlarmmailInfo.class, alarmmailInfoForm).dateConverter("yyyy-MM-dd").execute();
//			//登録実行
//			alarmmailInfoExService.insert(entity);
//			//採番したキー値を返却
//			JSONObject json = new JSONObject();
...
//			}else{
//				//現時点の編集対象データを取得
//				AlarmmailInfo alarmmailInfo = alarmmailInfoExService.findById(Integer.parseInt(alarmmailInfoForm.id));
//
//				//結果返却用JSON
//				JSONObject json = new JSONObject();
//				if(alarmmailInfo != null){
//					//編集対象が存在する場合、実行。
//					//現時点の編集対象データが他者により変更されているかを判定
//					if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, AlarmmailInfo.class, alarmmailInfo)){
//						//変更されていない場合、更新
//						entity = Beans.createAndCopy(AlarmmailInfo.class, alarmmailInfoForm).dateConverter("yyyy-MM-dd").execute();
//						PropertyName<?>[] updateExcludesAry = null;
//						List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
//						//更新実行
//						alarmmailInfoExService.update(entity, updateExcludesAry);
...
//			//検索条件マップ
//			BeanMap conditions = new BeanMap();
//			conditions.put("id", Integer.parseInt(alarmmailInfoForm.id));
//			int count = alarmmailInfoExService.getCount(conditions);
//			if(count==1){
//				//削除対象が存在する場合、削除実行。
//				entity = alarmmailInfoExService.findById(Integer.parseInt(alarmmailInfoForm.id));
//				//削除実行
//				alarmmailInfoExService.delete(entity);
//			}else{
