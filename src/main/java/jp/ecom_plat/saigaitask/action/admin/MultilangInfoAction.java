...
import jp.ecom_plat.saigaitask.dto.admin.MultiLangInfoDto;
import jp.ecom_plat.saigaitask.entity.Names;
import jp.ecom_plat.saigaitask.entity.db.MultilangInfo;
import jp.ecom_plat.saigaitask.form.db.MultilangInfoForm;
...
@RequestMapping("/admin/multilangInfo")
public class MultilangInfoAction extends AbstractAdminAction<MultilangInfo> {
	protected MultilangInfoForm multilangInfoForm;
	...
	public String index(Map<String,Object>model) {
		...
		return "/admin/multilangInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = multilangInfoService.getCount(conditions);

			// 一覧内容取得
			List<MultilangInfo> list = new ArrayList<MultilangInfo>();
			List<MultiLangInfoDto> printList = new ArrayList<MultiLangInfoDto>();
			if(count > 0){
				list = multilangInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = multilangInfoService.getLargestDisporder(maxDisporderConditions);
			...
			for(MultilangInfo multilangInfo :list){
				MultiLangInfoDto multiLangInfoDto = new MultiLangInfoDto();
				multiLangInfoDto.id = multilangInfo.id;
				multiLangInfoDto.code = multilangInfo.code;
				multiLangInfoDto.name = multilangInfo.name;
				multiLangInfoDto.disporder = multilangInfo.disporder;
				multiLangInfoDto.updatetime = multilangInfo.updatetime;
				multiLangInfoDto.printUpdatetime =new SimpleDateFormat(lang.__("MMM.d,yyyy 'at' HH:MM<!--3-->"), lang.getLocale())
					.format(multilangInfo.updatetime);
				printList.add(multiLangInfoDto);

			}
			json.put(JQG_PAGER_ROWS, jsonForCyclicalReferences(printList));   //結果一覧
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MultilangInfoForm multilangInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MultilangInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MultilangInfo.class, multilangInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				entity.updatetime = new Timestamp(System.currentTimeMillis());
				//登録実行
				multilangInfoService.insert(entity);
				//全コピー実施時
				...
				if(isAllCopy){
					...
					doAllCopy("multilang_info", oldId, entity.id);
				}
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MultilangInfo multilangInfo = multilangInfoService.findById(Long.decode(multilangInfoForm.id));
					...
					if(multilangInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MultilangInfo.class, multilangInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MultilangInfo.class, multilangInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							entity.updatetime = new Timestamp(System.currentTimeMillis());
							...
							//更新実行
							multilangInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(multilangInfoForm.id));
				int count = multilangInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = multilangInfoService.findById(Long.decode(multilangInfoForm.id));
					//削除実行
					try{
						multilangInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	...
	/**
	 * Jqgrid code selectボックスタグの構成要素を生成（Ajax）
	 * @return null
	 * @throws ServiceException
	 */
	@RequestMapping(value="/createCodeSelect")
	@ResponseBody
	public String createCodeSelect() throws ServiceException {
		try{

			//返却値
			JSONObject json = new JSONObject();

			List<String> codeList = SaigaiTaskLangUtils.getJqgridLangCodes(request);

			List<Map<String, Object>> selectTagList = new ArrayList<Map<String, Object>>();
	        for(Object entity : codeList){
	            Map<String, Object> map = new HashMap<String, Object>();
	            Object codeValue = entity;
	            map.put("key", codeValue);
                map.put("value", codeValue);
	            selectTagList.add(map);
	        }
			json.put(JQG_SELECT_TAG, StringUtil.json(selectTagList));   //結果一覧
