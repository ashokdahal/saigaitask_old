...
import jp.ecom_plat.saigaitask.form.db.MapreferencelayerInfoForm;
...
@RequestMapping("/admin/mapreferencelayerInfo")
public class MapreferencelayerInfoAction extends AbstractAdminAction<MapreferencelayerInfo> {
	protected MapreferencelayerInfoForm mapreferencelayerInfoForm;
	...
	public String index() {
		return "/admin/mapreferencelayerInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = mapreferencelayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<MapreferencelayerInfo> list = new ArrayList<MapreferencelayerInfo>();
			if(count > 0){
				list = mapreferencelayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = mapreferencelayerInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MapreferencelayerInfoForm mapreferencelayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MapreferencelayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MapreferencelayerInfo.class, mapreferencelayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				mapreferencelayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MapreferencelayerInfo mapreferencelayerInfo = mapreferencelayerInfoService.findById(Long.decode(mapreferencelayerInfoForm.id));
					...
					if(mapreferencelayerInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MapreferencelayerInfo.class, mapreferencelayerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MapreferencelayerInfo.class, mapreferencelayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							mapreferencelayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(mapreferencelayerInfoForm.id));
				int count = mapreferencelayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = mapreferencelayerInfoService.findById(Long.decode(mapreferencelayerInfoForm.id));
					//削除実行
					try{
						mapreferencelayerInfoService.delete(entity);
...
//			String parentId = (String)request.getParameter("parentId");
			...
			MenuInfo menuInfo = menuInfoService.findById(Long.parseLong(parentId));
	        ...
	        // 地図参照レイヤ情報テーブルから、
	        // 地図マスタテーブルのコミュニティID,グループID,地図IDを取得する
	        // 道のり：
	        //   地図参照レイヤ情報.メニューID → メニュー情報.ID
	        //     → メニュー情報.タスクID → メニュータスク種別情報.ID
	        //     → メニュータスク種別情報.自治体ID → 地図マスター情報.自治体ID
	        ...
	        //MenuloginInfo menuloginInfo = menuloginInfoService.findById(menuprocessInfo.menulogininfoid);
	        //GroupInfo groupInfo = groupInfoService.findById(menuloginInfo.groupid);
	        MapmasterInfo mapmasterInfo = mapmasterInfoService.findByLocalgovInfoId(menutasktypeInfo.localgovinfoid);
	        ...
	        if(mapmasterInfo != null){
	            ...
	            mapLayerInfos = ecommapDataGetService.getMapUsedReferenceLayersAll(mapmasterInfo.communityid, mapmasterInfo.mapgroupid, mapmasterInfo.mapid);
		  	    ...
		  	    List<MapLayerInfo> mapLayerInfoList = new ArrayList<MapLayerInfo>(mapLayerInfos.keySet());
		  	    if (limit != NON_LIMIT)
	            ...
	            for(MapLayerInfo mapLayerInfo : mapLayerInfoList) {
	            	...
	            	result.setLayerInfo(mapLayerInfo.layerId + ":" + mapLayerInfo.layerName);
	            	result.setChildLayerInfo("");
	            	results.add(result);
	//            	if(mapLayerInfos.get(mapLayerInfo).isEmpty()){
	//                	ResultGetEcommMap result = new ResultGetEcommMap();
	//                	result.setMapInfo(mapmasterInfo.mapid.toString()+":"+ecommapDataGetService.getMapName(mapmasterInfo.mapid));
	//                	result.setLayerInfo(mapLayerInfo.layerId + ":" + mapLayerInfo.layerName);
	//                	result.setChildLayerInfo("");
	//                	results.add(result);
	//            	}else{
	//                    for(MapLayerInfo mapLayerInfoChild : mapLayerInfos.get(mapLayerInfo) ){
	//                    	ResultGetEcommMap result = new ResultGetEcommMap();
	//                    	result.setMapInfo(mapmasterInfo.mapid.toString()+":"+ecommapDataGetService.getMapName(mapmasterInfo.mapid));
	//                    	result.setLayerInfo(mapLayerInfo.layerId + ":" + mapLayerInfo.layerName);
	//                    	result.setChildLayerInfo(mapLayerInfoChild.layerId + ":" + mapLayerInfoChild.layerName);
	//                    	results.add(result);
	//                    }
	//            	}
	            }
...
@RequestMapping("/admin/mapreferencelayerInfo")
    public class ResultGetEcommMap{
    	...
    	public void setChildLayerInfo(String childLayerInfo){
    		this.childLayerInfo = childLayerInfo;
    	}
    	...
    	public String getChildLayerInfo(){
    		return this.childLayerInfo;
    	}
    }
}
