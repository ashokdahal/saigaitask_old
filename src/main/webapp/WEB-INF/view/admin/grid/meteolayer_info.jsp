...
function meteolayer_info(subgrid_id, row_id){
	var tabName = 'meteolayer_info';
	var meteolayer_info_gridId = 'main';
	if(subgrid_id != null){
		meteolayer_info_gridId = subgrid_id;
	}
	var meteolayer_info_tabId = tabName;
	if(row_id != null){
		meteolayer_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'menu_info', 'menuinfoid');

	var meteolayer_info_pagerId = meteolayer_info_tabId+'Nav';
	$('#'+meteolayer_info_gridId).append('<table id="'+meteolayer_info_tabId+'" class="scroll"></table><div id="'+meteolayer_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+meteolayer_info_tabId).jqGrid({
		caption: '<%=lang.__("Weather layer info")%>',
		mtype: 'POST',
		...
		colNames: [
			'ID',
			'<%=lang.__("Menu ID")%>',
			'<%=lang.__("Weather info acquisition type ID")%>',
		...
		colModel:[
			...
			{name:'meteotypeid',
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+meteolayer_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+meteolayer_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.meteotypeid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'meteolayer_info', 'meteotype_master', false);
						}
				},
				editrules:{required:true, number:false, integer:false, email:false, edithidden:false},
				sortable:true,
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Weather info acquisition type ID(*)")%>'},
				hidden:false},
			{name:'visible',
				width:'150',
				align:'left',
		...
		beforeProcessing:function(data){
			...
			$('#'+meteolayer_info_tabId).jqGrid('setColProp', 'menuinfoid', {editoptions:{value:str}});
			...
			$('#'+meteolayer_info_tabId).jqGrid('setColProp', 'meteotypeid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#meteolayer_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+meteolayer_info_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+meteolayer_info_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+meteolayer_info_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+meteolayer_info_tabId).jqGrid('navGrid',
			'#'+meteolayer_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+meteolayer_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+meteolayer_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Weather layer info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+meteolayer_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+meteolayer_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
	  				var pagenum = $('#'+meteolayer_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+meteolayer_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+meteolayer_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+meteolayer_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+meteolayer_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Weather layer info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+meteolayer_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_menuinfoid .CaptionTD', formid).after('<td id="dispmenuinfoid">'+row_id+'</td>');
					$('#tr_menuinfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Weather layer info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+meteolayer_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+meteolayer_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Weather layer info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+meteolayer_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ meteolayer_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_meteolayer_info #id').val('');
					}else{
...
//	$('#'+meteolayer_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+meteolayer_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
