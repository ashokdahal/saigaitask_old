...
import jp.ecom_plat.saigaitask.form.db.TablecalculatecolumnInfoForm;
...
@RequestMapping("/admin/tablecalculatecolumnInfo")
public class TablecalculatecolumnInfoAction extends AbstractAdminAction<TablecalculatecolumnInfo> {
	protected TablecalculatecolumnInfoForm tablecalculatecolumnInfoForm;
	...
	public String index() {
		return "/admin/tablecalculatecolumnInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = tablecalculatecolumnInfoService.getCount(conditions);

			// 一覧内容取得
			List<TablecalculatecolumnInfo> list = new ArrayList<TablecalculatecolumnInfo>();
			if(count > 0){
				list = tablecalculatecolumnInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TablecalculatecolumnInfoForm tablecalculatecolumnInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TablecalculatecolumnInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TablecalculatecolumnInfo.class, tablecalculatecolumnInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				tablecalculatecolumnInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TablecalculatecolumnInfo tablecalculatecolumnInfo = tablecalculatecolumnInfoService.findById(Long.decode(tablecalculatecolumnInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(tablecalculatecolumnInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TablecalculatecolumnInfo.class, tablecalculatecolumnInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TablecalculatecolumnInfo.class, tablecalculatecolumnInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							tablecalculatecolumnInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(tablecalculatecolumnInfoForm.id));
				int count = tablecalculatecolumnInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = tablecalculatecolumnInfoService.findById(Long.decode(tablecalculatecolumnInfoForm.id));
					//削除実行
					try{
						tablecalculatecolumnInfoService.delete(entity);
