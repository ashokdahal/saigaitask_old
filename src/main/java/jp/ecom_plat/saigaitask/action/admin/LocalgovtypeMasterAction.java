...
import jp.ecom_plat.saigaitask.form.db.LocalgovtypeMasterForm;
...
@RequestMapping("/admin/localgovtypeMaster")
public class LocalgovtypeMasterAction extends AbstractAdminAction<LocalgovtypeMaster> {
	protected LocalgovtypeMasterForm localgovtypeMasterForm;

	/** サービスクラス */
	@Resource
	protected LocalgovtypeMasterService localgovtypeMasterService;
	...
	public String index() {
		return "/admin/localgovtypeMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = localgovtypeMasterService.getCount(conditions);

			// 一覧内容取得
			List<LocalgovtypeMaster> list = new ArrayList<LocalgovtypeMaster>();
			if(count > 0){
				list = localgovtypeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = localgovtypeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute LocalgovtypeMasterForm localgovtypeMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			LocalgovtypeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(LocalgovtypeMaster.class, localgovtypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				localgovtypeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					LocalgovtypeMaster localgovtypeMaster = localgovtypeMasterService.findById(Integer.parseInt(localgovtypeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(localgovtypeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, LocalgovtypeMaster.class, localgovtypeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(LocalgovtypeMaster.class, localgovtypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							localgovtypeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(localgovtypeMasterForm.id));
				int count = localgovtypeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = localgovtypeMasterService.findById(Integer.parseInt(localgovtypeMasterForm.id));
					//削除実行
					try{
						localgovtypeMasterService.delete(entity);
