...
import static jp.ecom_plat.saigaitask.entity.Names.judgeInfo;
...
public class JudgeInfoService extends AbstractService<JudgeInfo> {
	 ...
	 * @return 判定情報
	 */
    public JudgeInfo findById(Long id) {
     ...
     * @return 判定情報のリスト
     */
    public List<JudgeInfo> findByJudgemanInfoIdAndValid(Long jmid) {
    	return select().where(
    			and(
    				eq(judgeInfo().judgemaninfoid(), jmid),
    				eq(judgeInfo().valid(), true)
    		)).getResultList();
    }
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(judgeInfo().judgemanInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<JudgeInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = judgeInfo().id().toString();
		}
		...
		if(!judgeInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(judgeInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(JudgeInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
