...
import jp.ecom_plat.saigaitask.entity.db.DecisionsupporttypeMaster;
import jp.ecom_plat.saigaitask.form.db.DecisionsupporttypeMasterForm;
import jp.ecom_plat.saigaitask.service.db.DecisionsupporttypeMasterService;
...
@RequestMapping("/admin/decisionsupporttypeMaster")
public class DecisionsupporttypeMasterAction extends AbstractAdminAction<DecisionsupporttypeMaster> {
	protected DecisionsupporttypeMasterForm decisionsupporttypeMasterForm;

	/** サービスクラス */
	@Resource
	protected DecisionsupporttypeMasterService decisionsupporttypeMasterService;
	...
	public String index() {
		return "/admin/decisionsupporttypeMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = decisionsupporttypeMasterService.getCount(conditions);

			// 一覧内容取得
			List<DecisionsupporttypeMaster> list = new ArrayList<DecisionsupporttypeMaster>();
			if(count > 0){
				list = decisionsupporttypeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = decisionsupporttypeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute DecisionsupporttypeMasterForm decisionsupporttypeMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			DecisionsupporttypeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(DecisionsupporttypeMaster.class, decisionsupporttypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				decisionsupporttypeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					DecisionsupporttypeMaster decisionsupporttypeMaster = decisionsupporttypeMasterService.findById(Long.decode(decisionsupporttypeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(decisionsupporttypeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, DecisionsupporttypeMaster.class, decisionsupporttypeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(DecisionsupporttypeMaster.class, decisionsupporttypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							decisionsupporttypeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(decisionsupporttypeMasterForm.id));
				int count = decisionsupporttypeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = decisionsupporttypeMasterService.findById(Long.decode(decisionsupporttypeMasterForm.id));
					//削除実行
					decisionsupporttypeMasterService.delete(entity);
				}else{
