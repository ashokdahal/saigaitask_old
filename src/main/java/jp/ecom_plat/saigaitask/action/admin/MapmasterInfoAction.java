...
import jp.ecom_plat.saigaitask.form.db.MapmasterInfoForm;
...
@RequestMapping("/admin/mapmasterInfo")
public class MapmasterInfoAction extends AbstractAdminAction<MapmasterInfo> {
	protected MapmasterInfoForm mapmasterInfoForm;
	...
	public String index() {
		return "/admin/mapmasterInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.mapmasterInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = mapmasterInfoService.getCount(conditions);

			// 一覧内容取得
			List<MapmasterInfo> list = new ArrayList<MapmasterInfo>();
			if(count > 0){
				list = mapmasterInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MapmasterInfoForm mapmasterInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MapmasterInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MapmasterInfo.class, mapmasterInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				mapmasterInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MapmasterInfo mapmasterInfo = mapmasterInfoService.findById(Long.decode(mapmasterInfoForm.id));
					...
					if(mapmasterInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MapmasterInfo.class, mapmasterInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MapmasterInfo.class, mapmasterInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							mapmasterInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(mapmasterInfoForm.id));
				int count = mapmasterInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = mapmasterInfoService.findById(Long.decode(mapmasterInfoForm.id));
					//削除実行
					try{
						mapmasterInfoService.deleteLogically(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
     ...
     * @throws Exception
     */
	@RequestMapping(value="/jqgridecommap")
	@ResponseBody
    public String jqgridecommap() throws ServiceException {
		try{
			//検索条件マップ
			BeanMap conditions = new BeanMap();
	        ...
	        // eコミマップユーザ情報取得
	        UserInfo userInfo = ecommapDataGetService.getAuthIdUserInfo(loginDataDto.getEcomUser());

	        //コミュニティ情報取得
	        Map<Integer, String> communityInfos = ecommapDataGetService.getCommunityIds();

	  	    int start = (page - 1) * limit;
	  	    int end = page * limit;
	        for(Integer communityId : communityInfos.keySet()){

	        	//グループ情報取得
	            Map<Integer,String> groupInfos;
	            groupInfos = ecommapDataGetService.getGroupInfos(communityId);
	            for(Integer groupId : groupInfos.keySet() ){

	            	//地図情報取得
	                Map<Long,String> mapInfos;
	                mapInfos = ecommapDataGetService.getMapInfo(userInfo, communityId, groupId);
	                for(Long mapId : mapInfos.keySet() ){
	                	ResultGetEcommMap result = new ResultGetEcommMap();
	                	if (count >= start && count < end) {
		                	result.setCommunityInfo(communityId.toString()+":"+communityInfos.get(communityId));
		                	result.setGroupInfo(groupId.toString()+":"+groupInfos.get(groupId));
		                	result.setMapInfo(mapId.toString()+":"+mapInfos.get(mapId));
		                	results.add(result);
	                	}
	                	count++;
	                }
