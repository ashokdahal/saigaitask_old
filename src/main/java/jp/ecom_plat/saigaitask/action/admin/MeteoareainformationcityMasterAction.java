...
import jp.ecom_plat.saigaitask.form.db.MeteoareainformationcityMasterForm;
...
/**
 * 気象・地震・火山情報/市町村等コードマスタのアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/meteoareainformationcityMaster")
public class MeteoareainformationcityMasterAction extends AbstractAdminAction<MeteoareainformationcityMaster> {
	protected MeteoareainformationcityMasterForm meteoareainformationcityMasterForm;
	...
	public String index() {
		return "/admin/meteoareainformationcityMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = meteoareainformationcityMasterService.getCount(conditions);

			// 一覧内容取得
			List<MeteoareainformationcityMaster> list = new ArrayList<MeteoareainformationcityMaster>();
			if(count > 0){
				list = meteoareainformationcityMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = meteoareainformationcityMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteoareainformationcityMasterForm meteoareainformationcityMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteoareainformationcityMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteoareainformationcityMaster.class, meteoareainformationcityMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteoareainformationcityMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteoareainformationcityMaster meteoareainformationcityMaster = meteoareainformationcityMasterService.findById(Long.decode(meteoareainformationcityMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteoareainformationcityMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteoareainformationcityMaster.class, meteoareainformationcityMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteoareainformationcityMaster.class, meteoareainformationcityMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteoareainformationcityMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteoareainformationcityMasterForm.id));
				int count = meteoareainformationcityMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteoareainformationcityMasterService.findById(Long.decode(meteoareainformationcityMasterForm.id));
					//削除実行
					try{
						meteoareainformationcityMasterService.delete(entity);
