...
import jp.ecom_plat.saigaitask.form.db.JalerttriggerInfoForm;
...
/**
 * JAlert情報トリガー情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/jalerttriggerInfo")
public class JalerttriggerInfoAction extends AbstractAdminAction<JalerttriggerInfo> {
	protected JalerttriggerInfoForm jalerttriggerInfoForm;
	...
	public String index() {
		return "/admin/jalerttriggerInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = jalerttriggerInfoService.getCount(conditions);

			// 一覧内容取得
			List<JalerttriggerInfo> list = new ArrayList<JalerttriggerInfo>();
			if(count > 0){
				list = jalerttriggerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JalerttriggerInfoForm jalerttriggerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			JalerttriggerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JalerttriggerInfo.class, jalerttriggerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				jalerttriggerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JalerttriggerInfo jalerttriggerInfo = jalerttriggerInfoService.findById(Long.decode(jalerttriggerInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(jalerttriggerInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JalerttriggerInfo.class, jalerttriggerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JalerttriggerInfo.class, jalerttriggerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							jalerttriggerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(jalerttriggerInfoForm.id));
				int count = jalerttriggerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = jalerttriggerInfoService.findById(Long.decode(jalerttriggerInfoForm.id));
					//削除実行
					jalerttriggerInfoService.delete(entity);
				}else{
