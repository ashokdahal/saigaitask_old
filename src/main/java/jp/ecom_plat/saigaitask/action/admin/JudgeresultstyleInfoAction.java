...
import jp.ecom_plat.saigaitask.form.db.JudgeresultstyleInfoForm;
...
/**
 * データ判定結果スタイル情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/judgeresultstyleInfo")
public class JudgeresultstyleInfoAction extends AbstractAdminAction<JudgeresultstyleInfo> {
	protected JudgeresultstyleInfoForm judgeresultstyleInfoForm;
	...
	public String index() {
		return "/admin/judgeresultstyleInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = judgeresultstyleInfoService.getCount(conditions);

			// 一覧内容取得
			List<JudgeresultstyleInfo> list = new ArrayList<JudgeresultstyleInfo>();
			if(count > 0){
				list = judgeresultstyleInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JudgeresultstyleInfoForm judgeresultstyleInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			JudgeresultstyleInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JudgeresultstyleInfo.class, judgeresultstyleInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				judgeresultstyleInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JudgeresultstyleInfo judgeresultstyleInfo = judgeresultstyleInfoService.findById(Long.decode(judgeresultstyleInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(judgeresultstyleInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JudgeresultstyleInfo.class, judgeresultstyleInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JudgeresultstyleInfo.class, judgeresultstyleInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							judgeresultstyleInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(judgeresultstyleInfoForm.id));
				int count = judgeresultstyleInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = judgeresultstyleInfoService.findById(Long.decode(judgeresultstyleInfoForm.id));
					//削除実行
					try{
						judgeresultstyleInfoService.delete(entity);
