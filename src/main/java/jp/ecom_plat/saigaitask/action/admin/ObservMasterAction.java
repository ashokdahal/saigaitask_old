...
import jp.ecom_plat.saigaitask.entity.db.ObservMaster;
import jp.ecom_plat.saigaitask.form.db.ObservMasterForm;
import jp.ecom_plat.saigaitask.service.TableService;
import jp.ecom_plat.saigaitask.service.db.ObservMasterService;
...
@RequestMapping("/admin/observMaster")
public class ObservMasterAction extends AbstractAdminAction<ObservMaster> {
	protected ObservMasterForm observMasterForm;

	/** サービスクラス */
	@Resource
	protected ObservMasterService observMasterService;
	...
	public String index() {
		return "/admin/observMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = observMasterService.getCount(conditions);

			// 一覧内容取得
			List<ObservMaster> list = new ArrayList<ObservMaster>();
			if(count > 0){
				list = observMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = observMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ObservMasterForm observMasterForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ObservMaster.class, observMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				observMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ObservMaster observMaster = observMasterService.findById(Integer.parseInt(observMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(observMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ObservMaster.class, observMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ObservMaster.class, observMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							observMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(observMasterForm.id));
				int count = observMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = observMasterService.findById(Integer.parseInt(observMasterForm.id));
					//削除実行
					try{
						observMasterService.delete(entity);
