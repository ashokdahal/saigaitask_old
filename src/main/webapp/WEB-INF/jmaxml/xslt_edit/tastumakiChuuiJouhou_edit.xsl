...
<xsl:template match="/">
	...
	<xsl:for-each select="jmx:Report/jmx_ib:Head/jmx_ib:Headline/jmx_ib:Information">
		...
		<xsl:for-each select="jmx_ib:Item">
			...
			<!-- 注意情報名,type=comboboxsave,levelflag=1_1_field,groupid=2 -->
			<xsl:value-of select="jmx_ib:Kind/jmx_ib:Name"/>
		
			<!-- 注意情報コード,type=combobox,levelflag=1_1_field,dbuid=dbtatsumakiwarncode,groupid=2 -->
			...
			<xsl:for-each select="jmx_ib:Areas/jmx_ib:Area">
				...
				<!-- 領域コード,type=combobox,levelflag=1_1_1_field,dbuid=dbtatsumakiareacode,groupid=1 -->
