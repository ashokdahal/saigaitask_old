...
import jp.ecom_plat.saigaitask.form.db.DisasterMasterForm;
...
@RequestMapping("/admin/disasterMaster")
public class DisasterMasterAction extends AbstractAdminAction<DisasterMaster> {
	protected DisasterMasterForm disasterMasterForm;
	...
	public String index() {
		return "/admin/disasterMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = disasterMasterService.getCount(conditions);

			// 一覧内容取得
			List<DisasterMaster> list = new ArrayList<DisasterMaster>();
			if(count > 0){
				list = disasterMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = disasterMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute DisasterMasterForm disasterMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			DisasterMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(DisasterMaster.class, disasterMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				disasterMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					DisasterMaster disasterMaster = disasterMasterService.findById(Integer.parseInt(disasterMasterForm.id));
					...
					if(disasterMaster != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, DisasterMaster.class, disasterMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(DisasterMaster.class, disasterMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							disasterMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(disasterMasterForm.id));
				int count = disasterMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = disasterMasterService.findById(Integer.parseInt(disasterMasterForm.id));
					//削除実行
					try{
						disasterMasterService.delete(entity);
