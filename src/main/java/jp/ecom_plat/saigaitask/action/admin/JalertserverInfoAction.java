...
import jp.ecom_plat.saigaitask.form.db.JalertserverInfoForm;
...
/**
 * JAlertサーバ情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/jalertserverInfo")
public class JalertserverInfoAction extends AbstractAdminAction<JalertserverInfo> {
	protected JalertserverInfoForm jalertserverInfoForm;
	...
	public String index() {
		return "/admin/jalertserverInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.jalertserverInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = jalertserverInfoService.getCount(conditions);

			// 一覧内容取得
			List<JalertserverInfo> list = new ArrayList<JalertserverInfo>();
			if(count > 0){
				list = jalertserverInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JalertserverInfoForm jalertserverInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			JalertserverInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JalertserverInfo.class, jalertserverInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				if(StringUtils.isNotEmpty(entity.password)){
					//暗号化対象を暗号化
					entity.password = UserAuthorization.getEncryptedPass(entity.password);
				}
				//登録実行
				jalertserverInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JalertserverInfo jalertserverInfo = jalertserverInfoService.findById(Long.decode(jalertserverInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(jalertserverInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JalertserverInfo.class, jalertserverInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JalertserverInfo.class, jalertserverInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							...
							//更新実行
							jalertserverInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(jalertserverInfoForm.id));
				int count = jalertserverInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = jalertserverInfoService.findById(Long.decode(jalertserverInfoForm.id));
					//削除実行
					jalertserverInfoService.delete(entity);
				}else{
