...
import jp.ecom_plat.saigaitask.entity.names.LandmarkInfoNames;
import jp.ecom_plat.saigaitask.form.db.LandmarkInfoForm;
...
@RequestMapping("/admin/landmarkInfo")
public class LandmarkInfoAction extends AbstractAdminAction<LandmarkInfo> {
	protected LandmarkInfoForm landmarkInfoForm;
	...
	public String index() {
		return "/admin/landmarkInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.landmarkInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = landmarkInfoService.getCount(conditions);

			// 一覧内容取得
			List<LandmarkInfo> list = new ArrayList<LandmarkInfo>();
			if(count > 0){
				list = landmarkInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute LandmarkInfoForm landmarkInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(LandmarkInfo.class, landmarkInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//localgovinfoid重複チェック
				BeanMap conditions = new BeanMap();
				conditions.put(LandmarkInfoNames.localgovinfoid().toString(), entity.localgovinfoid);
				if(landmarkInfoService.getCount(conditions)>0){
					json.put(JQG_MESSAGE, lang.__("Landmark info record of local gov. ID [{0}] already registered.", entity.localgovinfoid));
					responseService.responseJsonText(json);
					return null;
				}else{
					//登録実行
					landmarkInfoService.insert(entity);
					//採番したキー値を返却
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					LandmarkInfo landmarkInfo = landmarkInfoService.findById(Long.decode(landmarkInfoForm.id));

					if(landmarkInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, LandmarkInfo.class, landmarkInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(LandmarkInfo.class, landmarkInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							//localgovinfoid重複チェック
							BeanMap conditions = new BeanMap();
							...
							if(landmarkInfoService.getCount(conditions)>0){
								...
								//更新実行
								landmarkInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(landmarkInfoForm.id));
				int count = landmarkInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = landmarkInfoService.findById(Long.decode(landmarkInfoForm.id));
					//削除実行
					try{
						landmarkInfoService.delete(entity);
