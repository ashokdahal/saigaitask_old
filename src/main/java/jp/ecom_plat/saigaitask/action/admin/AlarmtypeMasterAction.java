...
import jp.ecom_plat.saigaitask.entity.db.AlarmtypeMaster;
import jp.ecom_plat.saigaitask.form.db.AlarmtypeMasterForm;
import jp.ecom_plat.saigaitask.service.TableService;
import jp.ecom_plat.saigaitask.service.db.AlarmtypeMasterService;
...
@RequestMapping("/admin/alarmtypeMaster")
public class AlarmtypeMasterAction extends AbstractAdminAction<AlarmtypeMaster> {
	protected AlarmtypeMasterForm alarmtypeMasterForm;

	/** サービスクラス */
	@Resource
	protected AlarmtypeMasterService alarmtypeMasterService;
	...
	public String index() {
		return "/admin/alarmtypeMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = alarmtypeMasterService.getCount(conditions);

			// 一覧内容取得
			List<AlarmtypeMaster> list = new ArrayList<AlarmtypeMaster>();
			if(count > 0){
				list = alarmtypeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = alarmtypeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute AlarmtypeMasterForm alarmtypeMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			AlarmtypeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(AlarmtypeMaster.class, alarmtypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				alarmtypeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					AlarmtypeMaster alarmtypeMaster = alarmtypeMasterService.findById(Integer.parseInt(alarmtypeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(alarmtypeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, AlarmtypeMaster.class, alarmtypeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(AlarmtypeMaster.class, alarmtypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							alarmtypeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(alarmtypeMasterForm.id));
				int count = alarmtypeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = alarmtypeMasterService.findById(Integer.parseInt(alarmtypeMasterForm.id));
					//削除実行
					try{
					alarmtypeMasterService.delete(entity);
