...
import jp.ecom_plat.saigaitask.form.db.JsonimportapiInfoForm;
...
@RequestMapping("/admin/jsonimportapiInfo")
public class JsonimportapiInfoAction extends AbstractAdminAction<JsonimportapiInfo> {

	/** サービスクラス */
	@Resource
	protected JsonimportapiInfoService jsonimportapiInfoService;
	@Resource
	protected JsonImportService jsonImortService;
	...
	public String index() {
		return "/admin/jsonimportapiInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.jsonimportapiInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = jsonimportapiInfoService.getCount(conditions);

			// 一覧内容取得
			List<JsonimportapiInfo> list = new ArrayList<JsonimportapiInfo>();
			if(count > 0){
				list = jsonimportapiInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JsonimportapiInfoForm jsonimportapiInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			JsonimportapiInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JsonimportapiInfo.class, jsonimportapiInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				jsonimportapiInfoService.insert(entity);
				//バックグラウンド処理に反映
				jsonImortService.reschedule(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JsonimportapiInfo jsonimportapiInfo = jsonimportapiInfoService.findById(Long.decode(jsonimportapiInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(jsonimportapiInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JsonimportapiInfo.class, jsonimportapiInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JsonimportapiInfo.class, jsonimportapiInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							jsonimportapiInfoService.update(entity, updateExcludesAry);
							//バックグラウンド処理に反映
							jsonImortService.reschedule(entity);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(jsonimportapiInfoForm.id));
				int count = jsonimportapiInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = jsonimportapiInfoService.findById(Long.decode(jsonimportapiInfoForm.id));
					//削除実行
					try{
						jsonimportapiInfoService.delete(entity);
						//バックグラウンド処理に反映
