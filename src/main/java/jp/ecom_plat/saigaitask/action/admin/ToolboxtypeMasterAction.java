...
import static jp.ecom_plat.saigaitask.util.Constants.JQG_CODE_COLUMN;
import static jp.ecom_plat.saigaitask.util.Constants.JQG_DECODE_COLUMN;
...
import static jp.ecom_plat.saigaitask.util.Constants.JQG_SELECT_TAG_COLUMN;
import static jp.ecom_plat.saigaitask.util.Constants.JQG_SERIALIZED_PRE_EDIT_DATA;
import static jp.ecom_plat.saigaitask.util.Constants.JQG_TABLE_NAME;
...
import jp.ecom_plat.saigaitask.entity.db.ToolboxtypeMaster;
import jp.ecom_plat.saigaitask.form.db.GroupInfoForm;
import jp.ecom_plat.saigaitask.form.db.ToolboxtypeMasterForm;
import jp.ecom_plat.saigaitask.service.db.ToolboxtypeMasterService;
...
/**
 * 意思決定支援レイヤ種別マスタのアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/toolboxtypeMaster")
public class ToolboxtypeMasterAction extends AbstractAdminAction<ToolboxtypeMaster> {
	/** アクションフォーム */
	protected ToolboxtypeMasterForm toolboxtypeMasterForm;

	/** サービスクラス */
	@Resource
	protected ToolboxtypeMasterService toolboxtypeMasterService;
	...
	public String index(Map<String,Object>model, @ModelAttribute ToolboxtypeMasterForm toolboxtypeMasterForm) {
		this.toolboxtypeMasterForm = toolboxtypeMasterForm;
		return "/admin/toolboxtypeMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = toolboxtypeMasterService.getCount(conditions);

			// 一覧内容取得
			List<ToolboxtypeMaster> list = new ArrayList<ToolboxtypeMaster>();
			if(count > 0){
				list = toolboxtypeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = toolboxtypeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ToolboxtypeMasterForm toolboxtypeMasterForm) throws ServiceException {
		try{
			//操作種別取得
			...
			//操作種別に応じたアクションを実行
			ToolboxtypeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ToolboxtypeMaster.class, toolboxtypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				toolboxtypeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ToolboxtypeMaster toolboxtypeMaster = toolboxtypeMasterService.findById(Long.decode(toolboxtypeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(toolboxtypeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ToolboxtypeMaster.class, toolboxtypeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ToolboxtypeMaster.class, toolboxtypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							toolboxtypeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(toolboxtypeMasterForm.id));
				int count = toolboxtypeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = toolboxtypeMasterService.findById(Long.decode(toolboxtypeMasterForm.id));
					//削除実行
					toolboxtypeMasterService.delete(entity);
				}else{
