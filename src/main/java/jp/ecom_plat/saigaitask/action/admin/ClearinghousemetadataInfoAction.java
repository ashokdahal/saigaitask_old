...
import jp.ecom_plat.saigaitask.form.db.ClearinghousemetadataInfoForm;
...
/**
 * クリアリングハウス事前データ情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/clearinghousemetadataInfo")
public class ClearinghousemetadataInfoAction extends AbstractAdminAction<ClearinghousemetadataInfo> {
	protected ClearinghousemetadataInfoForm clearinghousemetadataInfoForm;
	...
	public String index() {
		...
		if (loginDataDto.isUseCkan()) {
			return "forward:/admin/ckanmetadataInfo";
		}

		return "/admin/clearinghousemetadataInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.clearinghousemetadataInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = clearinghousemetadataInfoService.getCount(conditions);

			// 一覧内容取得
			List<ClearinghousemetadataInfo> list = new ArrayList<ClearinghousemetadataInfo>();
			if(count > 0){
				list = clearinghousemetadataInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ClearinghousemetadataInfoForm clearinghousemetadataInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ClearinghousemetadataInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ClearinghousemetadataInfo.class, clearinghousemetadataInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ClearinghousemetadataInfo clearinghousemetadataInfo = clearinghousemetadataInfoService.findById(Long.decode(clearinghousemetadataInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(clearinghousemetadataInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ClearinghousemetadataInfo.class, clearinghousemetadataInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ClearinghousemetadataInfo.class, clearinghousemetadataInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							clearinghousemetadataInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				conditions.put("id", Integer.parseInt(clearinghousemetadataInfoForm.id));
				int count = clearinghousemetadataInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = clearinghousemetadataInfoService.findById(Long.decode(clearinghousemetadataInfoForm.id));
					String mdFileID = entity.metadataid;
					//削除実行
					try{
						clearinghousemetadataInfoService.delete(entity);

						//クリアリングハウスの削除を行う
						boolean ret = CSWUtil.deleteMetadata(mdFileID);
						if (!ret) {
							JSONObject json = new JSONObject();
							json.put(JQG_MESSAGE, lang.__("MetadataID not registered in clearing house."));
							responseService.responseJson(json);
