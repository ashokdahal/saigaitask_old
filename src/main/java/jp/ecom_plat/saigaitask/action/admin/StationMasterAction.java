...
import jp.ecom_plat.saigaitask.form.db.StationMasterForm;
...
@RequestMapping("/admin/stationMaster")
public class StationMasterAction extends AbstractAdminAction<StationMaster> {
	protected StationMasterForm stationMasterForm;
	...
	public String index() {
		return "/admin/stationMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = stationMasterService.getCount(conditions);

			// 一覧内容取得
			List<StationMaster> list = new ArrayList<StationMaster>();
			if(count > 0){
				list = stationMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = stationMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute StationMasterForm stationMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			StationMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(StationMaster.class, stationMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				stationMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					StationMaster stationMaster = stationMasterService.findById(Integer.parseInt(stationMasterForm.id));
					...
					if(stationMaster != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, StationMaster.class, stationMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(StationMaster.class, stationMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							stationMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(stationMasterForm.id));
				int count = stationMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = stationMasterService.findById(Integer.parseInt(stationMasterForm.id));
					//削除実行
					try{
						stationMasterService.delete(entity);
