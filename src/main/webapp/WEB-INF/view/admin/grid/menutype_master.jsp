...
function menutype_master(subgrid_id, row_id){
	var tabName = 'menutype_master';
	var menutype_master_gridId = 'main';
	if(subgrid_id != null){
		menutype_master_gridId = subgrid_id;
	}
	var menutype_master_tabId = tabName;
	if(row_id != null){
		menutype_master_tabId += row_id;
	}else{
		row_id = '';
	}

	var menutype_master_pagerId = menutype_master_tabId+'Nav';
	$('#'+menutype_master_gridId).append('<table id="'+menutype_master_tabId+'" class="scroll"></table><div id="'+menutype_master_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+menutype_master_tabId).jqGrid({
		caption: '<%=lang.__("Menu type master")%>',
		mtype: 'POST',
		...
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#menutype_masterNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+menutype_master_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+menutype_master_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+menutype_master_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+menutype_master_tabId).jqGrid('navGrid',
			'#'+menutype_master_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+menutype_master_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+menutype_master_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Menu type master")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					var rowid = $('#'+menutype_master_tabId).jqGrid('getGridParam', 'selrow');
					var s = "";
					$('#'+menutype_master_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+menutype_master_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+menutype_master_tabId).jqGrid('getGridParam', 'page');
					$('#'+menutype_master_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+menutype_master_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+menutype_master_tabId).jqGrid('getGridParam', 'page');
					$('#'+menutype_master_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Menu type master")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+menutype_master_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Menu type master")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+menutype_master_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+menutype_master_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Menu type master")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+menutype_master_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ menutype_master_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_menutype_master #id').val('');
					}else{
...
//	$('#'+menutype_master_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+menutype_master_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
