...
import jp.ecom_plat.saigaitask.form.db.StationalarmInfoForm;
...
@RequestMapping("/admin/stationalarmInfo")
public class StationalarmInfoAction extends AbstractAdminAction<StationalarmInfo> {
	protected StationalarmInfoForm stationalarmInfoForm;

	/** サービスクラス */
	@Resource
	protected StationalarmInfoService stationalarmInfoService;
	...
	public String index() {
		return "/admin/stationalarmInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.stationalarmInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = stationalarmInfoService.getCount(conditions);

			// 一覧内容取得
			List<StationalarmInfo> list = new ArrayList<StationalarmInfo>();
			if(count > 0){
				list = stationalarmInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute StationalarmInfoForm stationalarmInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			StationalarmInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(StationalarmInfo.class, stationalarmInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				stationalarmInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					StationalarmInfo stationalarmInfo = stationalarmInfoService.findById(Long.decode(stationalarmInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(stationalarmInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, StationalarmInfo.class, stationalarmInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(StationalarmInfo.class, stationalarmInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							stationalarmInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(stationalarmInfoForm.id));
				int count = stationalarmInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = stationalarmInfoService.findById(Long.decode(stationalarmInfoForm.id));
					//削除実行
					try{
						stationalarmInfoService.delete(entity);
