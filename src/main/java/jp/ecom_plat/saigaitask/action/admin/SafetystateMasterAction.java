...
import jp.ecom_plat.saigaitask.entity.db.SafetystateMaster;
import jp.ecom_plat.saigaitask.form.db.SafetystateMasterForm;
...
@RequestMapping("/admin/safetystateMaster")
public class SafetystateMasterAction extends AbstractAdminAction<SafetystateMaster> {
	protected SafetystateMasterForm safetystateMasterForm;

	/** サービスクラス */
	@Resource
	protected SafetystateMasterService safetystateMasterService;
	...
	public String index() {
		return "/admin/safetystateMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = safetystateMasterService.getCount(conditions);

			// 一覧内容取得
			List<SafetystateMaster> list = new ArrayList<SafetystateMaster>();
			if(count > 0){
				list = safetystateMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = safetystateMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute SafetystateMasterForm safetystateMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			SafetystateMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(SafetystateMaster.class, safetystateMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				safetystateMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					SafetystateMaster safetystateMaster = safetystateMasterService.findById(Integer.parseInt(safetystateMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(safetystateMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, SafetystateMaster.class, safetystateMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(SafetystateMaster.class, safetystateMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							safetystateMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(safetystateMasterForm.id));
				int count = safetystateMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = safetystateMasterService.findById(Integer.parseInt(safetystateMasterForm.id));
					//削除実行
					try{
						safetystateMasterService.delete(entity);
