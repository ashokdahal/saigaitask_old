...
import jp.ecom_plat.saigaitask.form.db.TablerowstyleInfoForm;
...
@RequestMapping("/admin/tablerowstyleInfo")
public class TablerowstyleInfoAction extends AbstractAdminAction<TablerowstyleInfo> {
	protected TablerowstyleInfoForm tablerowstyleInfoForm;
	...
	public String index() {
		return "/admin/tablerowstyleInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = tablerowstyleInfoService.getCount(conditions);

			// 一覧内容取得
			List<TablerowstyleInfo> list = new ArrayList<TablerowstyleInfo>();
			if(count > 0){
				list = tablerowstyleInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TablerowstyleInfoForm tablerowstyleInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TablerowstyleInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TablerowstyleInfo.class, tablerowstyleInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				tablerowstyleInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TablerowstyleInfo tablerowstyleInfo = tablerowstyleInfoService.findById(Long.decode(tablerowstyleInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(tablerowstyleInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TablerowstyleInfo.class, tablerowstyleInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TablerowstyleInfo.class, tablerowstyleInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							tablerowstyleInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(tablerowstyleInfoForm.id));
				int count = tablerowstyleInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = tablerowstyleInfoService.findById(Long.decode(tablerowstyleInfoForm.id));
					//削除実行
					try{
						tablerowstyleInfoService.delete(entity);
