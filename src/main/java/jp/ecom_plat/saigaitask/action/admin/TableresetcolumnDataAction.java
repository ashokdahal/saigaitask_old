...
import jp.ecom_plat.saigaitask.form.db.TableresetcolumnDataForm;
...
/**
 * テーブルリセット対象属性情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/tableresetcolumnData")
public class TableresetcolumnDataAction extends AbstractAdminAction<TableresetcolumnData> {
	protected TableresetcolumnDataForm tableresetcolumnDataForm;
	...
	public String index() {
		return "/admin/tableresetcolumnData/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = tableresetcolumnDataService.getCount(conditions);

			// 一覧内容取得
			List<TableresetcolumnData> list = new ArrayList<TableresetcolumnData>();
			if(count > 0){
				list = tableresetcolumnDataService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TableresetcolumnDataForm tableresetcolumnDataForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TableresetcolumnData entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TableresetcolumnData.class, tableresetcolumnDataForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				tableresetcolumnDataService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TableresetcolumnData tableresetcolumnData = tableresetcolumnDataService.findById(Long.decode(tableresetcolumnDataForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(tableresetcolumnData != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TableresetcolumnData.class, tableresetcolumnData)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TableresetcolumnData.class, tableresetcolumnDataForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							tableresetcolumnDataService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(tableresetcolumnDataForm.id));
				int count = tableresetcolumnDataService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = tableresetcolumnDataService.findById(Long.decode(tableresetcolumnDataForm.id));
					//削除実行
					tableresetcolumnDataService.delete(entity);
				}else{
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	...
	public String jqgridecommap(
		...
		try{
			...
			//テーブルマスタ情報のレイヤIDを取得
//			String parentId = (String)request.getParameter("parentId");
			conditions.put(Names.tablemasterInfo().id().toString(), parentId);
			List<TablemasterInfo> tablemasterInfoList = tablemasterInfoService.findByCondition(conditions, null, null, null, null);
			TablemasterInfo tablemasterInfo = tablemasterInfoList.get(0);
