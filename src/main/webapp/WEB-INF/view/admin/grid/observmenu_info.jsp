...
function observmenu_info(subgrid_id, row_id){
	var tabName = 'observmenu_info';
	var observmenu_info_gridId = 'main';
	if(subgrid_id != null){
		observmenu_info_gridId = subgrid_id;
	}
	var observmenu_info_tabId = tabName;
	if(row_id != null){
		observmenu_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var observmenu_info_pagerId = observmenu_info_tabId+'Nav';
	$('#'+observmenu_info_gridId).append('<table id="'+observmenu_info_tabId+'" class="scroll"></table><div id="'+observmenu_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+observmenu_info_tabId).jqGrid({
		caption: '<%=lang.__("Monitoring/observation menu info")%>',
		mtype: 'POST',
		...
		colNames: [
			'ID',
			'<%=lang.__("Menu ID")%>',
			'<%=lang.__("Number of times display data")%>',
			'<%=lang.__("Notes")%>',
		],
		colModel:[
			{name:'id',
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+observmenu_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+observmenu_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.menuinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'observmenu_info', 'menu_info', false);
						}
				},
			...
			{name:'count',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Number of times display data(*)")%>'},
				hidden:false},
			{name:'note',
				width:'150',
				align:'left',
		...
		beforeProcessing:function(data){
			...
			$('#'+observmenu_info_tabId).jqGrid('setColProp', 'menuinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#observmenu_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+observmenu_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+observmenu_info_tabId).jqGrid('navGrid',
			'#'+observmenu_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+observmenu_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+observmenu_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Monitoring/observation menu info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+observmenu_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+observmenu_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
	  				var pagenum = $('#'+observmenu_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+observmenu_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+observmenu_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+observmenu_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+observmenu_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Monitoring/observation menu info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+observmenu_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Monitoring/observation menu info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+observmenu_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+observmenu_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Monitoring/observation menu info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+observmenu_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ observmenu_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_observmenu_info #id').val('');
					}else{
...
//	$('#'+observmenu_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+observmenu_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');

	//サブグリッド設定
	$('#'+observmenu_info_tabId).jqGrid('setGridParam',
			{subGridRowExpanded: function(subgrid_id, row_id) {
				observlist_info(subgrid_id, row_id);
			}}
	);	//サブグリッドend
