...
import jp.ecom_plat.saigaitask.form.db.IssuelayerInfoForm;
...
@RequestMapping("/admin/issuelayerInfo")
public class IssuelayerInfoAction extends AbstractAdminAction<IssuelayerInfo> {
	protected IssuelayerInfoForm issuelayerInfoForm;

	/** サービスクラス */
	@Resource
	protected IssuelayerInfoService issuelayerInfoService;
	@Resource
	...
	public String index() {
		return "/admin/issuelayerInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.issuelayerInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = issuelayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<IssuelayerInfo> list = new ArrayList<IssuelayerInfo>();
			if(count > 0){
				list = (List<IssuelayerInfo>) issuelayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute IssuelayerInfoForm issuelayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			IssuelayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(IssuelayerInfo.class, issuelayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				issuelayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					IssuelayerInfo issuelayerInfo = issuelayerInfoService.findById(Long.decode(issuelayerInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(issuelayerInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, IssuelayerInfo.class, issuelayerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(IssuelayerInfo.class, issuelayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							issuelayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(issuelayerInfoForm.id));
				int count = issuelayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = issuelayerInfoService.findById(Long.decode(issuelayerInfoForm.id));
					//削除実行
					try{
						issuelayerInfoService.delete(entity);
