...
function maplayerattr_info(subgrid_id, row_id){
	var tabName = 'maplayerattr_info';
	var maplayerattr_info_gridId = 'main';
	if(subgrid_id != null){
		maplayerattr_info_gridId = subgrid_id;
	}
	var maplayerattr_info_tabId = tabName;
	if(row_id != null){
		maplayerattr_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'maplayer_info', 'maplayerinfoid');

	var maplayerattr_info_pagerId = maplayerattr_info_tabId+'Nav';
	$('#'+maplayerattr_info_gridId).append('<table id="'+maplayerattr_info_tabId+'" class="scroll"></table><div id="'+maplayerattr_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+maplayerattr_info_tabId).jqGrid({
		caption: '<%=lang.__("Map layer attribute info")%>',
		mtype: 'POST',
		url:convertTabNameToMethodMame(tabName)+'/jqgridindex/'+parentgrid_refkey+'/'+row_id+'',
		datatype: 'json',
		colNames: [
			'ID',
			'<%=lang.__("Map layer ID")%>',
			'<%=lang.__("Attribute item name")%>',
			'<%=lang.__("Name")%>',
			'<%=lang.__("Edit flag")%>',
			'<%=lang.__("Highlight")%>',
			'<%=lang.__("Grouping")%>',
			'<%=lang.__("Initial check")%>',
			'<%=lang.__("Group initial check")%>',
			'<%=lang.__("Batch addition")%>',
		...
		colModel:[
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Map layer ID(*)")%>'},
			hidden:false},
			{name:'attrid',
			width:'150',
			align:'left',
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Edit flag(*)")%>'},
			hidden:false},
			{name:'highlight',
			width:'150',
			align:'left',
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Highlight(*)")%>'},
			hidden:false},
			{name:'grouping',
			width:'150',
			align:'left',
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Grouping(*)")%>'},
			hidden:false},
			{name:'defaultcheck',
				width:'150',
				align:'left',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Initial check(*)")%>'},
				hidden:false},
			{name:'groupdefaultcheck',
				width:'150',
				align:'left',
				formatter: 'select',
				edittype:'select',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Group initial check(*)")%>'},
				hidden:false},
			{name:'addable',
				width:'150',
				align:'left',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Batch addition(*)")%>'},
				hidden:false},
			{name:'disporder',
			width:'100',
			align:'right',
		...
		pager: maplayerattr_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			for(i=0; i < data.rows.length; i++){
				...
				var ref = data.rows[i].maplayerInfo;
				if(ref != null){
					var refkey = ref.id;
					str += refkey + ':' + refkey;
				}
			...
			$('#'+maplayerattr_info_tabId).jqGrid('setColProp', 'maplayerinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+maplayerattr_info_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+maplayerattr_info_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+maplayerattr_info_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+maplayerattr_info_tabId).jqGrid('navGrid',
			'#'+maplayerattr_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+maplayerattr_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+maplayerattr_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Map layer attribute info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					var rowid = $('#'+maplayerattr_info_tabId).jqGrid('getGridParam', 'selrow');
					var s = "";
					var rowdata = $('#'+maplayerattr_info_tabId).jqGrid('getRowData', rowid);
					s += '&maplayerinfoid='+rowdata.maplayerinfoid;
					$('#'+maplayerattr_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
					$('#tr_id .CaptionTD', formid).after('<td id="dispid">'+rowid+'</td>');
					$('#tr_id .DataTD', formid).hide();
					$('#tr_maplayerinfoid .CaptionTD', formid).after('<td id="dispmaplayerinfoid">'+row_id+'</td>');
					$('#tr_maplayerinfoid .DataTD', formid).hide();
					//フォームモードに編集モードを設定
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+maplayerattr_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+maplayerattr_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+maplayerattr_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+maplayerattr_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+maplayerattr_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+maplayerattr_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
				...
				afterclickPgButtons:function(whichbutton, formid, rowid){
					...
					$('#tr_id .CaptionTD', formid).after('<td id="dispid">'+rowid+'</td>');
					$('#tr_id .DataTD', formid).hide();
					$('#tr_maplayerinfoid #dispmaplayerinfoid', formid).remove();
					$('#tr_maplayerinfoid .CaptionTD', formid).after('<td id="dispmaplayerinfoid">'+row_id+'</td>');
					$('#tr_maplayerinfoid .DataTD', formid).hide();
					return [true,''];
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Map layer attribute info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+maplayerattr_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_maplayerinfoid .CaptionTD', formid).after('<td id="dispmaplayerinfoid">'+row_id+'</td>');
					$('#tr_maplayerinfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Map layer attribute info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+maplayerattr_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+maplayerattr_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Map layer attribute info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+maplayerattr_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ maplayerattr_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_maplayerattr_info #id').val('');
					}else{
...
//	$('#'+maplayerattr_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+maplayerattr_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
...
//					},
					...
					caption: '<%=lang.__("e-Com map layer attribute info")%>',
					loadtext:'',
					loadui:'block',
					loadonce:false,
					rowNum:30,
