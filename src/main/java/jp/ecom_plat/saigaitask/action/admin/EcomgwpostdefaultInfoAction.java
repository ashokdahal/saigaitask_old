...
import jp.ecom_plat.saigaitask.form.db.EcomgwpostdefaultInfoForm;
...
/**
 * eコミグループウェア投稿先デフォルト情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/ecomgwpostdefaultInfo")
public class EcomgwpostdefaultInfoAction extends AbstractAdminAction<EcomgwpostdefaultInfo> {
	protected EcomgwpostdefaultInfoForm ecomgwpostdefaultInfoForm;
	...
	public String index() {
		return "/admin/ecomgwpostdefaultInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = ecomgwpostdefaultInfoService.getCount(conditions);

			// 一覧内容取得
			List<EcomgwpostdefaultInfo> list = new ArrayList<EcomgwpostdefaultInfo>();
			if(count > 0){
				list = ecomgwpostdefaultInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute EcomgwpostdefaultInfoForm ecomgwpostdefaultInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			EcomgwpostdefaultInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(EcomgwpostdefaultInfo.class, ecomgwpostdefaultInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				ecomgwpostdefaultInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					EcomgwpostdefaultInfo ecomgwpostdefaultInfo = ecomgwpostdefaultInfoService.findById(Long.decode(ecomgwpostdefaultInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(ecomgwpostdefaultInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, EcomgwpostdefaultInfo.class, ecomgwpostdefaultInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(EcomgwpostdefaultInfo.class, ecomgwpostdefaultInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							ecomgwpostdefaultInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(ecomgwpostdefaultInfoForm.id));
				int count = ecomgwpostdefaultInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = ecomgwpostdefaultInfoService.findById(Long.decode(ecomgwpostdefaultInfoForm.id));
					//削除実行
					try{
						ecomgwpostdefaultInfoService.delete(entity);
