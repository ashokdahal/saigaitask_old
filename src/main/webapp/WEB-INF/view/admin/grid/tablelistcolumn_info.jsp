...
function tablelistcolumn_info(subgrid_id, row_id){
	var tabName = 'tablelistcolumn_info';
	var tablelistcolumn_info_gridId = 'main';
	if(subgrid_id != null){
		tablelistcolumn_info_gridId = subgrid_id;
	}
	var tablelistcolumn_info_tabId = tabName;
	if(row_id != null){
		tablelistcolumn_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'menutable_info', 'menutableinfoid');

	var tablelistcolumn_info_pagerId = tablelistcolumn_info_tabId+'Nav';
	$('#'+tablelistcolumn_info_gridId).append('<table id="'+tablelistcolumn_info_tabId+'" class="scroll"></table><div id="'+tablelistcolumn_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+tablelistcolumn_info_tabId).jqGrid({
		caption: '<%=lang.__("Table list item info")%>',
		mtype: 'POST',
		...
		colNames: [
			...
			'<%=lang.__("Grouping")%>',
			'<%=lang.__("Allowed to sort")%>',
			'<%=lang.__("Default sort item")%>',
			'<%=lang.__("Allowed to upload file")%>',
			'<%=lang.__("Allowed to log output")%>',
		...
		colModel:[
			...
			{name:'sortable',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Allowed to sort(*)")%>'},
				hidden:false},
			{name:'defaultsort',
				width:'150',
				align:'right',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Default sort item(*)")%>'},
				hidden:false},
			{name:'uploadable',
				width:'150',
				align:'left',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Allowed to upload file(*)")%>'},
				hidden:false},
			{name:'loggable',
				width:'150',
				align:'left',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Allowed to log output(*)")%>'},
				hidden:false},
			{name:'defaultcheck',
				width:'150',
				align:'left',
		...
		pager: tablelistcolumn_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+tablelistcolumn_info_tabId).jqGrid('setColProp', 'menutableinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+tablelistcolumn_info_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+tablelistcolumn_info_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+tablelistcolumn_info_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+tablelistcolumn_info_tabId).jqGrid('navGrid',
			'#'+tablelistcolumn_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+tablelistcolumn_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+tablelistcolumn_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Table list item info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					var rowid = $('#'+tablelistcolumn_info_tabId).jqGrid('getGridParam', 'selrow');
					var s = "";
					var rowdata = $('#'+tablelistcolumn_info_tabId).jqGrid('getRowData', rowid);
					s += '&menutableinfoid='+rowdata.menutableinfoid;
					$('#'+tablelistcolumn_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+tablelistcolumn_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+tablelistcolumn_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+tablelistcolumn_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+tablelistcolumn_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+tablelistcolumn_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+tablelistcolumn_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Table list item info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+tablelistcolumn_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_menutableinfoid .CaptionTD', formid).after('<td id="dispmenutableinfoid">'+row_id+'</td>');
					$('#tr_menutableinfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Table list item info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+tablelistcolumn_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+tablelistcolumn_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Table list item info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+tablelistcolumn_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ tablelistcolumn_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_tablelistcolumn_info #id').val('');
					}else{
...
//	$('#'+tablelistcolumn_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+tablelistcolumn_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');

	//サブグリッド設定
	$('#'+tablelistcolumn_info_tabId).jqGrid('setGridParam',
			{subGridRowExpanded: function(subgrid_id, row_id) {
				tablerowstyle_info(subgrid_id, row_id);
			}}
	);	//サブグリッドend
