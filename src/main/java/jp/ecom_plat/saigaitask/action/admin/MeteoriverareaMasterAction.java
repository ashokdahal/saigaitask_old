...
import jp.ecom_plat.saigaitask.form.db.MeteoriverareaMasterForm;
...
@RequestMapping("/admin/meteoriverareaMaster")
public class MeteoriverareaMasterAction extends AbstractAdminAction<MeteoriverareaMaster> {
	protected MeteoriverareaMasterForm meteoriverareaMasterForm;

	/** サービスクラス */
	@Resource
	protected MeteoriverareaMasterService meteoriverareaMasterService;
	...
	public String index() {
		return "/admin/meteoriverareaMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = meteoriverareaMasterService.getCount(conditions);

			// 一覧内容取得
			List<MeteoriverareaMaster> list = new ArrayList<MeteoriverareaMaster>();
			if(count > 0){
				list = meteoriverareaMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = meteoriverareaMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteoriverareaMasterForm meteoriverareaMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteoriverareaMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteoriverareaMaster.class, meteoriverareaMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteoriverareaMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteoriverareaMaster meteoriverareaMaster = meteoriverareaMasterService.findById(Integer.parseInt(meteoriverareaMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteoriverareaMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteoriverareaMaster.class, meteoriverareaMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteoriverareaMaster.class, meteoriverareaMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteoriverareaMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteoriverareaMasterForm.id));
				int count = meteoriverareaMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteoriverareaMasterService.findById(Integer.parseInt(meteoriverareaMasterForm.id));
					//削除実行
					try{
						meteoriverareaMasterService.delete(entity);
