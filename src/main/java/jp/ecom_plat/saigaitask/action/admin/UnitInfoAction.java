...
import jp.ecom_plat.saigaitask.form.db.UnitInfoForm;
...
@RequestMapping("/admin/unitInfo")
public class UnitInfoAction extends AbstractAdminAction<UnitInfo> {
	protected UnitInfoForm unitInfoForm;
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.unitInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			if (!loginDataDto.isAdmin()) {
				//検索条件をセット
				conditions.put(Names.unitInfo().id().toString(), loginDataDto.getUnitid());
			}
			...
			// 一覧件数取得
			int count = unitInfoService.getCount(conditions);

			// 一覧内容取得
			List<UnitInfo> list = new ArrayList<UnitInfo>();
			if(count > 0){
				list = unitInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = unitInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute UnitInfoForm unitInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(UnitInfo.class, unitInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					UnitInfo unitInfo = unitInfoService.findById(Long.decode(unitInfoForm.id));
					...
					if(unitInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, UnitInfo.class, unitInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(UnitInfo.class, unitInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							...
							if(StringUtils.isNotEmpty(entity.password)){
								...
								updateExcludesList.add(Names.unitInfo().password());
							}
							...
							if(! StringUtils.isEmpty(apikeyModify)) {
								...
								case APIKEY_CLEAR:
									...
									break;
								default:
									updateExcludesList.add(Names.unitInfo().apikey());
									break;
								}
							}else {
								updateExcludesList.add(Names.unitInfo().apikey());
							...
							//更新実行
							unitInfoService.update(entity, updateExcludesAry);
							json.put(JQG_EDIT_ENTITY, StringUtil.jsonForCyclicalReferences(entity));
							responseService.responseJson(json);
							//ログイン情報を更新
							// 課でログインしている場合のみ
							if(loginDataDto.getUnitInfo() != null){
								if(loginDataDto.getUnitInfo().id.equals(entity.id)){
									loginDataDto.setUnitInfo(entity);
								}
							}
						}else{
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(unitInfoForm.id));
				int count = unitInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = unitInfoService.findById(Long.decode(unitInfoForm.id));
					//削除実行
					try{
						unitInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	...
	/**
	 * 自治体IDに紐づく課情報データ(id, name)を取得する関数
	 * @return null
	 * @throws ServiceException
	 */
