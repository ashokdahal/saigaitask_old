...
import jp.ecom_plat.saigaitask.form.db.FacebookpostInfoForm;
import jp.ecom_plat.saigaitask.service.TableService;
import jp.ecom_plat.saigaitask.service.db.FacebookpostInfoService;
...
/**
 * Facebook投稿先情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/facebookpostInfo")
public class FacebookpostInfoAction extends AbstractAdminAction<FacebookpostInfo> {
	protected FacebookpostInfoForm facebookpostInfoForm;

	/** サービスクラス */
	@Resource
	protected FacebookpostInfoService facebookpostInfoService;
	...
	public String index() {
		return "/admin/facebookpostInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.facebookpostInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = facebookpostInfoService.getCount(conditions);

			// 一覧内容取得
			List<FacebookpostInfo> list = new ArrayList<FacebookpostInfo>();
			if(count > 0){
				list = facebookpostInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = facebookpostInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute FacebookpostInfoForm facebookpostInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			FacebookpostInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(FacebookpostInfo.class, facebookpostInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				facebookpostInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					FacebookpostInfo facebookpostInfo = facebookpostInfoService.findById(Long.decode(facebookpostInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(facebookpostInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, FacebookpostInfo.class, facebookpostInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(FacebookpostInfo.class, facebookpostInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							facebookpostInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(facebookpostInfoForm.id));
				int count = facebookpostInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = facebookpostInfoService.findById(Long.decode(facebookpostInfoForm.id));
					//削除実行
					try{
						facebookpostInfoService.delete(entity);
