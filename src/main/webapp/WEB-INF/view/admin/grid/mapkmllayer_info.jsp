...
function mapkmllayer_info(subgrid_id, row_id){
	var tabName = 'mapkmllayer_info';
	var mapkmllayer_info_gridId = 'main';
	if(subgrid_id != null){
		mapkmllayer_info_gridId = subgrid_id;
	}
	var mapkmllayer_info_tabId = tabName;
	if(row_id != null){
		mapkmllayer_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'menu_info', 'menuinfoid');

	var mapkmllayer_info_pagerId = mapkmllayer_info_tabId+'Nav';
	$('#'+mapkmllayer_info_gridId).append('<table id="'+mapkmllayer_info_tabId+'" class="scroll"></table><div id="'+mapkmllayer_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+mapkmllayer_info_tabId).jqGrid({
		caption: '<%=lang.__("Map KML layer info")%>',
		mtype: 'POST',
...
//			search:true,
//			searchrules:{required:true},
//			formoptions:{elmprefix:'', elmsuffix:'<a id="callecom_mapkmllayerinfo" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" href="javascript:void(0)">検索<span class="ui-icon ui-icon-search"></span></a>', label:'子レイヤID(*)'},
//			hidden:false},
			{name:'visible',
			width:'150',
			align:'left',
...
//			search:true,
//			searchrules:{required:true},
//			formoptions:{elmprefix:'', elmsuffix:'', label:'凡例折りたたみ(*)'},
//			hidden:false},
			{name:'searchable',
				width:'150',
				align:'left',
		...
		pager: mapkmllayer_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+mapkmllayer_info_tabId).jqGrid('setColProp', 'menuinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+mapkmllayer_info_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+mapkmllayer_info_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+mapkmllayer_info_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+mapkmllayer_info_tabId).jqGrid('navGrid',
			'#'+mapkmllayer_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+mapkmllayer_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+mapkmllayer_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Map KML layer info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					var rowid = $('#'+mapkmllayer_info_tabId).jqGrid('getGridParam', 'selrow');
					var s = "";
					var rowdata = $('#'+mapkmllayer_info_tabId).jqGrid('getRowData', rowid);
					s += '&menuinfoid='+rowdata.menuinfoid;
					$('#'+mapkmllayer_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+mapkmllayer_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+mapkmllayer_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+mapkmllayer_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+mapkmllayer_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+mapkmllayer_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+mapkmllayer_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Map KML layer info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+mapkmllayer_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_menuinfoid .CaptionTD', formid).after('<td id="dispmenuinfoid">'+row_id+'</td>');
					$('#tr_menuinfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Map KML layer info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+mapkmllayer_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+mapkmllayer_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Map KML layer info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+mapkmllayer_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ mapkmllayer_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_mapkmllayer_info #id').val('');
					}else{
...
//	$('#'+mapkmllayer_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+mapkmllayer_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
