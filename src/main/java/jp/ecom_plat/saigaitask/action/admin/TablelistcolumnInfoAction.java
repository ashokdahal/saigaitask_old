...
import jp.ecom_plat.saigaitask.form.db.TablelistcolumnInfoForm;
...
/**
 * テーブルリスト項目情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/tablelistcolumnInfo")
public class TablelistcolumnInfoAction extends AbstractAdminAction<TablelistcolumnInfo> {
	protected TablelistcolumnInfoForm tablelistcolumnInfoForm;
	...
	public String index() {
		return "/admin/tablelistcolumnInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = tablelistcolumnInfoService.getCount(conditions);

			// 一覧内容取得
			List<TablelistcolumnInfo> list = new ArrayList<TablelistcolumnInfo>();
			if(count > 0){
				list = tablelistcolumnInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = tablelistcolumnInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TablelistcolumnInfoForm tablelistcolumnInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TablelistcolumnInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TablelistcolumnInfo.class, tablelistcolumnInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				tablelistcolumnInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TablelistcolumnInfo tablelistcolumnInfo = tablelistcolumnInfoService.findById(Long.decode(tablelistcolumnInfoForm.id));
					...
					if(tablelistcolumnInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TablelistcolumnInfo.class, tablelistcolumnInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TablelistcolumnInfo.class, tablelistcolumnInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							tablelistcolumnInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(tablelistcolumnInfoForm.id));
				int count = tablelistcolumnInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = tablelistcolumnInfoService.findById(Long.decode(tablelistcolumnInfoForm.id));
					//削除実行
					try{
						tablelistcolumnInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/jqgridecommap/{parentId}")
	@ResponseBody
	public String jqgridecommap(
		...
		try{
			...
			//メニューテーブル情報のテーブルIDに紐付くテーブルマスタ情報のレイヤIDを取得
//			String parentId = (String)request.getParameter("parentId");
			conditions.put(Names.menutableInfo().id().toString(), parentId);
			List<MenutableInfo> menutableInfoList = menutableInfoService.findByCondition(conditions, null, null, null, null);
			Long mapMasterInfoId = menutableInfoList.get(0).tablemasterInfo.mapmasterinfoid;
			String layerId = menutableInfoList.get(0).tablemasterInfo.layerid;
			String layerName = menutableInfoList.get(0).tablemasterInfo.name;
			String tableName = menutableInfoList.get(0).tablemasterInfo.tablename;
			...
			if (mapMasterInfoId.longValue() != 0) {
				j = ecommapDataGetService.getAttributeList(loginDataDto.getEcomUser(), layerId);
			}
