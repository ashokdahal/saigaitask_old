...
import jp.ecom_plat.saigaitask.form.db.AutocompleteInfoForm;
...
@RequestMapping("/admin/autocompleteInfo")
public class AutocompleteInfoAction extends AbstractAdminAction<AutocompleteInfo> {
	protected AutocompleteInfoForm autocompleteInfoForm;
	...
	public String index() {
		return "/admin/autocompleteInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.autocompleteInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = autocompleteInfoService.getCount(conditions);

			// 一覧内容取得
			List<AutocompleteInfo> list = new ArrayList<AutocompleteInfo>();
			if(count > 0){
				list = autocompleteInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute AutocompleteInfoForm autocompleteInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			AutocompleteInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(AutocompleteInfo.class, autocompleteInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				autocompleteInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					AutocompleteInfo autocompleteInfo = autocompleteInfoService.findById(Long.decode(autocompleteInfoForm.id));
					...
					if(autocompleteInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, AutocompleteInfo.class, autocompleteInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(AutocompleteInfo.class, autocompleteInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							autocompleteInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(autocompleteInfoForm.id));
				int count = autocompleteInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = autocompleteInfoService.findById(Long.decode(autocompleteInfoForm.id));
					//削除実行
					try{
						autocompleteInfoService.delete(entity);
