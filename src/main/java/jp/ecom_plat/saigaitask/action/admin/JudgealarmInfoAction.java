...
import jp.ecom_plat.saigaitask.form.db.JudgealarmInfoForm;
...
/**
 * データ判定アラーム情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/judgealarmInfo")
public class JudgealarmInfoAction extends AbstractAdminAction<JudgealarmInfo> {
	protected JudgealarmInfoForm judgealarmInfoForm;
	...
	public String index() {
		return "/admin/judgealarmInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = judgealarmInfoService.getCount(conditions);

			// 一覧内容取得
			List<JudgealarmInfo> list = new ArrayList<JudgealarmInfo>();
			if(count > 0){
				list = judgealarmInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JudgealarmInfoForm judgealarmInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			JudgealarmInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JudgealarmInfo.class, judgealarmInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				judgealarmInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JudgealarmInfo judgealarmInfo = judgealarmInfoService.findById(Long.decode(judgealarmInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(judgealarmInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JudgealarmInfo.class, judgealarmInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JudgealarmInfo.class, judgealarmInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							judgealarmInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(judgealarmInfoForm.id));
				int count = judgealarmInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = judgealarmInfoService.findById(Long.decode(judgealarmInfoForm.id));
					//削除実行
					try{
						judgealarmInfoService.delete(entity);
