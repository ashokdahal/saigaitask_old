...
import jp.ecom_plat.saigaitask.form.db.PagebuttonMasterForm;
...
@RequestMapping("/admin/pagebuttonMaster")
public class PagebuttonMasterAction extends AbstractAdminAction<PagebuttonMaster> {
	protected PagebuttonMasterForm pagebuttonMasterForm;
	...
	public String index() {
		return "/admin/pagebuttonMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = pagebuttonMasterService.getCount(conditions);

			// 一覧内容取得
			List<PagebuttonMaster> list = new ArrayList<PagebuttonMaster>();
			if(count > 0){
				list = pagebuttonMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = pagebuttonMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute PagebuttonMasterForm pagebuttonMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			PagebuttonMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(PagebuttonMaster.class, pagebuttonMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				pagebuttonMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					PagebuttonMaster pagebuttonMaster = pagebuttonMasterService.findById(Integer.parseInt(pagebuttonMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(pagebuttonMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, PagebuttonMaster.class, pagebuttonMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(PagebuttonMaster.class, pagebuttonMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							pagebuttonMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(pagebuttonMasterForm.id));
				int count = pagebuttonMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = pagebuttonMasterService.findById(Integer.parseInt(pagebuttonMasterForm.id));
					//削除実行
					try{
					pagebuttonMasterService.delete(entity);
