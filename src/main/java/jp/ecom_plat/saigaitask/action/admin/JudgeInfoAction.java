...
import jp.ecom_plat.saigaitask.form.db.JudgeInfoForm;
...
@RequestMapping("/admin/judgeInfo")
public class JudgeInfoAction extends AbstractAdminAction<JudgeInfo> {
	protected JudgeInfoForm judgeInfoForm;
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = judgeInfoService.getCount(conditions);

			// 一覧内容取得
			List<JudgeInfo> list = new ArrayList<JudgeInfo>();
			if(count > 0){
				list = judgeInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JudgeInfoForm judgeInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JudgeInfo.class, judgeInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				judgeInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JudgeInfo judgeInfo = judgeInfoService.findById(Long.decode(judgeInfoForm.id));
					...
					if(judgeInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JudgeInfo.class, judgeInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JudgeInfo.class, judgeInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							judgeInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(judgeInfoForm.id));
				int count = judgeInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = judgeInfoService.findById(Long.decode(judgeInfoForm.id));
					//削除実行
					try{
						judgeInfoService.delete(entity);
