...
import jp.ecom_plat.saigaitask.form.db.ObservatoryriverlayerInfoForm;
...
/**
 * 河川水位観測地点レイヤ情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/observatoryriverlayerInfo")
public class ObservatoryriverlayerInfoAction extends AbstractAdminAction<ObservatoryriverlayerInfo> {
	protected ObservatoryriverlayerInfoForm observatoryriverlayerInfoForm;
	...
	public String index() {
		return "/admin/observatoryriverlayerInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.observatoryriverlayerInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = observatoryriverlayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<ObservatoryriverlayerInfo> list = new ArrayList<ObservatoryriverlayerInfo>();
			if(count > 0){
				list = observatoryriverlayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ObservatoryriverlayerInfoForm observatoryriverlayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ObservatoryriverlayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ObservatoryriverlayerInfo.class, observatoryriverlayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				observatoryriverlayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ObservatoryriverlayerInfo observatoryriverlayerInfo = observatoryriverlayerInfoService.findById(Long.decode(observatoryriverlayerInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(observatoryriverlayerInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ObservatoryriverlayerInfo.class, observatoryriverlayerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ObservatoryriverlayerInfo.class, observatoryriverlayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							observatoryriverlayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(observatoryriverlayerInfoForm.id));
				int count = observatoryriverlayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = observatoryriverlayerInfoService.findById(Long.decode(observatoryriverlayerInfoForm.id));
					//削除実行
					try{
						observatoryriverlayerInfoService.delete(entity);
