...
import jp.ecom_plat.saigaitask.form.db.MeteotsunamiareaMasterForm;
...
@RequestMapping("/admin/meteotsunamiareaMaster")
public class MeteotsunamiareaMasterAction extends AbstractAdminAction<MeteotsunamiareaMaster> {
	protected MeteotsunamiareaMasterForm meteotsunamiareaMasterForm;

	/** サービスクラス */
	@Resource
	protected MeteotsunamiareaMasterService meteotsunamiareaMasterService;
	...
	public String index() {
		return "/admin/meteotsunamiareaMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = meteotsunamiareaMasterService.getCount(conditions);

			// 一覧内容取得
			List<MeteotsunamiareaMaster> list = new ArrayList<MeteotsunamiareaMaster>();
			if(count > 0){
				list = meteotsunamiareaMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = meteotsunamiareaMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteotsunamiareaMasterForm meteotsunamiareaMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteotsunamiareaMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteotsunamiareaMaster.class, meteotsunamiareaMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteotsunamiareaMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteotsunamiareaMaster meteotsunamiareaMaster = meteotsunamiareaMasterService.findById(Integer.parseInt(meteotsunamiareaMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteotsunamiareaMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteotsunamiareaMaster.class, meteotsunamiareaMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteotsunamiareaMaster.class, meteotsunamiareaMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteotsunamiareaMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteotsunamiareaMasterForm.id));
				int count = meteotsunamiareaMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteotsunamiareaMasterService.findById(Integer.parseInt(meteotsunamiareaMasterForm.id));
					//削除実行
					try{
						meteotsunamiareaMasterService.delete(entity);
