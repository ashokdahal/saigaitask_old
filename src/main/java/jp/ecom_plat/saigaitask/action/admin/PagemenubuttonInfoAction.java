...
import jp.ecom_plat.saigaitask.form.db.PagemenubuttonInfoForm;
...
@RequestMapping("/admin/pagemenubuttonInfo")
public class PagemenubuttonInfoAction extends AbstractAdminAction<PagemenubuttonInfo> {
	protected PagemenubuttonInfoForm pagemenubuttonInfoForm;
	...
	public String index() {
		return "/admin/pagemenubuttonInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = pagemenubuttonInfoService.getCount(conditions);

			// 一覧内容取得
			List<PagemenubuttonInfo> list = new ArrayList<PagemenubuttonInfo>();
			if(count > 0){
				list = pagemenubuttonInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = pagemenubuttonInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute PagemenubuttonInfoForm pagemenubuttonInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			PagemenubuttonInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(PagemenubuttonInfo.class, pagemenubuttonInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				pagemenubuttonInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					PagemenubuttonInfo pagemenubuttonInfo = pagemenubuttonInfoService.findById(Long.decode(pagemenubuttonInfoForm.id));
					...
					if(pagemenubuttonInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, PagemenubuttonInfo.class, pagemenubuttonInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(PagemenubuttonInfo.class, pagemenubuttonInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							pagemenubuttonInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(pagemenubuttonInfoForm.id));
				int count = pagemenubuttonInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = pagemenubuttonInfoService.findById(Long.decode(pagemenubuttonInfoForm.id));
					//削除実行
					try{
						pagemenubuttonInfoService.delete(entity);
