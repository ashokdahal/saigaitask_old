...
import static jp.ecom_plat.saigaitask.entity.Names.observMaster;
...
public class ObservMasterService extends AbstractService<ObservMaster> {

    public ObservMaster findById(Integer id) {
	 ...
	 * @return 検索結果
	 */
	public List<ObservMaster> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = observMaster().disporder().toString();
		}
		...
		if(!observMaster().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(observMaster().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		conditions.put(observMaster().disporder().toString()+"_IS_NOT_NULL", Boolean.TRUE);
		List<ObservMaster> list = findByCondition(conditions, observMaster().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(ObservMaster entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
