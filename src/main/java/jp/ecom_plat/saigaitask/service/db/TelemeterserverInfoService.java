...
import static jp.ecom_plat.saigaitask.entity.Names.telemeterserverInfo;
...
public class TelemeterserverInfoService extends AbstractService<TelemeterserverInfo> {
	 ...
	 * @return テレメータサーバ
	 */
    public TelemeterserverInfo findById(Long id) {
     ...
     * @return テレメータサーバリスト
     */
    public List<TelemeterserverInfo> findByValid() {
    	return select().where(eq(telemeterserverInfo().valid(), true)).getResultList();
    }
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(telemeterserverInfo().localgovInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<TelemeterserverInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = telemeterserverInfo().id().toString();
		}
		...
		if(!telemeterserverInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(telemeterserverInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(TelemeterserverInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
	...
	@Override
	public DeleteCascadeResult deleteCascade(TelemeterserverInfo entity, DeleteCascadeResult result) {

		result.cascade(TelemeterofficeInfo.class, Names.telemeterofficeInfo().telemeterserverinfoid(), entity.id);
