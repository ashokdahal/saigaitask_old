...
import static jp.ecom_plat.saigaitask.entity.Names.facebookMaster;
...
/**
 * {@link FacebookMaster}のサービスクラスです。
 *
 */
...
public class FacebookMasterService extends AbstractService<FacebookMaster> {
     ...
     * @return エンティティ
     */
    public FacebookMaster findById(Integer id) {
     ...
     * @return 設定
     */
    public FacebookMaster find() {
    	return select().limit(1).getSingleResult();
    }
	 ...
	 * @return 検索結果
	 */
	public List<FacebookMaster> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = facebookMaster().id().toString();
		}
		...
		if(!facebookMaster().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(facebookMaster().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(FacebookMaster entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
