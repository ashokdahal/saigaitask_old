...
import jp.ecom_plat.saigaitask.form.db.NoticegroupaddressInfoForm;
import jp.ecom_plat.saigaitask.service.TableService;
import jp.ecom_plat.saigaitask.service.db.NoticegroupaddressInfoService;
...
/**
 * 通知グループ連絡先情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/noticegroupaddressInfo")
public class NoticegroupaddressInfoAction extends AbstractAdminAction<NoticegroupaddressInfo> {
	protected NoticegroupaddressInfoForm noticegroupaddressInfoForm;
	...
	public String index() {
		return "/admin/noticegroupaddressInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = noticegroupaddressInfoService.getCount(conditions);

			// 一覧内容取得
			List<NoticegroupaddressInfo> list = new ArrayList<NoticegroupaddressInfo>();
			if(count > 0){
				list = noticegroupaddressInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute NoticegroupaddressInfoForm noticegroupaddressInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			NoticegroupaddressInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(NoticegroupaddressInfo.class, noticegroupaddressInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				noticegroupaddressInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					NoticegroupaddressInfo noticegroupaddressInfo = noticegroupaddressInfoService.findById(Long.decode(noticegroupaddressInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(noticegroupaddressInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, NoticegroupaddressInfo.class, noticegroupaddressInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(NoticegroupaddressInfo.class, noticegroupaddressInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							noticegroupaddressInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(noticegroupaddressInfoForm.id));
				int count = noticegroupaddressInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = noticegroupaddressInfoService.findById(Long.decode(noticegroupaddressInfoForm.id));
					//削除実行
					try{
						noticegroupaddressInfoService.delete(entity);
