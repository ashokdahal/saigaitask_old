...
import static jp.ecom_plat.saigaitask.entity.Names.authorizationInfo;
...
public class AuthorizationInfoService extends AbstractService<AuthorizationInfo> {

    public AuthorizationInfo findById(Long id) {
        return select().leftOuterJoin(authorizationInfo().localgovInfo()).id(id).getSingleResult();
    }
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.leftOuterJoin(authorizationInfo().localgovInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<AuthorizationInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = authorizationInfo().id().toString();
		}
		...
		if(!authorizationInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(authorizationInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return 更新数
	*/
	public int update(AuthorizationInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
	...
	@Override
	public DeleteCascadeResult deleteCascade(AuthorizationInfo entity, DeleteCascadeResult result) {
