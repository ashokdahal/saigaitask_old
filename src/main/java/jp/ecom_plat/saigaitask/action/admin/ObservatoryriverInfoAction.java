...
import jp.ecom_plat.saigaitask.form.db.ObservatoryriverInfoForm;
...
@RequestMapping("/admin/observatoryriverInfo")
public class ObservatoryriverInfoAction extends AbstractAdminAction<ObservatoryriverInfo> {
	protected ObservatoryriverInfoForm observatoryriverInfoForm;
	...
	public String index() {
		return "/admin/observatoryriverInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.observatoryriverInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = observatoryriverInfoService.getCount(conditions);

			// 一覧内容取得
			List<ObservatoryriverInfo> list = new ArrayList<ObservatoryriverInfo>();
			if(count > 0){
				list = observatoryriverInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ObservatoryriverInfoForm observatoryriverInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ObservatoryriverInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ObservatoryriverInfo.class, observatoryriverInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				observatoryriverInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ObservatoryriverInfo observatoryriverInfo = observatoryriverInfoService.findById(Long.decode(observatoryriverInfoForm.id));
					...
					if(observatoryriverInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ObservatoryriverInfo.class, observatoryriverInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ObservatoryriverInfo.class, observatoryriverInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							observatoryriverInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(observatoryriverInfoForm.id));
				int count = observatoryriverInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = observatoryriverInfoService.findById(Long.decode(observatoryriverInfoForm.id));
					//削除実行
					try{
						observatoryriverInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
			...
			if(StringUtils.isEmpty(selectTagColumn)
				...
				long localgovinfoid = loginDataDto.getLocalgovinfoid();
				// システム管理者の場合は全件出力
				String dbServiceMethodName = "";
				Class<?>[] methodArgType = {Long.class};
				Object[] methodArg = {localgovinfoid};
				if(Constants.ADMIN_LOCALGOVINFOID == localgovinfoid){
					dbServiceMethodName = "findAll";
					methodArgType = null;
					methodArg = null;
				}else{
					dbServiceMethodName = "findByLocalgovinfoid";
				}
