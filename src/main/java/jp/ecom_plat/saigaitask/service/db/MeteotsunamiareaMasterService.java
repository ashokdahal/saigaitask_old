...
import static jp.ecom_plat.saigaitask.entity.Names.meteotsunamiareaMaster;
...
public class MeteotsunamiareaMasterService extends AbstractService<MeteotsunamiareaMaster> {

    public MeteotsunamiareaMaster findById(Integer id) {
	 ...
	 * @return 検索結果
	 */
	public List<MeteotsunamiareaMaster> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = meteotsunamiareaMaster().disporder().toString();
		}
		...
		if(!meteotsunamiareaMaster().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(meteotsunamiareaMaster().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		conditions.put(meteotsunamiareaMaster().disporder().toString()+"_IS_NOT_NULL", Boolean.TRUE);
		List<MeteotsunamiareaMaster> list = findByCondition(conditions, meteotsunamiareaMaster().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(MeteotsunamiareaMaster entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
