...
import jp.ecom_plat.saigaitask.form.db.MenutaskmenuInfoForm;
...
@RequestMapping("/admin/menutaskmenuInfo")
public class MenutaskmenuInfoAction extends AbstractAdminAction<MenutaskmenuInfo> {
	protected MenutaskmenuInfoForm menutaskmenuInfoForm;
	...
	public String index() {
		return "/admin/menutaskmenuInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = menutaskmenuInfoService.getCount(conditions);

			// 一覧内容取得
			List<MenutaskmenuInfo> list = new ArrayList<MenutaskmenuInfo>();
			if(count > 0){
				list = menutaskmenuInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = menutaskmenuInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MenutaskmenuInfoForm menutaskmenuInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MenutaskmenuInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MenutaskmenuInfo.class, menutaskmenuInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				menutaskmenuInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MenutaskmenuInfo menutaskmenuInfo = menutaskmenuInfoService.findById(Long.decode(menutaskmenuInfoForm.id));
					...
					if(menutaskmenuInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MenutaskmenuInfo.class, menutaskmenuInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MenutaskmenuInfo.class, menutaskmenuInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							menutaskmenuInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(menutaskmenuInfoForm.id));
				int count = menutaskmenuInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = menutaskmenuInfoService.findById(Long.decode(menutaskmenuInfoForm.id));
					//削除実行
					try{
						menutaskmenuInfoService.delete(entity);
	 ...
	 * @throws ServiceException
	 */
	@RequestMapping(value="/createSelectTag/{selectTagColumn}/{tableName}/{codeColumn}/{decodeColumn}/{parentId}")
	@ResponseBody
	public String createSelectTag(
				@PathVariable String selectTagColumn,
				@PathVariable String tableName,
				@PathVariable String codeColumn,
				@PathVariable String decodeColumn,
				@PathVariable String parentId) throws ServiceException {
		try{
			//パラメータ取得
...
//			String parentId = (String) request.getParameter("parentId"); //デコード値を保持しているテーブルのデコードカラム
...
//				long localgovinfoid = 23;
				long menutaskinfoid = Long.parseLong(parentId);
				MenutaskInfo menuTask = menutaskInfoService.findById(menutaskinfoid);
				menutaskinfoid = menuTask.menutasktypeinfoid;
				String dbServiceMethodName = "findByMenutasktypeinfoid";
				Class<?>[] methodArgType = {Long.class};
				Object[] methodArg = {menutaskinfoid};
				...
				if("localgov_info".equals(tableName)){
					...
					json.put(JQG_SELECT_TAG, StringUtil.json(JqGridUtil.createSelectTagList(parentList, codeColumn, decodeColumn,"note")));   //結果一覧
				}
			}
