...
import jp.ecom_plat.saigaitask.form.db.TablelistkarteInfoForm;
...
/**
 * テーブルリストカルテ情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/tablelistkarteInfo")
public class TablelistkarteInfoAction extends AbstractAdminAction<TablelistkarteInfo> {
	protected TablelistkarteInfoForm tablelistkarteInfoForm;
	...
	public String index() {
		return "/admin/tablelistkarteInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = tablelistkarteInfoService.getCount(conditions);

			// 一覧内容取得
			List<TablelistkarteInfo> list = new ArrayList<TablelistkarteInfo>();
			if(count > 0){
				list = tablelistkarteInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = tablelistkarteInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TablelistkarteInfoForm tablelistkarteInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TablelistkarteInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TablelistkarteInfo.class, tablelistkarteInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				tablelistkarteInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TablelistkarteInfo tablelistkarteInfo = tablelistkarteInfoService.findById(Long.decode(tablelistkarteInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(tablelistkarteInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TablelistkarteInfo.class, tablelistkarteInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TablelistkarteInfo.class, tablelistkarteInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							tablelistkarteInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(tablelistkarteInfoForm.id));
				int count = tablelistkarteInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = tablelistkarteInfoService.findById(Long.decode(tablelistkarteInfoForm.id));
					//削除実行
					try{
						tablelistkarteInfoService.delete(entity);
