...
import jp.ecom_plat.saigaitask.form.db.MeteorainareaMasterForm;
...
@RequestMapping("/admin/meteorainareaMaster")
public class MeteorainareaMasterAction extends AbstractAdminAction<MeteorainareaMaster> {
	protected MeteorainareaMasterForm meteorainareaMasterForm;

	/** サービスクラス */
	@Resource
	protected MeteorainareaMasterService meteorainareaMasterService;
	...
	public String index() {
		return "/admin/meteorainareaMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = meteorainareaMasterService.getCount(conditions);

			// 一覧内容取得
			List<MeteorainareaMaster> list = new ArrayList<MeteorainareaMaster>();
			if(count > 0){
				list = meteorainareaMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = meteorainareaMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteorainareaMasterForm meteorainareaMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteorainareaMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteorainareaMaster.class, meteorainareaMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteorainareaMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteorainareaMaster meteorainareaMaster = meteorainareaMasterService.findById(Integer.parseInt(meteorainareaMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteorainareaMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteorainareaMaster.class, meteorainareaMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteorainareaMaster.class, meteorainareaMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteorainareaMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteorainareaMasterForm.id));
				int count = meteorainareaMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteorainareaMasterService.findById(Integer.parseInt(meteorainareaMasterForm.id));
					//削除実行
					try{
						meteorainareaMasterService.delete(entity);
