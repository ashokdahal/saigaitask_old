...
import jp.ecom_plat.saigaitask.form.db.FacebookInfoForm;
...
@RequestMapping("/admin/facebookInfo")
public class FacebookInfoAction extends AbstractAdminAction<FacebookInfo> {
	protected FacebookInfoForm facebookInfoForm;
	...
	public String index() {
		return "/admin/facebookInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.facebookInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = facebookInfoService.getCount(conditions);

			// 一覧内容取得
			List<FacebookInfo> list = new ArrayList<FacebookInfo>();
			if(count > 0){
				list = facebookInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute FacebookInfoForm facebookInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(FacebookInfo.class, facebookInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				facebookInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					FacebookInfo facebookInfo = facebookInfoService.findById(Long.decode(facebookInfoForm.id));
					...
					if(facebookInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, FacebookInfo.class, facebookInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(FacebookInfo.class, facebookInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							facebookInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(facebookInfoForm.id));
				int count = facebookInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = facebookInfoService.findById(Long.decode(facebookInfoForm.id));
					//削除実行
					try{
						facebookInfoService.delete(entity);
