...
import jp.ecom_plat.saigaitask.form.db.StationclassInfoForm;
...
@RequestMapping("/admin/stationclassInfo")
public class StationclassInfoAction extends AbstractAdminAction<StationclassInfo> {
	protected StationclassInfoForm stationclassInfoForm;

	/** サービスクラス */
	@Resource
	protected StationclassInfoService stationclassInfoService;
	...
	public String index() {
		return "/admin/stationclassInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.stationclassInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = stationclassInfoService.getCount(conditions);

			// 一覧内容取得
			List<StationclassInfo> list = new ArrayList<StationclassInfo>();
			if(count > 0){
				list = stationclassInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = stationclassInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute StationclassInfoForm stationclassInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			StationclassInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(StationclassInfo.class, stationclassInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				stationclassInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					StationclassInfo stationclassInfo = stationclassInfoService.findById(Long.decode(stationclassInfoForm.id));
					...
					if(stationclassInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, StationclassInfo.class, stationclassInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(StationclassInfo.class, stationclassInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							stationclassInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(stationclassInfoForm.id));
				int count = stationclassInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = stationclassInfoService.findById(Long.decode(stationclassInfoForm.id));
					//削除実行
					try{
						stationclassInfoService.delete(entity);
