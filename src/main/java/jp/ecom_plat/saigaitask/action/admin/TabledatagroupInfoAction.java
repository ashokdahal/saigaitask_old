...
//public class TabledatagroupInfoAction extends AbstractAdminAction<TabledatagroupInfo> {
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/tabledatagroupInfo")
    public class TabledatagroupInfoAction{
//	protected TabledatagroupInfoForm tabledatagroupInfoForm;
//
//	/** サービスクラス */
//	@Resource
//	protected TabledatagroupInfoExService tabledatagroupInfoExService;
//
//	/** サービスクラス */
//	@Resource
...
//
	...
	public String index() {
		return "/admin/tabledatagroupInfo/index";
	}
//
//	/**
...
//
//		// 一覧件数取得
//		int count = tabledatagroupInfoExService.getCount(conditions);
//
//		// 一覧内容取得
//		List<TabledatagroupInfo> list = new ArrayList<TabledatagroupInfo>();
//		if(count > 0){
//			list = tabledatagroupInfoExService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
//		}
//
//		//ユーザーデータ取得
...
//
//		//操作種別に応じたアクションを実行
//		TabledatagroupInfo entity;
//		//登録
//		if("add".equals(oper)) {
//			entity = Beans.createAndCopy(TabledatagroupInfo.class, tabledatagroupInfoForm).dateConverter("yyyy-MM-dd").execute();
//			//登録実行
//			tabledatagroupInfoExService.insert(entity);
//			//採番したキー値を返却
//			JSONObject json = new JSONObject();
...
//			}else{
//				//現時点の編集対象データを取得
//				TabledatagroupInfo tabledatagroupInfo = tabledatagroupInfoExService.findById(Integer.parseInt(tabledatagroupInfoForm.id));
//
//				//結果返却用JSON
//				JSONObject json = new JSONObject();
//				if(tabledatagroupInfo != null){
//					//編集対象が存在する場合、実行。
//					//現時点の編集対象データが他者により変更されているかを判定
//					if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TabledatagroupInfo.class, tabledatagroupInfo)){
//						//変更されていない場合、更新
//						entity = Beans.createAndCopy(TabledatagroupInfo.class, tabledatagroupInfoForm).dateConverter("yyyy-MM-dd").execute();
//						PropertyName<?>[] updateExcludesAry = null;
//						List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
//						//更新実行
//						tabledatagroupInfoExService.update(entity, updateExcludesAry);
...
//			//検索条件マップ
//			BeanMap conditions = new BeanMap();
//			conditions.put("id", Integer.parseInt(tabledatagroupInfoForm.id));
//			int count = tabledatagroupInfoExService.getCount(conditions);
//			if(count==1){
//				//削除対象が存在する場合、削除実行。
//				entity = tabledatagroupInfoExService.findById(Integer.parseInt(tabledatagroupInfoForm.id));
//				//削除実行
//				tabledatagroupInfoExService.delete(entity);
//			}else{
