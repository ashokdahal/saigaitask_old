...
import jp.ecom_plat.saigaitask.form.db.TimelinetableInfoForm;
...
@RequestMapping("/admin/timelinetableInfo")
public class TimelinetableInfoAction extends AbstractAdminAction<TimelinetableInfo> {
	protected TimelinetableInfoForm timelinetableInfoForm;
	...
	public String index() {
		return "/admin/timelinetableInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = timelinetableInfoService.getCount(conditions);

			// 一覧内容取得
			List<TimelinetableInfo> list = new ArrayList<TimelinetableInfo>();
			if(count > 0){
				list = timelinetableInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TimelinetableInfoForm timelinetableInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TimelinetableInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TimelinetableInfo.class, timelinetableInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				timelinetableInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TimelinetableInfo timelinetableInfo = timelinetableInfoService.findById(Long.decode(timelinetableInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(timelinetableInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TimelinetableInfo.class, timelinetableInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TimelinetableInfo.class, timelinetableInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							timelinetableInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(timelinetableInfoForm.id));
				int count = timelinetableInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = timelinetableInfoService.findById(Long.decode(timelinetableInfoForm.id));
					//削除実行
					try{
						timelinetableInfoService.delete(entity);
