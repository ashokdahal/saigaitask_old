...
import jp.ecom_plat.saigaitask.form.db.DecisionsupportInfoForm;
...
/**
 * 意思決定支援レイヤ情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/decisionsupportInfo")
public class DecisionsupportInfoAction extends AbstractAdminAction<DecisionsupportInfo> {
	protected DecisionsupportInfoForm decisionsupportInfoForm;
	...
	public String index() {
		return "/admin/decisionsupportInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.decisionsupportInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = decisionsupportInfoService.getCount(conditions);

			// 一覧内容取得
			List<DecisionsupportInfo> list = new ArrayList<DecisionsupportInfo>();
			if(count > 0){
				list = decisionsupportInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = decisionsupportInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute DecisionsupportInfoForm decisionsupportInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			DecisionsupportInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(DecisionsupportInfo.class, decisionsupportInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				decisionsupportInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					DecisionsupportInfo decisionsupportInfo = decisionsupportInfoService.findById(Long.decode(decisionsupportInfoForm.id));
					...
					if(decisionsupportInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, DecisionsupportInfo.class, decisionsupportInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(DecisionsupportInfo.class, decisionsupportInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							decisionsupportInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(decisionsupportInfoForm.id));
				int count = decisionsupportInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = decisionsupportInfoService.findById(Long.decode(decisionsupportInfoForm.id));
					//削除実行
					decisionsupportInfoService.delete(entity);
				}else{
