...
import jp.ecom_plat.saigaitask.form.db.MenutypeMasterForm;
...
@RequestMapping("/admin/menutypeMaster")
public class MenutypeMasterAction extends AbstractAdminAction<MenutypeMaster> {
	protected MenutypeMasterForm menutypeMasterForm;
	...
	public String index() {
		return "/admin/menutypeMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = menutypeMasterService.getCount(conditions);

			// 一覧内容取得
			List<MenutypeMaster> list = new ArrayList<MenutypeMaster>();
			if(count > 0){
				list = menutypeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = menutypeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MenutypeMasterForm menutypeMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MenutypeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MenutypeMaster.class, menutypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				menutypeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MenutypeMaster menutypeMaster = menutypeMasterService.findById(Integer.parseInt(menutypeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(menutypeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MenutypeMaster.class, menutypeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MenutypeMaster.class, menutypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							menutypeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(menutypeMasterForm.id));
				int count = menutypeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = menutypeMasterService.findById(Integer.parseInt(menutypeMasterForm.id));
					//削除実行
					try{
						menutypeMasterService.delete(entity);
