...
import jp.ecom_plat.saigaitask.form.db.NoticegroupInfoForm;
...
@RequestMapping("/admin/noticegroupInfo")
public class NoticegroupInfoAction extends AbstractAdminAction<NoticegroupInfo> {
	protected NoticegroupInfoForm noticegroupInfoForm;
	...
	public String index() {
		return "/admin/noticegroupInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.noticegroupInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = noticegroupInfoService.getCount(conditions);

			// 一覧内容取得
			List<NoticegroupInfo> list = new ArrayList<NoticegroupInfo>();
			if(count > 0){
				list = noticegroupInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = noticegroupInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute NoticegroupInfoForm noticegroupInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			NoticegroupInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(NoticegroupInfo.class, noticegroupInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				noticegroupInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					NoticegroupInfo noticegroupInfo = noticegroupInfoService.findById(Long.decode(noticegroupInfoForm.id));
					...
					if(noticegroupInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, NoticegroupInfo.class, noticegroupInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(NoticegroupInfo.class, noticegroupInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							noticegroupInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(noticegroupInfoForm.id));
				int count = noticegroupInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = noticegroupInfoService.findById(Long.decode(noticegroupInfoForm.id));
					//削除実行
					try{
						noticegroupInfoService.delete(entity);
