...
import static jp.ecom_plat.saigaitask.entity.Names.judgealarmInfo;
...
public class JudgealarmInfoService extends AbstractService<JudgealarmInfo> {
	 ...
	 * @return 判定アラーム情報のリスト
	 */
    public JudgealarmInfo findById(Long id) {
     ...
     * @return 判定アラーム情報のリスト
     */
    public List<JudgealarmInfo> findByJudgemanIdAndValid(Long jmid) {
    	return select().where(
    			and(
    				eq(judgealarmInfo().judgemaninfoid(), jmid),
    				eq(judgealarmInfo().valid(), true)
    		)).getResultList();
    }
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(judgealarmInfo().alarmmessageInfo())
			.innerJoin(judgealarmInfo().judgemanInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<JudgealarmInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = judgealarmInfo().id().toString();
		}
		...
		if(!judgealarmInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(judgealarmInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(JudgealarmInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
	...
	@Override
	public DeleteCascadeResult deleteCascade(JudgealarmInfo entity, DeleteCascadeResult result) {

		result.cascade(JudgenoticeInfo.class, Names.judgenoticeInfo().judgealarminfoid(), entity.id);
