...
<div id="global_header_admin">
	<div id="global_header_title" style="float: left;">
		...
		<a href="${f:url('/admin/setupper')}" style="text-decoration: none;"><span style="color: white;"><%=lang.__("City configuration")%></span></a>
		<c:if test="${1<fn:length(localgovSelectOptions)}">
		<select id="loginlocalgov-select" style="position: relative; top: -1px; font-size:12px;"
			onchange="location.href='${f:url('/admin/setupper/?localgovinfoid=')}'+$(this).val()">
			<c:forEach var="localgovSelectOption" items="${localgovSelectOptions}">
			<c:set var="selected" value="${localgovSelectOption.key==loginDataDto.localgovinfoid}"/>
