...
/**
 * {@link JsonimportlayerInfo}のサービスクラスです。
 * 
 */
@org.springframework.stereotype.Repository
public class JsonimportlayerInfoService extends AbstractService<JsonimportlayerInfo> {

    public JsonimportlayerInfo findById(Long id) {
        return select().leftOuterJoin(jsonimportlayerInfo().tablemasterInfo()).id(id).getSingleResult();
    }
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.leftOuterJoin(jsonimportlayerInfo().jsonimportapiInfo())
			.leftOuterJoin(jsonimportlayerInfo().tablemasterInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<JsonimportlayerInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = jsonimportlayerInfo().id().toString();
		}
		...
		if(!jsonimportlayerInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(jsonimportlayerInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return 更新数
	*/
	public int update(JsonimportlayerInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
