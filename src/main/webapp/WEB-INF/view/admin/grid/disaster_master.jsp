...
function disaster_master(subgrid_id, row_id){
	var tabName = 'disaster_master';
	var disaster_master_gridId = 'main';
	if(subgrid_id != null){
		disaster_master_gridId = subgrid_id;
	}
	var disaster_master_tabId = tabName;
	if(row_id != null){
		disaster_master_tabId += row_id;
	}else{
		row_id = '';
	}

	var disaster_master_pagerId = disaster_master_tabId+'Nav';
	$('#'+disaster_master_gridId).append('<table id="'+disaster_master_tabId+'" class="scroll"></table><div id="'+disaster_master_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+disaster_master_tabId).jqGrid({
		caption: '<%=lang.__("Disaster type master")%>',
		mtype: 'POST',
		...
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#disaster_masterNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+disaster_master_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+disaster_master_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+disaster_master_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+disaster_master_tabId).jqGrid('navGrid',
			'#'+disaster_master_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+disaster_master_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+disaster_master_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Disaster type master")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					var rowid = $('#'+disaster_master_tabId).jqGrid('getGridParam', 'selrow');
					var s = "";
					$('#'+disaster_master_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+disaster_master_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+disaster_master_tabId).jqGrid('getGridParam', 'page');
					$('#'+disaster_master_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+disaster_master_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+disaster_master_tabId).jqGrid('getGridParam', 'page');
					$('#'+disaster_master_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Disaster type master")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+disaster_master_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
				},
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Disaster type master")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+disaster_master_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+disaster_master_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Disaster type master")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+disaster_master_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ disaster_master_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_disaster_master #id').val('');
					}else{
...
//	$('#'+disaster_master_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+disaster_master_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
