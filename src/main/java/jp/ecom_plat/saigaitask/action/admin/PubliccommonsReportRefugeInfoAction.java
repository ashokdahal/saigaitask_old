...
import jp.ecom_plat.saigaitask.form.db.PubliccommonsReportRefugeInfoForm;
...
/**
 * 公共情報コモンズ避難勧告情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/publiccommonsReportRefugeInfo")
public class PubliccommonsReportRefugeInfoAction extends AbstractAdminAction<PubliccommonsReportRefugeInfo> {
	protected PubliccommonsReportRefugeInfoForm publiccommonsReportRefugeInfoForm;
	...
	public String index() {
		return "/admin/publiccommonsReportRefugeInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.publiccommonsReportRefugeInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = publiccommonsReportRefugeInfoService.getCount(conditions);

			// 一覧内容取得
			List<PubliccommonsReportRefugeInfo> list = new ArrayList<PubliccommonsReportRefugeInfo>();
			if(count > 0){
				list = publiccommonsReportRefugeInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute PubliccommonsReportRefugeInfoForm publiccommonsReportRefugeInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			PubliccommonsReportRefugeInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(PubliccommonsReportRefugeInfo.class, publiccommonsReportRefugeInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				publiccommonsReportRefugeInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					PubliccommonsReportRefugeInfo publiccommonsReportRefugeInfo = publiccommonsReportRefugeInfoService.findById(Long.decode(publiccommonsReportRefugeInfoForm.id));
					...
					if(publiccommonsReportRefugeInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, PubliccommonsReportRefugeInfo.class, publiccommonsReportRefugeInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(PubliccommonsReportRefugeInfo.class, publiccommonsReportRefugeInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							publiccommonsReportRefugeInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(publiccommonsReportRefugeInfoForm.id));
				int count = publiccommonsReportRefugeInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = publiccommonsReportRefugeInfoService.findById(Long.decode(publiccommonsReportRefugeInfoForm.id));
					//削除実行
					try{
						publiccommonsReportRefugeInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	...
	public String loadDialogTextInputColumnValue() throws ServiceException {
		...
		attrsMap.put(lang.__("Announcement status"), "attrorderstatus");
		attrsMap.put(lang.__("Number of affected households"), "attrhouseholds");
		attrsMap.put(lang.__("Number of people"), "attrpeople");
		attrsMap.put(lang.__("Announcement date and time"), "attrordertime");
