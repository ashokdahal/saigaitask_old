...
import jp.ecom_plat.saigaitask.form.db.NoticetemplatetypeMasterForm;
...
/**
 * 通知テンプレート種別マスタのアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/noticetemplatetypeMaster")
public class NoticetemplatetypeMasterAction extends AbstractAdminAction<NoticetemplatetypeMaster> {
	protected NoticetemplatetypeMasterForm noticetemplatetypeMasterForm;
	...
	public String index() {
		return "/admin/noticetemplatetypeMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = noticetemplatetypeMasterService.getCount(conditions);

			// 一覧内容取得
			List<NoticetemplatetypeMaster> list = new ArrayList<NoticetemplatetypeMaster>();
			if(count > 0){
				list = noticetemplatetypeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = noticetemplatetypeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute NoticetemplatetypeMasterForm noticetemplatetypeMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			NoticetemplatetypeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(NoticetemplatetypeMaster.class, noticetemplatetypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				noticetemplatetypeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					NoticetemplatetypeMaster noticetemplatetypeMaster = noticetemplatetypeMasterService.findById(Integer.parseInt(noticetemplatetypeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(noticetemplatetypeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, NoticetemplatetypeMaster.class, noticetemplatetypeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(NoticetemplatetypeMaster.class, noticetemplatetypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							noticetemplatetypeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(noticetemplatetypeMasterForm.id));
				int count = noticetemplatetypeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = noticetemplatetypeMasterService.findById(Integer.parseInt(noticetemplatetypeMasterForm.id));
					//削除実行
					try{
						noticetemplatetypeMasterService.delete(entity);
