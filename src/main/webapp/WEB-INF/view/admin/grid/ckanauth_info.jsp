...
function ckanauth_info(subgrid_id, row_id){
	var tabName = 'ckanauth_info';
	var ckanauth_info_gridId = 'main';
	if(subgrid_id != null){
		ckanauth_info_gridId = subgrid_id;
	}
	var ckanauth_info_tabId = tabName;
	if(row_id != null){
		ckanauth_info_tabId += row_id;
	}else{
		row_id = '';
	}

	var ckanauth_info_pagerId = ckanauth_info_tabId+'Nav';
	$('#'+ckanauth_info_gridId).append('<table id="'+ckanauth_info_tabId+'" class="scroll"></table><div id="'+ckanauth_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+ckanauth_info_tabId).jqGrid({
		caption: '<%=lang.__("CKAN authorization info")%>',
		mtype: 'POST',
		...
		colNames: [
			...
			'<%=lang.__("CKAN auth key")%>',
			'<%=lang.__("Notes")%>',
			'<%=lang.__("CKAN auth key for training")%>',
		],
		colModel:[
			{name:'id',
				width:'60',
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+ckanauth_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+ckanauth_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.localgovinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'ckanauth_info', 'localgov_info');
						}
				},
			...
			{name:'authkey',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("CKAN auth key")%>(*)'},
				hidden:false},
			{name:'note',
				width:'150',
				align:'left',
			...
			{name:'trainingauthkey',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("CKAN auth key for training")%>(*)'},
				hidden:false},
		],
		ajaxGridOptions :{
		...
		beforeProcessing:function(data){
			...
			$('#'+ckanauth_info_tabId).jqGrid('setColProp', 'localgovinfoid', {editoptions:{value:str}});
		},
		loadComplete : function (data) {
			//ページボタンセンター配置対策
			$("#ckanauth_infoNav_left").css("width","");
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+ckanauth_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+ckanauth_info_tabId).jqGrid('navGrid',
			'#'+ckanauth_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+ckanauth_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+ckanauth_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - CKAN authorization info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					var rowdata = $('#'+ckanauth_info_tabId).jqGrid('getRowData', rowid);
					$('#'+ckanauth_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+ckanauth_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+ckanauth_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+ckanauth_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});
					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+ckanauth_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+ckanauth_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+ckanauth_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:' ' + '<%=lang.__("Add - CKAN authorization info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+ckanauth_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - CKAN authorization info")%>',
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+ckanauth_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - CKAN authorization info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
									...
									afterSubmit: function(){	//フォームサブミット後、グリッドデータをサーバから再取得する設定。
										...
										//$('#'+ckanauth_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
										$('#'+ckanauth_info_tabId).jqGrid('setGridParam',{datatype:'json'});
										return [true,''];
									},
									ajaxEditOptions :{
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+ckanauth_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ ckanauth_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_ckanauth_info #id').val('');
					}else{
...
//	$('#'+ckanauth_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+ckanauth_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
}
