...
import jp.ecom_plat.saigaitask.form.db.ObservatorydamlayerInfoForm;
...
@RequestMapping("/admin/observatorydamlayerInfo")
public class ObservatorydamlayerInfoAction extends AbstractAdminAction<ObservatorydamlayerInfo> {
	protected ObservatorydamlayerInfoForm observatorydamlayerInfoForm;
	...
	public String index() {
		return "/admin/observatorydamlayerInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.observatorydamlayerInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = observatorydamlayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<ObservatorydamlayerInfo> list = new ArrayList<ObservatorydamlayerInfo>();
			if(count > 0){
				list = observatorydamlayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ObservatorydamlayerInfoForm observatorydamlayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ObservatorydamlayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ObservatorydamlayerInfo.class, observatorydamlayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				observatorydamlayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ObservatorydamlayerInfo observatorydamlayerInfo = observatorydamlayerInfoService.findById(Long.decode(observatorydamlayerInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(observatorydamlayerInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ObservatorydamlayerInfo.class, observatorydamlayerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ObservatorydamlayerInfo.class, observatorydamlayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							observatorydamlayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(observatorydamlayerInfoForm.id));
				int count = observatorydamlayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = observatorydamlayerInfoService.findById(Long.decode(observatorydamlayerInfoForm.id));
					//削除実行
					try{
						observatorydamlayerInfoService.delete(entity);
