...
function externalmapdata_info(subgrid_id, row_id){
	var tabName = 'externalmapdata_info';
	var externalmapdata_info_gridId = 'main';
	if(subgrid_id != null){
		externalmapdata_info_gridId = subgrid_id;
	}
	var externalmapdata_info_tabId = tabName;
	if(row_id != null){
		externalmapdata_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'menu_info', 'menuinfoid');

	var externalmapdata_info_pagerId = externalmapdata_info_tabId+'Nav';
	$('#'+externalmapdata_info_gridId).append('<table id="'+externalmapdata_info_tabId+'" class="scroll"></table><div id="'+externalmapdata_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+externalmapdata_info_tabId).jqGrid({
		caption: '<%=lang.__("External map data info")%>',
		mtype: 'POST',
		...
		colNames: [
			'ID',
			'<%=lang.__("Menu ID")%>',
			'<%=lang.__("Identifier of metadata")%>',
			'<%=lang.__("Data name")%>',
			'<%=lang.__("Filter<!--2-->")%>',
		...
		colModel:[
			...
			{name:'metadataid',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Identifier of metadata(*)")%>'},
				hidden:false},
			{name:'name',
				width:'150',
				align:'left',
			...
			{name:'filterid',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Filter<!--2-->")%>'},
				hidden:false},
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+externalmapdata_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+externalmapdata_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.authorizationinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'externalmapdata_info', 'authorization_info');
						}
				},
				editrules:{required:false, number:false, integer:false, email:false, edithidden:false},
				sortable:true,
		...
		pager: externalmapdata_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+externalmapdata_info_tabId).jqGrid('setColProp', 'menuinfoid', {editoptions:{value:str}});
			...
			$('#'+externalmapdata_info_tabId).jqGrid('setColProp', 'authorizationinfoid', {editoptions:{value:str}});
		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+externalmapdata_info_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+externalmapdata_info_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+externalmapdata_info_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+externalmapdata_info_tabId).jqGrid('navGrid',
			'#'+externalmapdata_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+externalmapdata_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+externalmapdata_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - External map data info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+externalmapdata_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+externalmapdata_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+externalmapdata_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+externalmapdata_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+externalmapdata_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+externalmapdata_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+externalmapdata_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - External map data info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+externalmapdata_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_menuinfoid .CaptionTD', formid).after('<td id="dispmenuinfoid">'+row_id+'</td>');
					$('#tr_menuinfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - External map data info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+externalmapdata_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+externalmapdata_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - External map data info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+externalmapdata_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ externalmapdata_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_externalmapdata_info #id').val('');
					}else{
...
//	$('#'+externalmapdata_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+externalmapdata_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
