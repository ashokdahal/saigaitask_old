...
function externaltabledata_info(subgrid_id, row_id){
	var tabName = 'externaltabledata_info';
	var externaltabledata_info_gridId = 'main';
	if(subgrid_id != null){
		externaltabledata_info_gridId = subgrid_id;
	}
	var externaltabledata_info_tabId = tabName;
	if(row_id != null){
		externaltabledata_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'menu_info', 'menuinfoid');

	var externaltabledata_info_pagerId = externaltabledata_info_tabId+'Nav';
	$('#'+externaltabledata_info_gridId).append('<table id="'+externaltabledata_info_tabId+'" class="scroll"></table><div id="'+externaltabledata_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+externaltabledata_info_tabId).jqGrid({
		caption: '<%=lang.__("External list data info")%>',
		mtype: 'POST',
				...
				editoptions:{value:'',
					...
					readonly:false
						...
						buildSelect: function (data) {
							...
							var rowid = $('#'+externaltabledata_info_tabId).jqGrid('getGridParam', 'selrow');
							var selectedId = null;
							if(rowid != null){
								//編集の場合
								var rowdata = $('#'+externaltabledata_info_tabId).jqGrid('getRowData', rowid);
								selectedId = rowdata.authorizationinfoid;
							}else{
							}
							return createSelectTag(rowid, selectTag, selectedId, 'externaltabledata_info', 'authorization_info');
						}
				},
				editrules:{required:false, number:false, integer:false, email:false, edithidden:false},
				sortable:true,
		...
		pager: externaltabledata_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+externaltabledata_info_tabId).jqGrid('setColProp', 'menuinfoid', {editoptions:{value:str}});
			...
			$('#'+externaltabledata_info_tabId).jqGrid('setColProp', 'authorizationinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+externaltabledata_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+externaltabledata_info_tabId).jqGrid('navGrid',
			'#'+externaltabledata_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+externaltabledata_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+externaltabledata_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - External list data info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+externaltabledata_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+externaltabledata_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+externaltabledata_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+externaltabledata_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+externaltabledata_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+externaltabledata_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+externaltabledata_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - External list data info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+externaltabledata_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_menuinfoid .CaptionTD', formid).after('<td id="dispmenuinfoid">'+row_id+'</td>');
					$('#tr_menuinfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - External list data info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+externaltabledata_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
		...
		//コピーボタン
			...
			'#'+externaltabledata_info_pagerId,
			{
				id:'copy_externaltabledata_info',
				caption:'',
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+externaltabledata_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - External list data info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+externaltabledata_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ externaltabledata_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_externaltabledata_info #id').val('');
					}else{
...
//	$('#'+externaltabledata_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+externaltabledata_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
