...
/**
 * 外部リスト集計項目情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/externallistsummarycolumnInfo")
public class ExternallistsummarycolumnInfoAction extends AbstractAdminAction<SummarylistcolumnInfo> {
	protected SummarylistcolumnInfoForm externallistsummarycolumnInfoForm;

	/** サービスクラス */
	@Resource
	protected SummarylistcolumnInfoService externallistsummarycolumnInfoService;
	...
	public String index() {
		return "/admin/externallistsummarycolumnInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = externallistsummarycolumnInfoService.getCount(conditions);

			// 一覧内容取得
			List<SummarylistcolumnInfo> list = new ArrayList<SummarylistcolumnInfo>();
			if(count > 0){
				list = externallistsummarycolumnInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			//maxDisporder = externallistsummarycolumnInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute SummarylistcolumnInfoForm externallistsummarycolumnInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(SummarylistcolumnInfo.class, externallistsummarycolumnInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				externallistsummarycolumnInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					SummarylistcolumnInfo externallistsummarycolumnInfo = externallistsummarycolumnInfoService.findById(Long.decode(externallistsummarycolumnInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(externallistsummarycolumnInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, SummarylistcolumnInfo.class, externallistsummarycolumnInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(SummarylistcolumnInfo.class, externallistsummarycolumnInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							externallistsummarycolumnInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(externallistsummarycolumnInfoForm.id));
				int count = externallistsummarycolumnInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = externallistsummarycolumnInfoService.findById(Long.decode(externallistsummarycolumnInfoForm.id));
					//削除実行
					try{
						externallistsummarycolumnInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	 ...
	 * @throws ServiceException
	 */
	@RequestMapping(value="/createFunctionSelectTag/{menuinfoid}")
	@ResponseBody
	public String createFunctionSelectTag(
				@PathVariable String menuinfoid) throws ServiceException {
		try{
			//パラメータ取得
