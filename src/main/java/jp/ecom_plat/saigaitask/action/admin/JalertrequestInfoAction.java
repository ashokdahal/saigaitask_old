...
import jp.ecom_plat.saigaitask.form.db.JalertrequestInfoForm;
...
/**
 * JAlert情報取得情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/jalertrequestInfo")
public class JalertrequestInfoAction extends AbstractAdminAction<JalertrequestInfo> {
	protected JalertrequestInfoForm jalertrequestInfoForm;
	...
	public String index() {
		return "/admin/jalertrequestInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.jalertrequestInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = jalertrequestInfoService.getCount(conditions);

			// 一覧内容取得
			List<JalertrequestInfo> list = new ArrayList<JalertrequestInfo>();
			if(count > 0){
				list = jalertrequestInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JalertrequestInfoForm jalertrequestInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			JalertrequestInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JalertrequestInfo.class, jalertrequestInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				jalertrequestInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JalertrequestInfo jalertrequestInfo = jalertrequestInfoService.findById(Long.decode(jalertrequestInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(jalertrequestInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JalertrequestInfo.class, jalertrequestInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JalertrequestInfo.class, jalertrequestInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							jalertrequestInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(jalertrequestInfoForm.id));
				int count = jalertrequestInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = jalertrequestInfoService.findById(Long.decode(jalertrequestInfoForm.id));
					//削除実行
					jalertrequestInfoService.delete(entity);
				}else{
