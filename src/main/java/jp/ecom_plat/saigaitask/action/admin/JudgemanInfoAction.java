...
import jp.ecom_plat.saigaitask.form.db.JudgemanInfoForm;
...
@RequestMapping("/admin/judgemanInfo")
public class JudgemanInfoAction extends AbstractAdminAction<JudgemanInfo> {
	protected JudgemanInfoForm judgemanInfoForm;
	...
	public String index() {
		return "/admin/judgemanInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.judgemanInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = judgemanInfoService.getCount(conditions);

			// 一覧内容取得
			List<JudgemanInfo> list = new ArrayList<JudgemanInfo>();
			if(count > 0){
				list = judgemanInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JudgemanInfoForm judgemanInfoForm) throws ServiceException {
		...
		try{
			...
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JudgemanInfo.class, judgemanInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				judgemanInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JudgemanInfo judgemanInfo = judgemanInfoService.findById(Long.decode(judgemanInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(judgemanInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JudgemanInfo.class, judgemanInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JudgemanInfo.class, judgemanInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							judgemanInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(judgemanInfoForm.id));
				int count = judgemanInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = judgemanInfoService.findById(Long.decode(judgemanInfoForm.id));
					//削除実行
					try{
						judgemanInfoService.delete(entity);
