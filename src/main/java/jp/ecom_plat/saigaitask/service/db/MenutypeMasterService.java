...
import static jp.ecom_plat.saigaitask.entity.Names.menutypeMaster;
...
public class MenutypeMasterService extends AbstractService<MenutypeMaster> {

    public MenutypeMaster findById(Integer id) {
	 ...
	 * @return 検索結果
	 */
	public List<MenutypeMaster> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = menutypeMaster().disporder().toString();
		}
		...
		if(!menutypeMaster().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(menutypeMaster().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		conditions.put(menutypeMaster().disporder().toString()+"_IS_NOT_NULL", Boolean.TRUE);
		List<MenutypeMaster> list = findByCondition(conditions, menutypeMaster().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(MenutypeMaster entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
