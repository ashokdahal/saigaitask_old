...
//public class TabledatagroupdataInfoAction extends AbstractAdminAction<TabledatagroupdataInfo> {
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/tabledatagroupdataInfo")
public class TabledatagroupdataInfoAction{
//	protected TabledatagroupdataInfoForm tabledatagroupdataInfoForm;
//
//	/** サービスクラス */
//	@Resource
//	protected TabledatagroupdataInfoExService tabledatagroupdataInfoExService;
//
//	/** サービスクラス */
//	@Resource
...
//
	...
	public String index() {
		return "/admin/tabledatagroupdataInfo/index";
	}
//
//	/**
...
//
//		// 一覧件数取得
//		int count = tabledatagroupdataInfoExService.getCount(conditions);
//
//		// 一覧内容取得
//		List<TabledatagroupdataInfo> list = new ArrayList<TabledatagroupdataInfo>();
//		if(count > 0){
//			list = tabledatagroupdataInfoExService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
//		}
//
//		//ユーザーデータ取得
...
//
//		//操作種別に応じたアクションを実行
//		TabledatagroupdataInfo entity;
//		//登録
//		if("add".equals(oper)) {
//			entity = Beans.createAndCopy(TabledatagroupdataInfo.class, tabledatagroupdataInfoForm).dateConverter("yyyy-MM-dd").execute();
//			//登録実行
//			tabledatagroupdataInfoExService.insert(entity);
//			//採番したキー値を返却
//			JSONObject json = new JSONObject();
...
//			}else{
//				//現時点の編集対象データを取得
//				TabledatagroupdataInfo tabledatagroupdataInfo = tabledatagroupdataInfoExService.findById(Integer.parseInt(tabledatagroupdataInfoForm.id));
//
//				//結果返却用JSON
//				JSONObject json = new JSONObject();
//				if(tabledatagroupdataInfo != null){
//					//編集対象が存在する場合、実行。
//					//現時点の編集対象データが他者により変更されているかを判定
//					if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TabledatagroupdataInfo.class, tabledatagroupdataInfo)){
//						//変更されていない場合、更新
//						entity = Beans.createAndCopy(TabledatagroupdataInfo.class, tabledatagroupdataInfoForm).dateConverter("yyyy-MM-dd").execute();
//						PropertyName<?>[] updateExcludesAry = null;
//						List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
//						//更新実行
//						tabledatagroupdataInfoExService.update(entity, updateExcludesAry);
...
//			//検索条件マップ
//			BeanMap conditions = new BeanMap();
//			conditions.put("id", Integer.parseInt(tabledatagroupdataInfoForm.id));
//			int count = tabledatagroupdataInfoExService.getCount(conditions);
//			if(count==1){
//				//削除対象が存在する場合、削除実行。
//				entity = tabledatagroupdataInfoExService.findById(Integer.parseInt(tabledatagroupdataInfoForm.id));
//				//削除実行
//				tabledatagroupdataInfoExService.delete(entity);
//			}else{
