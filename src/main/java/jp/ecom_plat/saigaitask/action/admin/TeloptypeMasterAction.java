...
import jp.ecom_plat.saigaitask.entity.db.TeloptypeMaster;
import jp.ecom_plat.saigaitask.form.db.TeloptypeMasterForm;
import jp.ecom_plat.saigaitask.service.TableService;
import jp.ecom_plat.saigaitask.service.db.TeloptypeMasterService;
...
@RequestMapping("/admin/teloptypeMaster")
public class TeloptypeMasterAction extends AbstractAdminAction<TeloptypeMaster> {
	protected TeloptypeMasterForm teloptypeMasterForm;

	/** サービスクラス */
	@Resource
	protected TeloptypeMasterService teloptypeMasterService;
	...
	public String index() {
		return "/admin/teloptypeMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = teloptypeMasterService.getCount(conditions);

			// 一覧内容取得
			List<TeloptypeMaster> list = new ArrayList<TeloptypeMaster>();
			if(count > 0){
				list = teloptypeMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = teloptypeMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TeloptypeMasterForm teloptypeMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TeloptypeMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TeloptypeMaster.class, teloptypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				teloptypeMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TeloptypeMaster teloptypeMaster = teloptypeMasterService.findById(Integer.parseInt(teloptypeMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(teloptypeMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TeloptypeMaster.class, teloptypeMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TeloptypeMaster.class, teloptypeMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							teloptypeMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(teloptypeMasterForm.id));
				int count = teloptypeMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = teloptypeMasterService.findById(Integer.parseInt(teloptypeMasterForm.id));
					//削除実行
					try{
						teloptypeMasterService.delete(entity);
