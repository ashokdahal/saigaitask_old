...
import jp.ecom_plat.saigaitask.form.db.StationlayerInfoForm;
...
@RequestMapping("/admin/stationlayerInfo")
public class StationlayerInfoAction extends AbstractAdminAction<StationlayerInfo> {
	protected StationlayerInfoForm stationlayerInfoForm;
	...
	public String index() {
		return "/admin/stationlayerInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.stationlayerInfo().tablemasterInfo().mapmasterInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = stationlayerInfoService.getCount(conditions);

			// 一覧内容取得
			List<StationlayerInfo> list = new ArrayList<StationlayerInfo>();
			if(count > 0){
				list = stationlayerInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute StationlayerInfoForm stationlayerInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			StationlayerInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(StationlayerInfo.class, stationlayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				stationlayerInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					StationlayerInfo stationlayerInfo = stationlayerInfoService.findById(Long.decode(stationlayerInfoForm.id));
					...
					if(stationlayerInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, StationlayerInfo.class, stationlayerInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(StationlayerInfo.class, stationlayerInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							stationlayerInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(stationlayerInfoForm.id));
				int count = stationlayerInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = stationlayerInfoService.findById(Long.decode(stationlayerInfoForm.id));
					//削除実行
					try{
						stationlayerInfoService.delete(entity);
