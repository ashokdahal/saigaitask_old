...
import jp.ecom_plat.saigaitask.form.db.ObservlistInfoForm;
...
@RequestMapping("/admin/observlistInfo")
public class ObservlistInfoAction extends AbstractAdminAction<ObservlistInfo> {
	protected ObservlistInfoForm observlistInfoForm;
	...
	public String index() {
		return "/admin/observlistInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = observlistInfoService.getCount(conditions);

			// 一覧内容取得
			List<ObservlistInfo> list = new ArrayList<ObservlistInfo>();
			if(count > 0){
				list = observlistInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = observlistInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ObservlistInfoForm observlistInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ObservlistInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ObservlistInfo.class, observlistInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				observlistInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ObservlistInfo observlistInfo = observlistInfoService.findById(Long.decode(observlistInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(observlistInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ObservlistInfo.class, observlistInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ObservlistInfo.class, observlistInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							observlistInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(observlistInfoForm.id));
				int count = observlistInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = observlistInfoService.findById(Long.decode(observlistInfoForm.id));
					//削除実行
					try{
						observlistInfoService.delete(entity);
