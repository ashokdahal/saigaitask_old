...
import jp.ecom_plat.saigaitask.form.db.MeteorequestInfoForm;
...
@RequestMapping("/admin/meteorequestInfo")
public class MeteorequestInfoAction extends AbstractAdminAction<MeteorequestInfo> {
	protected MeteorequestInfoForm meteorequestInfoForm;
	...
	public String index() {
		return "/admin/meteorequestInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.meteorequestInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = meteorequestInfoService.getCount(conditions);

			// 一覧内容取得
			List<MeteorequestInfo> list = new ArrayList<MeteorequestInfo>();
			if(count > 0){
				list = meteorequestInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MeteorequestInfoForm meteorequestInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MeteorequestInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MeteorequestInfo.class, meteorequestInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteorequestInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MeteorequestInfo meteorequestInfo = meteorequestInfoService.findById(Long.decode(meteorequestInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteorequestInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MeteorequestInfo.class, meteorequestInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MeteorequestInfo.class, meteorequestInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteorequestInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteorequestInfoForm.id));
				int count = meteorequestInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteorequestInfoService.findById(Long.decode(meteorequestInfoForm.id));
					//削除実行
					try{
						meteorequestInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
			...
			if(StringUtils.isEmpty(selectTagColumn)
				...
				//検索条件追加。
				BeanMap conditions = new BeanMap();
				Object conditionObj = conditions;
				long localgovinfoid = loginDataDto.getLocalgovinfoid();
				if("localgov_info".equals(tableName)){
					...
					String meteotypeid = request.getParameter("meteotypeid");
					if ("1".equals(meteotypeid))
						conditions.put("warnflag", true);
					else if ("4".equals(meteotypeid)) {
						conditionObj = "nameseismic <> ''";
						methodArgType[0] = String.class;
					}
					else if ("6".equals(meteotypeid))
						conditions.put("landslideflag", true);
					else if ("8".equals(meteotypeid))
						conditions.put("tatsumakiflag", true);
					else if ("9".equals(meteotypeid)) {
						conditionObj = "namevolcano <> ''";
						methodArgType[0] = String.class;
					}
				}
				Object[] methodArg = {conditionObj, codeColumn, ASC, NON_LIMIT, NON_SELECT_OFFSET};
