...
import static jp.ecom_plat.saigaitask.entity.Names.meteorainareaMaster;
...
public class MeteorainareaMasterService extends AbstractService<MeteorainareaMaster> {

    public MeteorainareaMaster findById(Integer id) {
	 ...
	 * @return 検索結果
	 */
	public List<MeteorainareaMaster> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = meteorainareaMaster().disporder().toString();
		}
		...
		if(!meteorainareaMaster().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(meteorainareaMaster().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		conditions.put(meteorainareaMaster().disporder().toString()+"_IS_NOT_NULL", Boolean.TRUE);
		List<MeteorainareaMaster> list = findByCondition(conditions, meteorainareaMaster().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(MeteorainareaMaster entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
