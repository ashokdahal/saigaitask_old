...
import jp.ecom_plat.saigaitask.form.db.EcomgwpostInfoForm;
...
/**
 * eコミグループウェア投稿先情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/ecomgwpostInfo")
public class EcomgwpostInfoAction extends AbstractAdminAction<EcomgwpostInfo> {
	protected EcomgwpostInfoForm ecomgwpostInfoForm;
	...
	public String index() {
		return "/admin/ecomgwpostInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.ecomgwpostInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = ecomgwpostInfoService.getCount(conditions);

			// 一覧内容取得
			List<EcomgwpostInfo> list = new ArrayList<EcomgwpostInfo>();
			if(count > 0){
				list = ecomgwpostInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = ecomgwpostInfoService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute EcomgwpostInfoForm ecomgwpostInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			EcomgwpostInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(EcomgwpostInfo.class, ecomgwpostInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				ecomgwpostInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					EcomgwpostInfo ecomgwpostInfo = ecomgwpostInfoService.findById(Long.decode(ecomgwpostInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(ecomgwpostInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, EcomgwpostInfo.class, ecomgwpostInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(EcomgwpostInfo.class, ecomgwpostInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							ecomgwpostInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(ecomgwpostInfoForm.id));
				int count = ecomgwpostInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = ecomgwpostInfoService.findById(Long.decode(ecomgwpostInfoForm.id));
					//削除実行
					try{
						ecomgwpostInfoService.delete(entity);
