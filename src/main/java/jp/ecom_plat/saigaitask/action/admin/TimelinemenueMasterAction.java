...
import jp.ecom_plat.saigaitask.entity.db.TimelinemenueMaster;
import jp.ecom_plat.saigaitask.form.db.TimelinemenueMasterForm;
import jp.ecom_plat.saigaitask.service.TableService;
import jp.ecom_plat.saigaitask.service.db.TimelinemenueMasterService;
...
@RequestMapping("/admin/timelinemenueMaster")
public class TimelinemenueMasterAction extends AbstractAdminAction<TimelinemenueMaster> {
	protected TimelinemenueMasterForm timelinemenueMasterForm;

	/** サービスクラス */
	@Resource
	protected TimelinemenueMasterService timelinemenueMasterService;
	...
	public String index() {
		return "/admin/timelinemenueMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = timelinemenueMasterService.getCount(conditions);

			// 一覧内容取得
			List<TimelinemenueMaster> list = new ArrayList<TimelinemenueMaster>();
			if(count > 0){
				list = timelinemenueMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = timelinemenueMasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TimelinemenueMasterForm timelinemenueMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TimelinemenueMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TimelinemenueMaster.class, timelinemenueMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				timelinemenueMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TimelinemenueMaster timelinemenueMaster = timelinemenueMasterService.findById(Long.decode(timelinemenueMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(timelinemenueMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TimelinemenueMaster.class, timelinemenueMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TimelinemenueMaster.class, timelinemenueMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							timelinemenueMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(timelinemenueMasterForm.id));
				int count = timelinemenueMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = timelinemenueMasterService.findById(Long.decode(timelinemenueMasterForm.id));
					//削除実行
					try{
						timelinemenueMasterService.delete(entity);
