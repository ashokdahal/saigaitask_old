...
import jp.ecom_plat.saigaitask.form.db.PubliccommonsReportShelterInfoForm;
...
/**
 * 公共情報コモンズ避難所情報のアクションクラス
 */
@jp.ecom_plat.saigaitask.action.RequestScopeController
@RequestMapping("/admin/publiccommonsReportShelterInfo")
public class PubliccommonsReportShelterInfoAction extends AbstractAdminAction<PubliccommonsReportShelterInfo> {
	protected PubliccommonsReportShelterInfoForm publiccommonsReportShelterInfoForm;
	...
	public String index() {
		return "/admin/publiccommonsReportShelterInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.publiccommonsReportShelterInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = publiccommonsReportShelterInfoService.getCount(conditions);

			// 一覧内容取得
			List<PubliccommonsReportShelterInfo> list = new ArrayList<PubliccommonsReportShelterInfo>();
			if(count > 0){
				list = publiccommonsReportShelterInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute PubliccommonsReportShelterInfoForm publiccommonsReportShelterInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			PubliccommonsReportShelterInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(PubliccommonsReportShelterInfo.class, publiccommonsReportShelterInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				publiccommonsReportShelterInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					PubliccommonsReportShelterInfo publiccommonsReportShelterInfo = publiccommonsReportShelterInfoService.findById(Long.decode(publiccommonsReportShelterInfoForm.id));
					...
					if(publiccommonsReportShelterInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, PubliccommonsReportShelterInfo.class, publiccommonsReportShelterInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(PubliccommonsReportShelterInfo.class, publiccommonsReportShelterInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							publiccommonsReportShelterInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(publiccommonsReportShelterInfoForm.id));
				int count = publiccommonsReportShelterInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = publiccommonsReportShelterInfoService.findById(Long.decode(publiccommonsReportShelterInfoForm.id));
					//削除実行
					try{
						publiccommonsReportShelterInfoService.delete(entity);
...
//			String decodeColumn = (String) request.getParameter(JQG_DECODE_COLUMN); //デコード値を保持しているテーブルのデコードカラム
	...
	/**
	 * Jqgrid テーブルIDのレイヤIDが持つ属性の名前と一致する入力項目に属性のIDをセットする（Ajax）
	 * @return null
	 * @throws ServiceException
	 */
	@RequestMapping(value="/loadDialogTextInputColumnValue", method=org.springframework.web.bind.annotation.RequestMethod.POST)
	@ResponseBody
	public String loadDialogTextInputColumnValue() throws ServiceException {

		// 入力項目の名前とカラムIDのマップ
		Map<String,String> attrsMap = new HashMap<String,String>();
		attrsMap.put(lang.__("Shelter name"),"attrshelter");
		attrsMap.put(lang.__("District"),"attrarea");
		attrsMap.put(lang.__("Address"),"attraddress");
		attrsMap.put(lang.__("Phone number"),"attrphone");
		attrsMap.put("FAX","attrfax");
		attrsMap.put(lang.__("Representative name"),"attrstaff");
		attrsMap.put(lang.__("Shelter status"),"attrstatus");
		attrsMap.put(lang.__("Capacity"),"attrcapacity");
		attrsMap.put(lang.__("Opening date and time"),"attrsetuptime");
		attrsMap.put(lang.__("Opening date and time"),"attrclosetime");
		attrsMap.put(lang.__("Type<!--2-->"),"attrtype");
		attrsMap.put(lang.__("Coordinates<!--2-->"),"attrcircle");
		attrsMap.put(lang.__("Number of evacuees"),"attrheadcount");
		attrsMap.put(lang.__("Number of evacuees (including voluntary evacuation)"),"attrheadcountvoluntary");
		attrsMap.put(lang.__("Number of evacuated households"),"attrhouseholds");
		attrsMap.put(lang.__("Number of evacuated households (including voluntary evacuation)"),"attrhouseholdsvoluntary");

		try{
			//パラメータ取得
			String tablemasterinfoid = (String) request.getParameter("tablemasterinfoid");

			//返却値
			JSONObject json = new JSONObject();

			Map<String,String> columnMap = new HashMap<String,String>();
			for(String columnName : attrsMap.values()){
				columnMap.put(columnName, "");
			}

			if(StringUtils.isEmpty(tablemasterinfoid)){
				throw new IllegalStateException(lang.__("Pull-down creation parameters is invalid."));
			}else{
				// eコミマップのレイヤ情報取得
				List<AttrInfo> attrInfos = mapService.getAttrInfoFromTablamasterId(tablemasterinfoid);
				for(AttrInfo attrInfo : attrInfos){
					String columName = attrsMap.get(attrInfo.name);
					if(columName != null){
						// 一致した場合はカラムIDに属性IDを紐づけておく
						columnMap.put(columName,attrInfo.attrId);
					}
				}
			}
			...
			json.put("columns",StringUtil.json(dataList));
