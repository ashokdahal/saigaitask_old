...
import jp.ecom_plat.saigaitask.form.db.TwitterMasterForm;
...
@RequestMapping("/admin/twitterMaster")
public class TwitterMasterAction extends AbstractAdminAction<TwitterMaster> {
	protected TwitterMasterForm twitterMasterForm;
	...
	public String index() {
		return "/admin/twitterMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = twitterMasterService.getCount(conditions);

			// 一覧内容取得
			List<TwitterMaster> list = new ArrayList<TwitterMaster>();
			if(count > 0){
				list = twitterMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TwitterMasterForm twitterMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TwitterMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TwitterMaster.class, twitterMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				twitterMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TwitterMaster twitterMaster = twitterMasterService.findById(Integer.parseInt(twitterMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(twitterMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TwitterMaster.class, twitterMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TwitterMaster.class, twitterMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							twitterMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(twitterMasterForm.id));
				int count = twitterMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = twitterMasterService.findById(Integer.parseInt(twitterMasterForm.id));
					//削除実行
					try{
						twitterMasterService.delete(entity);
