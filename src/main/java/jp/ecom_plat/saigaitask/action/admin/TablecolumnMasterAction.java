...
import jp.ecom_plat.saigaitask.form.db.TablecolumnMasterForm;
...
@RequestMapping("/admin/tablecolumnMaster")
public class TablecolumnMasterAction extends AbstractAdminAction<TablecolumnMaster> {
	protected TablecolumnMasterForm tablecolumnMasterForm;
	...
	public String index() {
		return "/admin/tablecolumnMaster/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = tablecolumnMasterService.getCount(conditions);

			// 一覧内容取得
			List<TablecolumnMaster> list = new ArrayList<TablecolumnMaster>();
			if(count > 0){
				list = tablecolumnMasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute TablecolumnMasterForm tablecolumnMasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			TablecolumnMaster entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(TablecolumnMaster.class, tablecolumnMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				tablecolumnMasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					TablecolumnMaster tablecolumnMaster = tablecolumnMasterService.findById(Integer.parseInt(tablecolumnMasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(tablecolumnMaster != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, TablecolumnMaster.class, tablecolumnMaster)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(TablecolumnMaster.class, tablecolumnMasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							tablecolumnMasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(tablecolumnMasterForm.id));
				int count = tablecolumnMasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = tablecolumnMasterService.findById(Integer.parseInt(tablecolumnMasterForm.id));
					//削除実行
					try{
						tablecolumnMasterService.delete(entity);
