...
import jp.ecom_plat.saigaitask.form.db.Meteoseismicarea1MasterForm;
...
@RequestMapping("/admin/meteoseismicarea1Master")
public class Meteoseismicarea1MasterAction extends AbstractAdminAction<Meteoseismicarea1Master> {
	protected Meteoseismicarea1MasterForm meteoseismicarea1MasterForm;

	/** サービスクラス */
	@Resource
	protected Meteoseismicarea1MasterService meteoseismicarea1MasterService;
	...
	public String index() {
		return "/admin/meteoseismicarea1Master/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			// 一覧件数取得
			int count = meteoseismicarea1MasterService.getCount(conditions);

			// 一覧内容取得
			List<Meteoseismicarea1Master> list = new ArrayList<Meteoseismicarea1Master>();
			if(count > 0){
				list = meteoseismicarea1MasterService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
			...
			maxDisporder = meteoseismicarea1MasterService.getLargestDisporder(maxDisporderConditions);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute Meteoseismicarea1MasterForm meteoseismicarea1MasterForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			Meteoseismicarea1Master entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(Meteoseismicarea1Master.class, meteoseismicarea1MasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				meteoseismicarea1MasterService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					Meteoseismicarea1Master meteoseismicarea1Master = meteoseismicarea1MasterService.findById(Integer.parseInt(meteoseismicarea1MasterForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(meteoseismicarea1Master != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, Meteoseismicarea1Master.class, meteoseismicarea1Master)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(Meteoseismicarea1Master.class, meteoseismicarea1MasterForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							meteoseismicarea1MasterService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(meteoseismicarea1MasterForm.id));
				int count = meteoseismicarea1MasterService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = meteoseismicarea1MasterService.findById(Integer.parseInt(meteoseismicarea1MasterForm.id));
					//削除実行
					try{
						meteoseismicarea1MasterService.delete(entity);
