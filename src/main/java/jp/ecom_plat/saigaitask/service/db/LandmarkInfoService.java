...
import static jp.ecom_plat.saigaitask.entity.Names.landmarkInfo;
...
public class LandmarkInfoService extends AbstractService<LandmarkInfo> {

	public LandmarkInfo findById(Long id) {
		return select().id(id).getSingleResult();
	}

	/**
	 * @param	localgovinfoid				自治体ID
	 * @return	LandmarkInfo	LandmarkInfoオブジェクト
	 */
	public LandmarkInfo findByLocalgovinfoId(Long localgovinfoid) {
		return select().where(
			eq(landmarkInfo().localgovinfoid(), localgovinfoid)
		).getSingleResult();
	}
	/**
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(landmarkInfo().localgovInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<LandmarkInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = landmarkInfo().id().toString();
		}
		...
		if(!landmarkInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(landmarkInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(LandmarkInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
	...
	@Override
	public DeleteCascadeResult deleteCascade(LandmarkInfo entity, DeleteCascadeResult result) {

		result.cascade(LandmarkData.class, Names.landmarkData().landmarkinfoid(), entity.id);
