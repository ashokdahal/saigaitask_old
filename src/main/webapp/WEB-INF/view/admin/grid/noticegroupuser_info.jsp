...
function noticegroupuser_info(subgrid_id, row_id){
	var tabName = 'noticegroupuser_info';
	var noticegroupuser_info_gridId = 'main';
	if(subgrid_id != null){
		noticegroupuser_info_gridId = subgrid_id;
	}
	var noticegroupuser_info_tabId = tabName;
	if(row_id != null){
		noticegroupuser_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'noticegroup_info', 'noticegroupinfoid');

	var noticegroupuser_info_pagerId = noticegroupuser_info_tabId+'Nav';
	$('#'+noticegroupuser_info_gridId).append('<table id="'+noticegroupuser_info_tabId+'" class="scroll"></table><div id="'+noticegroupuser_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+noticegroupuser_info_tabId).jqGrid({
		caption: '<%=lang.__("Notification group user info")%>',
		mtype: 'POST',
		...
		colModel:[
			...
			editoptions:{value:'',
				...
				delimiter:'|d|',
				readonly:false
					,dataUrl:convertTabNameToMethodMame(tabName)+'/createSelectTag/userid/user_info/id/name',
					//フォームのプルダウン作成
					buildSelect: function (data) {
						var selectTag = $.parseJSON(data).selectTag;
						var rowid = $('#'+noticegroupuser_info_tabId).jqGrid('getGridParam', 'selrow');
						var selectedId = null;
						if(rowid != null){
							//編集の場合
							var rowdata = $('#'+noticegroupuser_info_tabId).jqGrid('getRowData', rowid);
							selectedId = rowdata.userid;
						}else{
						}
						return createSelectTag(rowid, selectTag, selectedId, 'noticegroupuser_info', 'user_info');
					}
			},
			editrules:{required:true, number:false, integer:false, email:false, edithidden:false},
			sortable:true,
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("User ID(*)")%>'},
			hidden:false},
		],
		ajaxGridOptions :{
		...
		pager: noticegroupuser_info_pagerId,
		autowidth: true,
		autoencode: false,
		beforeProcessing:function(data){
			...
			$('#'+noticegroupuser_info_tabId).jqGrid('setColProp', 'noticegroupinfoid', {editoptions:{value:str}});
			...
			$('#'+noticegroupuser_info_tabId).jqGrid('setColProp', 'userid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+noticegroupuser_info_tabId).getDataIDs();
	...
	//ナビゲーション
	$('#'+noticegroupuser_info_tabId).jqGrid('navGrid',
			'#'+noticegroupuser_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+noticegroupuser_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+noticegroupuser_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Notification group user info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					var rowdata = $('#'+noticegroupuser_info_tabId).jqGrid('getRowData', rowid);
					s += '&userid='+rowdata.userid;
					$('#'+noticegroupuser_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+noticegroupuser_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+noticegroupuser_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+noticegroupuser_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+noticegroupuser_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+noticegroupuser_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+noticegroupuser_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Notification group user info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+noticegroupuser_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_noticegroupinfoid .CaptionTD', formid).after('<td id="dispnoticegroupinfoid">'+row_id+'</td>');
					$('#tr_noticegroupinfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Notification group user info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+noticegroupuser_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+noticegroupuser_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Notification group user info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+noticegroupuser_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ noticegroupuser_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_noticegroupuser_info #id').val('');
					}else{
...
//	$('#'+noticegroupuser_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+noticegroupuser_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');
