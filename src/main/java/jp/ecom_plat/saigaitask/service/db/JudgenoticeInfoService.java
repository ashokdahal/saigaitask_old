...
import static jp.ecom_plat.saigaitask.entity.Names.judgenoticeInfo;
...
public class JudgenoticeInfoService extends AbstractService<JudgenoticeInfo> {
	 ...
	 * @return 判定通知情報
	 */
    public JudgenoticeInfo findById(Long id) {
        return select().id(id).getSingleResult();
    }

    /**
     * 判定アラーム情報で検索
     * @param jid 判定アラーム情報ID
     * @return 判定通知情報
     */
    public List<JudgenoticeInfo> findByJudgealarmInfoIdAndValid(Long jid) {
    	return select().where(
    			and(
    				eq(judgenoticeInfo().judgealarminfoid(), jid),
    				eq(judgenoticeInfo().valid(), true)
    			)).getResultList();
    }
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(judgenoticeInfo().noticegroupInfo())
			.innerJoin(judgenoticeInfo().judgealarmInfo())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<JudgenoticeInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = judgenoticeInfo().id().toString();
		}
		...
		if(!judgenoticeInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(judgenoticeInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(JudgenoticeInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
