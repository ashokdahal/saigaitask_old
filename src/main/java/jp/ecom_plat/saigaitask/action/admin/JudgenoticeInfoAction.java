...
import jp.ecom_plat.saigaitask.form.db.JudgenoticeInfoForm;
...
@RequestMapping("/admin/judgenoticeInfo")
public class JudgenoticeInfoAction extends AbstractAdminAction<JudgenoticeInfo> {
	protected JudgenoticeInfoForm judgenoticeInfoForm;
	...
	public String index() {
		return "/admin/judgenoticeInfo/index";
	}
...
//			String parentId = (String)request.getParameter("parentId");
			...
			// 一覧件数取得
			int count = judgenoticeInfoService.getCount(conditions);

			// 一覧内容取得
			List<JudgenoticeInfo> list = new ArrayList<JudgenoticeInfo>();
			if(count > 0){
				list = judgenoticeInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute JudgenoticeInfoForm judgenoticeInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			JudgenoticeInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(JudgenoticeInfo.class, judgenoticeInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				judgenoticeInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					JudgenoticeInfo judgenoticeInfo = judgenoticeInfoService.findById(Long.decode(judgenoticeInfoForm.id));

					//結果返却用JSON
					JSONObject json = new JSONObject();
					if(judgenoticeInfo != null){
						//編集対象が存在する場合、実行。
						//現時点の編集対象データが他者により変更されているかを判定
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, JudgenoticeInfo.class, judgenoticeInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(JudgenoticeInfo.class, judgenoticeInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							judgenoticeInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(judgenoticeInfoForm.id));
				int count = judgenoticeInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = judgenoticeInfoService.findById(Long.decode(judgenoticeInfoForm.id));
					//削除実行
					try{
						judgenoticeInfoService.delete(entity);
