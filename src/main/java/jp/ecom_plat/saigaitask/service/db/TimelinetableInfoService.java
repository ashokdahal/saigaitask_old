...
import static jp.ecom_plat.saigaitask.entity.Names.timelinetableInfo;
...
public class TimelinetableInfoService extends AbstractService<TimelinetableInfo> {
     ...
     * @return エンティティ
     */
    public TimelinetableInfo findById(Long id) {
     ...
     * @return エンティティのリスト
     */
    public List<TimelinetableInfo> findAllOrderById() {
        return select().orderBy(asc(id())).getResultList();
    }

    public List<TimelinetableInfo> findByTimelinemenueMasterId(Long id){
		BeanMap conditions = new BeanMap();
		conditions.put(TimelinetableInfoNames.timelinemenuemasterid().toString(), id);
	...
	public int getCount(Map<String, Object> conditions) {
		return (int)select()
			.innerJoin(timelinetableInfo().tablemasterInfo())
			.innerJoin(timelinetableInfo().timelinemenueMaster())
			.where(conditions)
			.getCount();
	}
	 ...
	 * @return 検索結果
	 */
	public List<TimelinetableInfo> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = timelinetableInfo().id().toString();
		}
		...
		if(!timelinetableInfo().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(timelinetableInfo().id(), OrderingSpec.ASC));
		}
	 ...
	 * @return
	*/
	public int update(TimelinetableInfo entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
