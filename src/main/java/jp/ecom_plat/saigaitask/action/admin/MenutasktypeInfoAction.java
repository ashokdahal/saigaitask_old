...
import jp.ecom_plat.saigaitask.form.db.MenutasktypeInfoForm;
...
@RequestMapping("/admin/menutasktypeInfo")
public class MenutasktypeInfoAction extends AbstractAdminAction<MenutasktypeInfo> {
	protected MenutasktypeInfoForm menutasktypeInfoForm;

	/** サービスクラス */
	@Resource
	protected MenutasktypeInfoService menutasktypeInfoService;
	
	private static final boolean TEMPLATE_FLAG = false;
	...
	public String index(Map<String,Object>model) {
	   	setupModel(model);
		return "/admin/menutasktypeInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.menutasktypeInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = menutasktypeInfoService.getCount(conditions, TEMPLATE_FLAG);

			// 一覧内容取得
			List<MenutasktypeInfo> list = new ArrayList<MenutasktypeInfo>();
			if(count > 0){
				list = menutasktypeInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit, TEMPLATE_FLAG);
			}
			...
			maxDisporder = menutasktypeInfoService.getLargestDisporder(maxDisporderConditions, TEMPLATE_FLAG);
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute MenutasktypeInfoForm menutasktypeInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			MenutasktypeInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(MenutasktypeInfo.class, menutasktypeInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				menutasktypeInfoService.insert(entity);
				//全コピー実施時
				...
				if(isAllCopy){
					...
					doAllCopy("menutasktype_info", oldId, entity.id);
				}
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					MenutasktypeInfo menutasktypeInfo = menutasktypeInfoService.findById(Long.decode(menutasktypeInfoForm.id));
					...
					if(menutasktypeInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, MenutasktypeInfo.class, menutasktypeInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(MenutasktypeInfo.class, menutasktypeInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							menutasktypeInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(menutasktypeInfoForm.id));
				int count = menutasktypeInfoService.getCount(conditions, TEMPLATE_FLAG);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = menutasktypeInfoService.findById(Long.decode(menutasktypeInfoForm.id));
					//削除実行
					try{
						menutasktypeInfoService.deleteLogically(entity);
