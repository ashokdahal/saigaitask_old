...
import jp.ecom_plat.saigaitask.form.db.ObservatoryrainInfoForm;
...
@RequestMapping("/admin/observatoryrainInfo")
public class ObservatoryrainInfoAction extends AbstractAdminAction<ObservatoryrainInfo> {
	protected ObservatoryrainInfoForm observatoryrainInfoForm;
	...
	public String index() {
		return "/admin/observatoryrainInfo/index";
	}
	...
	public String jqgridindex() throws ServiceException {
		try{
			...
			if(localgovinfoid != ADMIN_LOCALGOVINFOID){
				...
				conditions.put(Names.observatoryrainInfo().localgovinfoid().toString(), localgovinfoid);
			}
			...
			// 一覧件数取得
			int count = observatoryrainInfoService.getCount(conditions);

			// 一覧内容取得
			List<ObservatoryrainInfo> list = new ArrayList<ObservatoryrainInfo>();
			if(count > 0){
				list = observatoryrainInfoService.findByCondition(conditions, sidx, sord, limit, (page-1)*limit);
			}
	...
	@ResponseBody
	public String jqgridedit(@ModelAttribute ObservatoryrainInfoForm observatoryrainInfoForm) throws ServiceException {
		...
		try{
			...
			//操作種別に応じたアクションを実行
			ObservatoryrainInfo entity;
			//登録
			if(JQG_OPER_ADD.equals(oper)) {
				entity = Beans.createAndCopy(ObservatoryrainInfo.class, observatoryrainInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
				//登録実行
				observatoryrainInfoService.insert(entity);
				//採番したキー値を返却
				JSONObject json = new JSONObject();
			...
			//編集
				...
				if(StringUtils.isEmpty(serializedPreEditData)){
					...
					//現時点の編集対象データを取得
					ObservatoryrainInfo observatoryrainInfo = observatoryrainInfoService.findById(Long.decode(observatoryrainInfoForm.id));
					...
					if(observatoryrainInfo != null){
						...
						if(JqGridUtil.existsNoChangeByOther(serializedPreEditData, ObservatoryrainInfo.class, observatoryrainInfo)){
							//変更されていない場合、更新
							entity = Beans.createAndCopy(ObservatoryrainInfo.class, observatoryrainInfoForm).dateConverter(DATE_FORMAT_PATTERN).execute();
							PropertyName<?>[] updateExcludesAry = null;
							List<PropertyName<?>> updateExcludesList = new ArrayList<PropertyName<?>>();
							//更新実行
							observatoryrainInfoService.update(entity, updateExcludesAry);
			...
			//削除
				...
				//検索条件マップ
				BeanMap conditions = new BeanMap();
				conditions.put("id", Integer.parseInt(observatoryrainInfoForm.id));
				int count = observatoryrainInfoService.getCount(conditions);
				if(count==1){
					//削除対象が存在する場合、削除実行。
					entity = observatoryrainInfoService.findById(Long.decode(observatoryrainInfoForm.id));
					//削除実行
					try{
						observatoryrainInfoService.delete(entity);
