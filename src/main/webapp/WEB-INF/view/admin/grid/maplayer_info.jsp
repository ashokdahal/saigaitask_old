...
function maplayer_info(subgrid_id, row_id){
	var tabName = 'maplayer_info';
	var maplayer_info_gridId = 'main';
	if(subgrid_id != null){
		maplayer_info_gridId = subgrid_id;
	}
	var maplayer_info_tabId = tabName;
	if(row_id != null){
		maplayer_info_tabId += row_id;
	}else{
		row_id = '';
	}
	var parentgrid_refkey = getParentgridRefkey(subgrid_id, 'menu_info', 'menuinfoid');

	var maplayer_info_pagerId = maplayer_info_tabId+'Nav';
	$('#'+maplayer_info_gridId).append('<table id="'+maplayer_info_tabId+'" class="scroll"></table><div id="'+maplayer_info_pagerId+'" class="scroll"></div>');

	var thisGridLevel = getBreadcrumb(subgrid_id!=''?getParentTableName(subgrid_id):'', tabName);

	//グリッド
	$('#'+maplayer_info_tabId).jqGrid({
		caption: '<%=lang.__("Map layer info")%>',
		mtype: 'POST',
		...
		colModel:[
			...
			editoptions:{value:'',
				...
				readonly:false
					...
					buildSelect: function (data) {
						...
						var rowid = $('#'+maplayer_info_tabId).jqGrid('getGridParam', 'selrow');
						var selectedId = null;
						if(rowid != null){
							//編集の場合
							var rowdata = $('#'+maplayer_info_tabId).jqGrid('getRowData', rowid);
							selectedId = rowdata.tablemasterinfoid;
						}else{
						}
						return createSelectTag(rowid, selectTag, selectedId, 'maplayer_info', 'tablemaster_info');
					}
			},
			editrules:{required:true, number:false, integer:false, email:false, edithidden:false},
			sortable:true,
			...
			search:true,
			searchrules:{required:true},
			formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Table master ID(*)")%>'},
			hidden:false},
			{name:'visible',
			width:'150',
			align:'left',
			...
			{name:'intersectionlayerid',
				...
				search:true,
				searchrules:{required:true},
				formoptions:{elmprefix:'', elmsuffix:'', label:'<%=lang.__("Cropping layer ID")%>'},
				hidden:false},
			{name:'valid',
			width:'130',
			align:'left',
		...
		beforeProcessing:function(data){
			...
			$('#'+maplayer_info_tabId).jqGrid('setColProp', 'menuinfoid', {editoptions:{value:str}});
			...
			$('#'+maplayer_info_tabId).jqGrid('setColProp', 'tablemasterinfoid', {editoptions:{value:str}});

		},
		loadComplete : function (data) {
			//console.log(data); //Ajaxで取得したデータがdataに入ってくる。
			//1行おきに背景色をかえる
			var rowIDs = $('#'+maplayer_info_tabId).getDataIDs();
			...
			var maxDisporder = $('#'+maplayer_info_tabId).jqGrid('getGridParam','userData').maxDisporder;
			$('#'+maplayer_info_tabId).jqGrid('setColProp', 'disporder', {editoptions:{defaultValue:maxDisporder+1}});
			if(getEditMode() == 'add'){
	...
	//ナビゲーション
	$('#'+maplayer_info_tabId).jqGrid('navGrid',
			'#'+maplayer_info_pagerId,
			//ナビゲーションオプション設定
			{edit:true,add:true,del:true,refresh:true,
				beforeRefresh: function(){	//リフレッシュボタン押下時
					//リフレッシュ処理直前にdatatypeを再設定することで再度Ajaxでデータ取得する。
					$('#'+maplayer_info_tabId).jqGrid('setGridParam',{datatype:'json'});
					//リフレッシュで検索関連のパラメータが消されないため、絞りこまれた状態で再表示となってしまう。
					//そこで検索関連のパラメータを削除する。_searchは消せないようだ。
					$('#'+maplayer_info_tabId).jqGrid('setGridParam',{postData:{searchField:'',searchString:'',searchOper:''}}).trigger('reloadGrid');
					return [true,''];
				}
			},
			//編集フォームオプション設定
			{recreateForm: true,
				editCaption:'<%=lang.__("Edit - Map layer info")%>',
				width:'auto',
				...
				//編集前データをシリアライズ化してpostDataに一時保存
					...
					$('#'+maplayer_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()+s}});
					//idをラベル表示にする。
				...
				onclickSubmit:function(opt, data){
					...
					return {'serializedPreEditData':$('#'+maplayer_info_tabId).jqGrid('getGridParam','postData').serializedPreEditData};
				},
				afterSubmit:function(){
					...
					//サブミット後、グリッド再取得
					var pagenum = $('#'+maplayer_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+maplayer_info_tabId).jqGrid('setGridParam',{datatype:'json',page:pagenum,postData:{editing:true}});

					return [true,''];
				},
				afterComplete:function(response, postdata, formid){
					//サブミット後、直前の編集データをシリアライズ化してpostDataに一時保存
					$('#'+maplayer_info_tabId).jqGrid('setGridParam',{postData:{'serializedPreEditData': formid.serialize()}});
					...
					var pagenum = $('#'+maplayer_info_tabId).jqGrid('getGridParam', 'page');
					$('#'+maplayer_info_tabId).trigger("reloadGrid",[{page:pagenum}]);
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			{recreateForm: true,
				width:'auto',
				addCaption:'<%=lang.__("Add - Map layer info")%>',
				bSubmit:'<%=lang.__("Save")%>',
				bCancel:'<%=lang.__("Cancel")%>',
				modal:true,
				closeOnEscape:true,
				...
				beforeShowForm:function(formid){
					...
					//行選択をはずす。
					$('#'+maplayer_info_tabId).jqGrid('resetSelection');
					//idは非表示。
					$('#tr_id', formid).hide();
					$('#tr_menuinfoid .CaptionTD', formid).after('<td id="dispmenuinfoid">'+row_id+'</td>');
					$('#tr_menuinfoid .DataTD', formid).hide();
...
//						var csrf_token = '${cookie.JSESSIONID.value}';
			...
			//削除フォームオプション設定
			{
				caption:'<%=lang.__("Delete - Map layer info")%>',
				...
				afterComplete:function(response, postdata, formid){
					...
					if(response.responseText){
						...
						if(res){
							if(res.message){
								showMessage($.jgrid.jqID(this.p.id), res.message, $.jgrid.nav.alertcap);
								$('#'+maplayer_info_tabId).jqGrid('setGridParam',{datatype:'json'}).trigger('reloadGrid');
							}
						}
					}
					return [true,''];
				...
				onClickButton:function(e){
					...
					if(rowid){
						...
						$('#'+maplayer_info_tabId).jqGrid(
								...
								'new',
								{recreateForm: true,
									addCaption:'<%=lang.__("Record copy - Map layer info")%>',
									bSubmit:'<%=lang.__("Save")%>',
									width:'auto',
									modal:true,
									closeAfterAdd:true,
...
//											var csrf_token = '${cookie.JSESSIONID.value}';
						...
						$('#'+maplayer_info_tabId).jqGrid('GridToForm', rowid, $('#FrmGrid_'+ maplayer_info_tabId).id);
						//idはコピーしない。
						$('#FrmGrid_maplayer_info #id').val('');
					}else{
...
//	$('#'+maplayer_info_tabId).jqGrid('gridResize');

	// 動的リサイズ。グリッド全体はmainFrameの画面幅の98%-開閉ハンドルで表示
	 $(window).bind('resize', function () {
	      $('#'+maplayer_info_tabId).setGridWidth(getGridWidth(thisGridLevel));
	  }).trigger('resize');

	//サブグリッド設定
	$('#'+maplayer_info_tabId).jqGrid('setGridParam',
