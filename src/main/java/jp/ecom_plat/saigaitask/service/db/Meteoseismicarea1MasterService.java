...
import static jp.ecom_plat.saigaitask.entity.Names.meteoseismicarea1Master;
...
public class Meteoseismicarea1MasterService extends AbstractService<Meteoseismicarea1Master> {

    public Meteoseismicarea1Master findById(Integer id) {
	 ...
	 * @return 検索結果
	 */
	public List<Meteoseismicarea1Master> findByCondition(Map<String, Object> conditions, String sortName, String sortOrder, Integer limit, Integer offset) {
		//ページャー条件
		...
		if(StringUtils.isEmpty(sortName)) {
			sortName = meteoseismicarea1Master().disporder().toString();
		}
		...
		if(!meteoseismicarea1Master().id().toString().equals(sortName)){
			orderByItemList.add(new OrderByItem(meteoseismicarea1Master().id(), OrderingSpec.ASC));
		}
	...
	public int getLargestDisporder(Map<String, Object> conditions) {
		conditions.put(meteoseismicarea1Master().disporder().toString()+"_IS_NOT_NULL", Boolean.TRUE);
		List<Meteoseismicarea1Master> list = findByCondition(conditions, meteoseismicarea1Master().disporder().toString(), DESC, NON_LIMIT, NON_SELECT_OFFSET);
	 ...
	 * @return
	*/
	public int update(Meteoseismicarea1Master entity, PropertyName<?>[] excludes) {
		if(excludes != null){
			return jdbcManager
				.update(entity)
				.excludes(excludes)
